...
Map explicit `$better-product-graph` and equivalent natural language through the same Host parser:
...
2. Run the installed `scripts/bpg_runner.py --self-check` before a state-changing intent. Resolve all paths relative to this installed Skill, never the source checkout.
