# Better Product Graph 2.x 产品 Roadmap v0.2

状态：CURRENT WORKING ROADMAP  
日期：2026-08-31  
产品代际：Better Product Graph 2.x  
上一份前向 Roadmap：`BETTER_PRODUCT_GRAPH_2_X_ROADMAP_v0.1.md`（冻结保留）  
权威产品方法：`docs/architecture/BPG_PRODUCT_PLANNING_METHOD_CONFIRMED_v0.2.md`  
当前公开源码版本：`2.0.0` Developer Alpha，BPG 2.0 单 PRD 主链为默认入口；GitHub Release 尚未创建

> 本 Roadmap 取代 BPG 2.x Roadmap v0.1 作为新的前向规划入口。它接受 Owner 对软件版本号的更正：完全替换旧架构的当前实现直接使用 `2.0.0`，成熟度继续由 `Developer Alpha` 单独表达。它不改写 v0.1、v0.17、旧架构、旧 Run、旧 Release 或历史验收结论。

## 1. 一句话结论

BPG 2.0 当前单 PRD 新架构直接使用软件版本 `2.0.0`，先以未发布的 Developer Alpha 源码快照进入公开仓验证；随后依次推进 2.1 多 PRD、2.2 轻量 Evals Generator、2.3 产品形态知识包、2.4 条件配置与专业 Reviewer、2.5 增强交付与 `eli-me`。软件版本号、发布通道和能力成熟度分别记录，不能再用旧 `0.2.x` 包版本掩盖产品代际。
...
| 插件软件版本 | 表示新架构的软件身份 | 当前 `2.0.0` |
| 发布通道 | 表示源码、Alpha、Beta、RC 或稳定 Release 的成熟度与冻结程度 | 当前 `公开源码 / Developer Alpha / Unreleased` |
| 文档与产物版本 | 表示某份 Roadmap、PRD、Profile、Schema 或 Review 的自身版本 | 各自在独立命名空间演进 |

公开包建议采用以下通道：

```text
0.2.20          旧版本线的最后一个冻结 GitHub Release
        ↓
2.0.0           新架构的软件身份；先发布公开源码快照
        ↓
Developer Alpha / Beta / RC / Stable
                通过仓库状态、Release 通道和证据边界表达成熟度
        ↓
2.1—2.5         在 2.x 线上继续交付后续能力里程碑
```

规则：

- `0.2.20` 保持不可变，不重打 Tag，不覆盖 Release。
- 当前完全替换旧架构的源码和插件身份直接使用 `2.0.0`；不再发布新的 `0.2.x` 或把同一架构命名为 `2.0.0-alpha.N`。
- `2.0.0` 的 Major 号表达架构代际，不自动证明产品成熟、稳定或已经发布；这些事实必须由发布通道与运行证据单独说明。
- 当前只推送 `2.0.0` 公开源码快照，不打 Tag、不创建 GitHub Release、不上传 Release 资产。
- `2.1`—`2.5` 是后续能力里程碑；对应软件版本在各里程碑形成可发布候选时单独决定。
...
- `0.2.20` 已作为旧版本线的过渡 Developer Alpha 发布；当前 BPG 2.0 已成为默认入口并改用 `2.0.0` 软件身份。
- Owner 在新任务中完成了一次初步实机使用，反馈“基本能跑通”；这证明基础可用性观察，不等于完整人类读者研究或产品效果验证。

### 3.2 尚未完成

- Owner 更完整、多场景的真人产品测试和问题回收。
...
因此当前状态仍是 **Developer Alpha**。软件版本为 `2.0.0` 不改变这些未完成边界。

## 4. 近期优先级：2.0.0 Developer Alpha 收敛
...
- 完成 `2.0.0` 源码、开发仓、公开仓和本地安装身份统一，结束“插件 0.2.20 内嵌 BPG 2.0”的过渡命名。
...
## 10. BPG 2.x 后续 Release Candidate 门槛

2.1—2.5 的任一正式 Release Candidate 都必须证明前序能力没有退化；具体软件版本在候选形成时确定，不反向改变当前 `2.0.0` 身份。
...
未来任何标记为稳定通道的 Release 都不能只因为“RC 没发现崩溃”就发布，至少还需要：
...
若这些条件未满足，继续使用 Developer Alpha、Beta 或 RC 通道，不用软件版本号冒充成熟度。
...
```text
统一开发仓、公开仓和本地安装的 2.0.0 身份
                    ↓
推送 2.0.0 公开源码快照，不创建 GitHub Release
                    ↓
继续收集并分级 Owner 实机问题
                    ↓
完成 Developer Alpha 收敛与全量回归
                    ↓
再启动小迭代 2.1
```
...
```text
BETTER_PRODUCT_GRAPH_2_X_ROADMAP_v0.3.md
```
