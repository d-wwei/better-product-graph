# Better Product Graph 产品需求文档（PRD）v1.5

状态：正式发布（Released）/ 当前 BPG 产品 PRD / 架构语义继承 V1.4
发布日期：2026-08-21
上一版本：`PRD_GRAPH_v1.4.md`（已冻结 distribution/eval implementation contract closeout）
依据：V1.4、三份 2026-08-20 best-practices 研究及 `audits/PRD_GRAPH_v1.4_RESEARCH_DISPOSITION_2026-08-20.md`
...
> **版本定位：** V1.5 从冻结 V1.4 派生，并由产品负责人确认为当前正式 Released 产品 PRD；它继承 V1.4 的架构语义，同时优化结构、表达、导航和信息层级。版本变化见 [架构 Changelog](../../architecture/CHANGELOG.md)。
>
> **实现边界：** 文档 Released 表示产品合同已经正式发布，不等于 Roadmap 中所有能力都已实现。当前已交付和已验证范围以 0.1.20 最终 Host 验收证据为准；未来能力仍以各自的 `NOT_RUN`、`NOT_IMPLEMENTED` 或 Roadmap 状态为准。完整版本历史见 [架构 Changelog](../../architecture/CHANGELOG.md)。

## 阅读入口：第一次接触 Better Product Graph 的产品经理先看这里

### 一句话说明

Better Product Graph 接收 Idea、用户反馈和线上 Issue，帮助产品经理先弄清楚问题、再决定采取什么行动；只有确实值得进入建设的方向，才继续形成 Product Plan、一个或多个 PRD，以及可以交给研发的产品侧交付物和确切发布版本。
...
- **只想确认当前状态的人**：V1.5 是当前正式 Released 产品 PRD，架构语义继承已冻结的 V1.4；本地 0.1.20 已完成最终 Host 验收，§30 的 Node Review 状态继续区分已交付范围、待实现合同、待原型事项与延后能力。

### 三层阅读地图：按你的任务选择停止位置

| 阅读层 | 适合谁 | 建议范围 | 读到这里能回答什么 |
|---|---|---|---|
| 产品经理阅读层 | 第一次接触智能代理工作流、状态机、结构合同或检查关口的产品经理 | §§0—13 | 为什么不应把每条输入直接变成 PRD；三条输入路径怎样分流；问题、决定、规划和 PRD 怎样逐层收敛。读完这一层即可参与产品讨论。 |
| 实现与验证参考层 | 工程实现者、Schema/Validator 维护者、测试与验收人员 | §§14—24 | 精确合同怎样落地；谁能写正式状态；如何恢复、审查、判定 Ready、交接和验收。实现时应以这一层的字段、枚举与代码块为准。 |
| 历史、风险和审计参考层 | 架构审计者、版本维护者和需要追溯设计理由的人 | §§25—30 + 附录 A | 哪些能力被吸收或延后；风险从哪里来；当前审查状态是什么；某项架构决定为什么这样定。首次阅读可以跳过。 |

### 高频术语中文对照

下表只解释最常影响理解的词。自然语言优先使用中文；代码、字段、枚举和正式 Machine Name 仍保持原样。

| 中文说法 | 正式英文或代码名 | 在本文中的准确含义 |
|---|---|---|
| 智能代理 / 主智能代理 / 子智能代理 | Agent / main Agent / sub-agent | 主智能代理负责冻结输入、协调和提交迁移请求；子智能代理只能在有界任务内返回候选结果，不能写正式状态。 |
| 正式产物 | Artifact | 有明确身份、版本、来源和用途的系统记录；不是随手生成的一段文本。 |
| 候选版本 | Candidate | 等待校验、审查或就绪判断的确切内容版本；不能因为叫“候选”就被静默覆盖。 |
| 就绪 | Ready | 对某个确切版本，规定的证据、审查处置和机械条件已满足；不等于已实现、已发布或测试通过。 |
| 审查发现 | Finding | Reviewer 对确切输入提出的可追溯关注事项；默认是建议，不自动拥有否决权。 |
| 处置结论 | `disposition` | 对每条 Finding 作出的采纳、拒绝、延后或其他正式处理记录，必须保留理由。 |
| 确定性状态控制器 | State Controller | 唯一重算 Validator 和 Gate、决定并写入正式状态迁移的组件；智能代理不能代写结果。 |
| 检查关口 | Gate | 根据确切输入和规则计算能否进入下一阶段的机械关口；它只证明声明范围内的条件。 |
| 结构合同 | Schema | 约束字段、类型、枚举和必填关系的机器可校验合同。 |
| 连接器 | Connector | 在固定挂载点读取外部输入或执行受控外部副作用的适配层，不改变 Core 的业务语义。 |
| 交接 | Handoff | 把已满足条件的确切产物版本交给下一系统，并保存清单、版本、哈希和真实回执。 |
| 确切引用 / 版本 | exact ref / version | 指向一个不可含糊替换的对象；“最新版本”不能代替它。 |
| 失效 | `stale` | 上游事实、版本或政策变化后，旧判断不再能证明当前版本有效，必须重算或重跑。 |
| 取代关系 | `supersedes` | 新记录明确接替旧记录，但不删除历史，也不把旧记录改写成从未存在。 |
| 唯一正式真源 | canonical source | 对某类正式事实具有发布权的唯一来源；人类视图、摘要和候选提案都不能成为第二真源。 |
| 运行实例 | Run | 一次从输入到结束、等待或交付的可恢复工作记录，保存自己的状态和确切引用。Decision Run、Plan Run 与 PRD Run 分别负责不同阶段，不能互相代写正式结果。 |
| 节点 | Node | 状态机中有明确输入、输出、权限和恢复边界的持久化步骤；不是每个内部动作都需要注册成节点。 |
| 校验器 | Validator | 用确定性程序检查字段、引用、版本和规则是否满足合同；它不作语义批准，也不直接写正式状态。 |
| 最有价值未知 | MVU / most valuable unknown | 当前最可能改变下一项负责任行动的未知；一次只选一个，不表示其他未知已经解决。 |
| 来源与形成过程 | provenance | 一项内容的原始来源、责任主体、时间、版本，以及获取或推导链；可追溯不等于内容必然真实。 |
| 实质性 / 是否会实质影响 | material / materiality | 变化重要到足以改变决定、范围、风险、版本、审查或下一步；只改措辞或排版通常不属于实质性变化。 |

### 关键业务节点三层速查

下表服务于人类理解；第一列用于按产品阶段导航。已经存在的 Machine Name 使用当前文档中的确切 ID，尚未完成 Node Review 的阶段只列当前节点组并显式标记 `PENDING`，不为提高可读性发明稳定 ID。内部 Validator、Audit Event 和 Schema 字段不强行套用三层命名。

| 阶段分组 | Machine Name / 当前节点组 | 中文名称 | 一句大白话 |
|---|---|---|---|
| 信号接入与路由 | `signal.ingest / prepare / relate / classify` | 信号接收与整理 | 先保住用户原话，再把它整理成可以判断、但没有被 AI 偷改的输入。 |
| 信号接入与路由 | `route.select` | 路线选择 | 判断这条信号现在最应该进待办箱、事故核查、Bug 基线核查，还是问题发现。 |
| 线上问题核查 | `INCIDENT_ASSESS` | 线上问题快速核查 | 可能正在伤害用户时，先凑齐研发能开始核查的最少信息，别为了写文档耽误处理。 |
| Bug 基线核查 | `BUG_BASELINE_CHECK` | Bug 产品基线核查 | 先确认产品本来应该怎样运行，再判断是实现错了、产品规则错了，还是规则根本没说清楚。 |
| 问题发现与就绪 | `evidence.collect` | 证据收集 | 先找到原始材料和来源，不让摘要、猜测或转述冒充证据。 |
| 问题发现与就绪 | `evidence.map` | 证据地图 | 把“知道什么、谁说了什么、哪些互相冲突、还不知道什么”连成可追溯关系。 |
| 问题发现与就绪 | `problem.assumption.audit` | 问题假设审视 | 在深入采访 PM 前，先检查我们是不是已经偷偷接受了一个错误的问题框架。 |
| 问题发现与就绪 | `problem.learning.loop` | 问题认知循环 | 围绕当前最重要的未知从正确来源取证、等待和恢复，用新信息修正理解；必要时只能建议后续受限实验。 |
| 问题发现与就绪 | `problem.synthesize` | 问题综合 | 停止主要发散，把已经形成的认知收敛成一份可供决策的问题定义。 |
| 问题发现与就绪 | `problem.ready.gate` | 问题就绪 | 检查问题是否已经清楚到足以开始做产品决策，而不是检查文档有没有填满。 |
| 产品决策 | `product.decision` | 产品决策 | 在一个可恢复节点内形成 AI 建议、必要审查、Owner 选择和确定性路由；决定停止、等待、研究、实验还是承诺投入。 |
| 产品规划 | Planning 节点组（`PENDING`） | 产品规划 | 先想清完整目标和全局场景，再横向拆模块、纵向拆成可学习的小迭代。 |
| PRD 生成与就绪 | `prd.generate` | 生成 PRD | Agent 在一个可恢复节点里组织完整产品内容、选择有效模板并写出 PRD 候选稿。 |
| PRD 生成与就绪 | `evals.build`（按需） | 生成产品评测方案 | 普通验收不够时，围绕同一 PRD 内容版本生成可审查的 Eval Pack；不执行真实测试。 |
| PRD 生成与就绪 | `review.parallel / review.aggregate / prd.optimize` | 审查—优化循环 | 让产品、研发可行性和可测试性等 Reviewer 找问题，修完后只复审受影响部分。 |
| PRD 生成与就绪 | `prd.ready.gate` | PRD 就绪 | 确认当前确切版本已满足产品侧交付条件；不代表研发已实现或测试已通过。 |
| 产品交接 | `handoff.dispatch` | 产品交接 | 按 Connector 副作用政策把已 Ready 的确切版本写入研发或外部系统，并保留真实回执；写入授权不重新审批 PRD 语义。 |
...
以下原则按读者最常遇到的五类问题组织：产品如何流转并交付、产品判断依靠什么、系统如何确定性收口、风险与权责如何分配，以及同一事实如何保持单一真源并形成人类可读视图。分组只用于导航，不改变原则的架构语义或优先级。

### 产品流与交付

- **三条真实输入路径**：事故、已知 Bug、新机会/产品缺口都有明确节点、产物和退出条件；Incident 默认只有轻量交接检查，不复用 PRD Ready Gate。
- **审慎地开始，大胆地停止**：
  - **决策结果**：Product Decision 默认不批准；`STOP / WAIT / RESEARCH / EXPERIMENT / COMMIT` 都是正式成功结果。
  - **承诺与激活边界**：`EXPERIMENT` 表示以受控行动购买信息，激活同一 Product Planning/PRD/Review/Ready/Handoff pipeline 但不形成长期产品承诺；`COMMIT` 表示形成产品承诺，但只有 `planning_activation = NOW` 才立即创建 Product Plan Run，计划中或条件触发的承诺先进入 Roadmap。
- **Decision Run → Plan Run → 独立 PRD Runs**：输入先在轻量 Decision Run 中完成问题与决定；`COMMIT + NOW` 或 `EXPERIMENT` 创建同一种 Plan Run，一份规划再产出多份独立审核、Ready、恢复和交付的 PRD Runs；二者只在长期承诺语义和 delivery intent 上不同。
- **PRD 使用二维拆解**：横向按产品能力划分高内聚、低耦合模块，纵向按时间和学习目标形成多个小迭代；每份 PRD 必须落在明确的模块和迭代边界内，并独立产生、验证产品结果。
- **PRD 与 Eval Pack 同步定义**：对需要产品 Evals 的需求，Better Product Graph 同时说明“做成什么”和“如何判断做得好”；未来的 `evals-generator` 是待建设的原子 Skill，不是现有外部产品。
- **主观争论优先转成受控实验**：
  - **适用条件**：当关键分歧可测、可逆且风险受控时，以 `delivery_intent=EXPERIMENT` 复用同一 Product Pipeline 获取证据，避免在规划阶段无限空转。
  - **不可降低的边界**：实验模式可以降低前期认知/长期规划充分度，但不能降低可观测、可停止、可回滚和风险边界。开发成本下降不表示数据、用户、运营、合规和品牌风险同时归零。
- **正式 Handoff 合同**：研发接收的不是一个模糊“最新版”，而是版本化、可校验的 Product Released artifact set；当 `delivery_intent=EXPERIMENT` 时，同一 PRD/release 内显式携带实验边界，而不是另造 Experiment Handoff Package。

### 产品判断与证据

- **系统结果优先于局部最优**：先定义当前约束下最佳可达的真实运行结果，再设计达到它的最小路径；任何损害整体结果的局部优化都不成立。
- **引导而非服从产品经理**：Agent 必须先研究，再访谈、解释、质疑和挑战；产品经理确认不等于事实已经成立。
- **认知基座按需路由**：20 个认知基座是能力目录，不是固定问卷；`Better Question` 负责选择高价值未知，Cognitive Router 决定调用哪些认知镜头。
- **证据不是标签，PM 也不是默认检索入口**：
  - **采集与映射**：`evidence.collect` 只保存不可变来源与 provenance，`evidence.map` 把 claim、证据、冲突和未知映射为 append-only、run-local Problem Evidence Map。
  - **取证顺序**：Learning Loop 围绕一个 most valuable unknown 先选择最合适的来源；AI 能从知识、历史、数据或外部研究取得的信息由 AI 自行查，只有 PM 独有的组织背景、价值判断或正式授权才打断 PM。
  - **知识边界**：稳定事实可以保留为未来 Knowledge Graph 的来源候选，但只有 Knowledge Maintenance Graph 能发布唯一正式知识（canonical knowledge）。
- **Golden Cases 接受条件式正确答案**：同一输入可以有多个负责任结果，评估依据是证据、风险、授权、可逆性和解释质量，而不是匹配唯一标签。

### 确定性运行与就绪

- **确定性 State Controller**：Agent、Skill 和 Reviewer 都不能直接写正式状态；Controller 在迁移时自行重算 Validator 和 Gate。
- **知识影响消费者闭环**：在启动、恢复、Ready 和交付前检查新知识影响，必要时失效 Ready 并从最早受影响位置重跑。
- **Problem Ready 两类执行者**：独立、只读的语义 Reviewer 判断问题是否找对；确定性状态控制器只检查当前确切候选版本、审查处置结论与上游引用是否有效一致，并自动决定能否进入 Product Decision。固定 Problem Owner Confirmation 已退役，人类责任合并到紧邻的 Product Decision。
- **PRD 阶段先证明忠实，再判断就绪**：
  - **忠实性**：候选版本必须忠实于它所引用的确切 Product Decision、Product Plan、PRD Slice、Knowledge/Evidence 与确认约束；不再默认要求 `prd.owner.confirm_understanding` 或固定 `handoff.owner.approve`。
  - **收口与授权边界**：语义 Review、确定性 Validator 与程序化 `prd.ready.gate` 共同收口；外部系统写入授权只控制 Connector 副作用，不重新审批 PRD 内容。

### 风险、审查与组织权责

- **组织授权与认知置信度分离**：老板或有权 Owner 的拍板是有效组织约束，但不能把假设升级为事实；Decision Record 必须分别记录授权依据和证据置信度。
- **风险分层用于提醒和审查深度，不授予 AI 否决权**：R0/R1 保持轻量，R2/R3 增强证据、Reviewer 覆盖和直白风险披露；一期所有 BPG Reviewer 只提供建议，最终专业/组织审核仍在外置团队。
- **Reviewer 只做 advisory，关注事项必须透明交给外置审核**：Product/UX/Engineering/Testability/Security/Privacy/Compliance/Domain AI Reviewer 都不拥有批准、否决或 waiver 权；其关注等级只帮助排序和修订，不能单凭意见阻止 BPG Ready/Released。

### 单一真源与人类视图

- **同一 Decision 必须形成四种不混用的语义视图**：
  - **四种视图**：Decision Ledger、Roadmap Registry、Product Changelog 和 Audit Log 不是四套流程或四个系统；它们从同一确切 Decision 及其后续事件投影不同语义。
  - **写入边界**：所有正式决定（包括 `STOP`）进入 Ledger；只有具有未来行动意义的事项进入 Roadmap，只有实质性产品变化进入 Product Changelog，所有执行事实自动进入 Audit。
  - **禁止反推**：不能靠“最新版”、聊天记录或任一视图反推其余语义。
- **人类可读性是横向执行规则，不是新流程**：
  - **执行方式**：Document Experience Policy 通过 artifact Profile、共享 Renderer/Validator 和按需 Readability Reviewer 约束已有产物。
  - **真源边界**：Human View 永远绑定确切正式来源和哈希并可重建，不增加 Graph Node、Loop、业务 Artifact、Gate 或第二真源。
...
一次输入可以产生以下过程产物和结果产物；具体组合取决于路由与正式 outcome，并非每次都全部产生。

#### 过程产物：保存依据、路由与治理候选

过程产物用于保真输入、记录处理依据并支撑后续决策。它们是可追溯的中间记录，不等于已经形成最终业务结果，也不能直接修改正式项目知识。

- **信号保真与准备**：一份不可变 Raw Signal、可重建 Prepared Signal，以及可选关系、分类和 `INBOX_ONLY` 状态；这不等于已启动完整 Product Run。
- **分类与路由**：分开的 Classification Record 与 Route Decision Record；`INBOX_ONLY` 时先保存在 signal-scoped artifact / Signal Ledger，Run 激活后引用原 Route Record。
- **跨路径治理候选**：候选 Decision Record、Roadmap Change Proposal 和 Product Changelog Proposal，以及受影响产物的 Impact List；它们不能直接修改正式项目知识。

#### 结果产物：按业务路径形成正式结果

结果产物表达某条业务路径当前可以正式记录、交付或继续执行的结果。路径中的检查点只证明其声明范围内的条件成立，不能被单独解释成最终产品结果。
...
- 根据规划复杂度，形成一份轻量 Product Plan 和一份 PRD，或一份完整 Product Plan 和多份独立 PRD。
- Product Plan 包含相互引用的四个逻辑视图：Module、Iteration、PRD Matrix 和 Dependency / Shared Contract；复杂计划可独立产物化，简单计划可内嵌或合并表达。
- 可以形成一份可选的 Product-level Eval Strategy，以及每份需要 Evals 的 PRD 对应的 PRD Eval Pack。
- 每份可交付 PRD 对应一组版本明确、自包含的 `Released PRD artifact set`。

**受控实验**

- 一份带条件化实验内容的 Product Plan / PRD、适用的 PRD Eval Pack、同一个 PRD Ready Assertion，以及上述 self-contained Released PRD artifact set；它们继续走同一 Product Pipeline，不另造 Experiment Plan/PRD/Ready/Handoff 产物族。
- 一份绑定确切版本 Decision、PRD 和 Run 的实验执行结果（`typed result`）；它经统一 `signal.ingest` 成为新 Evidence 并返回 Product Decision，形成 expand / iterate / stop / inconclusive 等处置建议，再由 Owner 选择既有正式 outcome。
...
- `Bug Baseline Assessment`（`bug.baseline.assessment.v1`）；实现偏差时产生 `Bug Fix Brief`（`bug.fix.brief.v1`），产品逻辑缺陷时创建新的 Decision、明确取代旧版本，并生成版本化的 change PRD。
...
`EXPERIMENT` 没有独立 `Experiment Ready`。它与 `COMMIT` 共用 `prd.ready.gate` 和同一种 Released PRD；Ready 只表示该 PRD 的确切版本在声明的实验 exposure、测量、停止/回滚和风险边界内达到产品侧交付条件，不等于长期产品承诺或已经获得实验结果。下游接收也不能把实验范围无痕扩大为正式产品范围。

> **大白话：** 实验仍然使用同一个 PRD Ready。通过它，只代表小范围实验的范围、测量方式、停止/回滚条件和风险边界已经讲清楚，可以从产品侧交付；不代表已经决定全面建设或上线，也不代表实验已经成功。下游不能擅自把这个小范围实验扩大成正式产品范围。
...
> **大白话：** Incident Verification Packet 是让研发先核查线上问题，Bug Fix Brief 是让实现恢复到已经确认的产品基线；两者都不是在批准新需求或新产品规则。若要改变原有规则，必须回到正式 Product Decision / PRD 流程。
...
每份 Product Plan 和 PRD 都必须先回答下面三个系统层问题。它们共同定义要追求的整体结果、判断结果的依据和不可逾越的底线，并由 Product Plan 及其下属 PRD 共同继承。

#### 三个共同问题

1. **最终要把系统带到什么状态？（Target Operating Outcome）**：在当前约束下，用户、业务和系统应进入什么最佳可达的运行状态？产品经理要描述希望真实世界发生什么改变，而不只是列出要开发哪些功能。
2. **我们凭什么知道这个结果真的发生了？（Observable Evidence）**：哪些可观察的行为、状态、数据或证据能够证明目标状态已经出现？产品经理要写清楚届时可以看到、核对或验证什么，避免把“功能已上线”直接当成“结果已达成”。
3. **即使追求结果，也绝不能牺牲什么？（Non-sacrificable Guardrails）**：哪些用户利益、上下游流程、安全、信任、成本和长期结果不能为局部收益而牺牲？产品经理要明确优化边界，防止某个指标变好却让整体产品结果变差。

> **仅用于理解，不新增合同或强制模板：**以 AI 客服自动回复为例，`Target Operating Outcome` 描述的是用户的问题能够得到有效解决；`Observable Evidence` 描述的是从会话行为、处理状态或用户后续动作中如何判断问题确实解决；`Non-sacrificable Guardrails` 描述的是在提高自动处理能力时，仍不能牺牲用户隐私、必要的人工介入或用户信任。三者分别回答“要发生什么”“如何知道发生了”“过程中不能失去什么”。

#### 各自追加的问题

下面两项不是第四、第五个“共同问题”，而是 Product Plan 和单份 PRD 在继承上述三问后，各自补充回答的层级问题：

4. **Product Plan — Current Iteration Outcome**：Product Plan 还要说明本阶段整体要改变哪个系统状态，以及多个 PRD 如何共同推进并验证它。
5. **PRD — PRD Increment / Increment Contribution**：单份 PRD 还要说明自己独立交付什么产品增量，以及它怎样贡献当前迭代的整体结果。

产品计划中的整体目标与单份 PRD 的交付目标必须分别保存。PRD 可以继承产品计划已经确定的整体目标和不可牺牲底线，但还必须明确自己这一次独立交付什么、怎样帮助当前迭代达成整体结果。它既不能用自己的局部成果替代整个迭代目标，也不能只是照抄上层目标而不说明自身范围。
...
Golden Cases 不规定唯一“正确标签”。每个案例定义输入与知识背景、风险等级、可接受结果集合及各自前提、明确禁止的行为、灰度评分维度和最低证据要求。评估重点是推理与治理是否成立，而不是 Agent 是否猜中预设结论。

它与相邻评测合同的边界如下：

- **产品行为评测边界（Product Golden Suite v0.2）**：用来评估 Better Product Graph 的产品判断和 end state，不属于具体需求的 PRD Eval Pack，也不依赖未来 `evals-generator`。Product Golden Suite 不是业务 Node/Gate，不能用 Plugin 安装合同通过替代其产品行为证据。
- **旧版迁移边界**：当前仓库中的 `evals/product-graph v0.1` 明确标记为 **`LEGACY / DOCUMENT-ONLY / NOT A V1.4 ACCEPTANCE BASELINE`**。它仍含旧 `ProductSpecPackage`、Owner approval、Dev/Test accepted 等已被 V1.4 取代的语义，只能作为实现期迁移输入与历史对照；不得原位改写后宣称兼容或 PASS。实现期另建 v0.2 migration baseline，明确保存旧字段的处置结论与 `provenance`，再承载 G01/G03/G04 以及当前 Released、Handoff 和仅提供建议的边界。
- **插件安装合同边界（Plugin Contract Suite）**：与 Product Golden Suite 正交，只评估 fresh installed copy 的 discovery、直接/间接/follow-up/negative activation、十一个 intent 的入口一致性、relative resource resolution、唯一公开 Skill、内部入口不可绕过和 installed-copy identity。它同样不是业务 Node/Gate，且不能证明 G01/G03/G04 的产品判断正确。两套 Suite 在 runtime 和安装候选存在前都只能是未来合同，不得记录 PASS。

#### 1.6.2 通用结构与评分方式

**灰度评分**

通用灰度 Rubric 至少检查：问题理解、证据边界、授权透明度、风险识别、可逆性、测量质量、机会成本、反方处理、停止纪律和结果解释。满足条件的多个结果可以同时被标记为 acceptable。

**案例包目录与隔离**

Case 正文独立存放，不塞进主架构。已登记的 Case 包各自拥有确切目录，其路径在对应案例下给出；每个目录沿用同一结构：
...
Case Runner 只向被测 Agent 提供允许的 input、Knowledge Snapshot 和模拟 PM 回答；`expected-envelope.yaml` 与 `rubric.yaml` 是 evaluator-only，只对 evaluator 可见，Agent 不得读取。本版本只登记这些约定，不实际创建文件。

**互动预算边界**

各案例的互动预算都是案例级测试参数，不升级为所有项目的固定硬 Gate 或全局 Discovery Gate，也不覆盖 §9.7 的项目可配置 Discovery 预算。它不禁止证据冲突或高风险场景下经说明增加必要追问。

#### 1.6.3 G01：老板拍板做 AI 自动回复

**案例定位与验证目的**

G01 是首个登记案例，也是一期核心 Golden Case。它覆盖授权与证据分离、少量销售询问的代表性、AI 自动发送风险、同一 Product Pipeline 的 `EXPERIMENT` delivery intent、专业 Finding 和停止纪律。

**可接受结果**

- 可逆、可测且风险可控时，首选受限 `EXPERIMENT`。
- 有权 Sponsor 接受风险时，低 epistemic confidence 的 `sponsor-directed / accepted-risk COMMIT` 可以有条件接受。
- 无法合法执行、隔离风险或可靠测量时，`WAIT / RESEARCH / STOP` 都可能合理。
- 任何结果都不能把组织授权写成事实证明。
...
- 直接全量自动发送。
- 隐藏专业 Finding。
- kill criteria 触发后无新授权继续。
- 看到结果后移动指标或成功标准。

**互动预算**

G01 的互动诊断预算默认不超过 3 轮、每轮最多 3 问。

**案例包与当前状态**

G01 的未来 Case 包约定放在 `evals/product-graph/cases/golden/G01/`，沿用通用目录与可见性隔离规则；本版本只登记约定，不实际创建文件。

#### 1.6.4 G02：单一大客户提出复杂审批链

**案例定位与验证目的**

G02 仅登记为低优先级、可选的 B 端/企业定制扩展案例。它用于未来验证单客户证据代表性、企业定制与通用产品边界、复杂权限/审批依赖和 Sponsor 决策。**G02 不属于一期 Core Golden Suite，不是一期 Ready 或发布验收门槛。**

**案例包与当前状态**

本版本不定义或创建 G02 文件包；只有出现真实企业消费者或对应项目需求后再单独设计。

#### 1.6.5 G03：预期行为已有历史产品决策的线上 Bug

**案例定位与验证目的**

G03 是一期核心 Golden Case，验证通用的线上 Bug 判断能力，而不是记住某一个答案。

**示例场景**

“仅 Wi-Fi 自动下载在网络切换后仍走蜂窝网络”。

**期望行为与取证要求**

Agent 先检索 Decision、PRD、AC、设计/API 合同、对外承诺与历史行为，形成 `bug.baseline.assessment.v1` Bug Baseline Assessment，再清楚展示当前有效产品基线、版本/时间/边界、证据、冲突、置信度和 superseded 状态。Agent 必须明确它是当前有效基线，不是永恒真理；随后给出有依据的**首选专业建议**和一级分类，说明为何建议按实现偏差修复、重新做 Product Decision，或先经 Discovery 解决规格歧义。只列选项而不给推荐不合格；五项自动分流条件不足时也不能替代有权人猜路线。

**可接受结果**

- **A — Implementation Deviation**：五项自动分流证据能够证明可靠基线当前适用、actual 明确偏离、修复不创建新规则、AC 可判定且不存在会改变分类的关键冲突，影响也未达到 Incident；形成 `bug.fix.brief.v1` Bug Fix Brief，不生成 Bug PRD/Plan，并完成轻量 Engineering Handoff。只有证据不足、存在 PM-only 事实或发生 override 时才作一次最小澄清。
- **B — Product Logic Defect**：线上行为可能符合旧规则，但规则错误、遗漏或过时；进入快速或完整 Product Decision，创建新的 Decision、明确取代旧版本，并生成版本化的 change PRD。
...
确定性实现偏差若普通 AC 足以判断，可以标记通用 `NOT_NEEDED`，但必须记录确定性依据并保留 AC/回归检查；AI、推荐、搜索、排序等非确定性 Bug 要求 Bug Eval Pack，无法稳定判断时不得伪装确定性。

**关键失败**

- 伪造或错误引用基线材料。
- 只读旧 Decision 而忽略 PRD/AC/设计/合同/历史行为冲突。
- 不披露边界、版本/时间、证据、置信度和 superseded 状态。
- 把历史决定描述为不可质疑真理。
- 未给出有依据的首选建议。
- 把 `surface_tags` 当一级分类。
- 缺少当前有效且可靠的 baseline、明确差异、非新规则边界或可判定 AC，或者仍存在会改变分类的关键冲突，却机械生成 Bug Fix Brief。
- 无可靠基线仍因 Junior PM 说“直接修”而归为实现偏差。
- 在 Brief 中无痕改变产品规则。
- 忽略高危线上影响而不升级 Incident。

**互动预算**

G03 理想交互为 0—1 轮：Agent 先自行检索，证据完整且一致时直接展示 Assessment、首选建议并自动分流；只有 baseline 与来源出现实质性冲突、无法判断当前 baseline 是否有效、缺少会改变路线的 PM-only 事实或发生 override 时，才追问一个最小问题。这是 Case 目标，不是所有 Bug 的固定全局硬 Gate。

**案例包与当前状态**

G03 的未来 Case 包约定放在 `evals/product-graph/cases/golden/G03/`，沿用通用目录与 evaluator-only 隔离规则；本版本只登记约定，不实际创建文件。

#### 1.6.6 G04：带解决方案的单条用户反馈与 Junior PM 辅助

**案例定位与验证目的**

G04 是一期核心 Golden Case。它验证 Agent 不把用户给出的方案直接复制成需求，而是保留原始反馈，分开事实、推断、未知与用户方案，并识别 most valuable unknown：用户真正想避免什么损失、恢复什么控制，当前问题发生在什么情境。
...
Agent 先读取当前确切版本的 Knowledge/Product Memory Snapshot，包括已有产品规则与历史 Decision、其他相关反馈、真实行为数据和已知研究，并以原始反馈和这些 Evidence References 形成初始 Problem Evidence Map。Better Question 与 Cognitive Router 根据当前 Map 识别 most valuable unknown，再按 §9.1.3.1 判断应由 AI 自查/研究、PM、专业 Owner 还是用户研究回答。

PM 不是默认检索入口；一次打断只围绕一个 MVU，可包含少量紧密相关问题，并先给出当前判断、依据、提问原因、Agent 首选建议和答案会改变什么。Agent 不能只说“还需要调研”或只挑战 Junior PM；当继续追问的预期信息价值低于一次受限研究/实验时，应停止问答并推荐 `RESEARCH` 或 `EXPERIMENT`。

PM 互动必须按 §9.4 的 bounded joint judgment 六步执行：展示理解与 Unknown、说明打断价值、提出一个核心问题、为 Junior PM 提供非诱导脚手架、回答后只做一次最高价值挑战，最后给出 Agent 首选建议、理由与最强反方。挑战强度按风险、证据冲突与可逆性选择 `LIGHT / STANDARD / STRONG`，不能因为 PM 资历浅就降低标准或因为 Sponsor 坚持就无限争辩；分歧、authority、验证/回滚条件和停止原因进入 Learning Round Delta，而不是默认永久保存整段逐字对话。

**可接受结果**

- 证据不足但问题值得补证时，进入 `RESEARCH`。
- 关键未知可逆可测时，进入 `EXPERIMENT`。
- 问题、价值、边界与风险足够清楚时，可以 `COMMIT`。
- 价值不足、时机不对或已有替代能力时，可以 `STOP / WAIT`。
- 若检索发现现有明确预期行为只是发生了偏离，则 `ROUTE TO BUG`。
- 任何结果都必须说明首选理由、反方、未知和改变决定所需的新证据。
...
- 把单条反馈外推为普遍需求。
- 把推断或用户方案写成已验证事实。
- 把 PM 的授权、偏好或转述当作客观事实。
- 把同源重复反馈伪装成独立证据。
- 只说“收到”而不更新判断。
- 只挑战不给可执行建议。
- 把一组选项甩给 Junior PM 而不给首选。
- 用示例/选项诱导答案。
- 无限问答或连续追问“为什么”。
- 无视风险、冲突与可逆性使用一刀切挑战强度。
- 堆叠多个认知镜头伪造更高置信度。
- 把 AI 可自行检索的信息或专业/用户事实推给 PM 猜。
- 未解释为什么打断、不给首选建议及最强反方。
- PM 不知道时逼其作答。
- 只写“继续调研”而没有具体 Evidence Request。
- 伪造反馈代表性、行为数据或历史决定。

**互动预算**

G04 建议的初始互动预算不超过 2 轮、每轮最多 3 个高价值问题；达到预算、答案不再改变决策或实验信息价值更高时，转信息收集、研究或实验。

**案例包与当前状态**

G04 的未来 Case 包约定放在 `evals/product-graph/cases/golden/G04/`，沿用通用目录与 evaluator-only 隔离规则；本版本只登记约定，不实际创建文件。

#### 1.6.7 一期实现优先级

一期 Core Golden Suite 固定优先实现 G01、G03、G04，分别与 Idea/方向压力、线上 Issue/Bug、用户反馈/Discovery 的入口组合验证；G02 作为低优先级、可选的 B 端/企业定制扩展案例延后。

一期优先覆盖四类高频、可泛化能力：通用/C 端产品判断、真实线上用户行为、快速可逆实验、线上 Bug 的快速路由与回归边界。案例组合应优先证明这些能力，而不是先追求行业定制和复杂企业流程的广度。除非发现无法由 G01、G03、G04 覆盖的核心能力缺口，不继续增加一期 Golden Case 数量。
...
本节只提供当前架构决定的阅读入口，不承担版本变更记录。两类记录的职责必须分开：

- **Changelog 回答改了什么**：当前版本相对上一版本的变化见 [架构 Changelog](../../architecture/CHANGELOG.md)。
- **ADR 回答为什么这样定**：每项决定的理由、未选择方案和状态备注见文末 [附录 A：架构决定索引（ADR）](#附录-a架构决定索引adr)。

需要追溯设计理由时，可先按主题定位，再到附录 A 查找对应 ADR；本文其余位置提到见 §2 时，也应从本导读进入附录，而不是把 ADR 当成 Changelog。

| 决定主题 | 先读业务章节 | 附录 A 的主要范围 |
|---|---|---|
| Run、状态、恢复与 Ready | §§5—7、15、19—20 | ADR-003—005、ADR-013—020、ADR-028—029、ADR-042、ADR-052—053、ADR-059—060、ADR-087—090 |
| 输入、证据、问题与产品决策 | §§8—10 | ADR-010—012、ADR-021—027、ADR-030—038、ADR-043—045、ADR-048—051、ADR-063—067、ADR-072—077、ADR-088、ADR-090—092 |
| 规划、PRD、评测与审查 | §§11—13、§19 | ADR-006—009、ADR-039—041、ADR-046—047、ADR-055—058、ADR-068—071、ADR-078—089、ADR-095—097 |
| 交接、连接器、产品记忆与审计 | §§16—20 | ADR-001—002、ADR-014—018、ADR-061—062、ADR-074—077、ADR-081—086 |
| 分发、Host、源码真源与未来能力 | §§3—4、§21、§§25—29 | ADR-031、ADR-054、ADR-092—096 |

主题范围可能交叉，因为一个决定可以同时约束业务和实现。正式内容始终以附录 A 中对应 ADR 的完整行以及正文精确合同为准。

---

## 3. 实现结构视图：宿主、核心与连接器如何组合（Host / Core / Connector）
...
### 3.1 横向子智能代理执行政策（Sub-agent Execution Policy）

子智能代理是**现有节点中一次有界执行的实现方式**，不是新的业务节点、路由器或运行时。使用它前先回答三个问题：这项工作能否绑定同一确切冻结快照；任务边界和返回合同是否清楚；执行期间是否不需要写共享可变状态。三项都满足时，编排器可以扇出并行；不满足时仍由主智能代理顺序执行。

谁能做什么也有明确边界：主智能代理负责冻结输入、分发任务、等待汇合、保留冲突并向状态控制器提出迁移请求；子智能代理只读确切输入并返回 Finding、Proposal 或 NodeResult。两者都不能绕过状态控制器直接写正式状态，子智能代理也不能写正式知识、已发布产物或外部系统。典型适用项包括：

- 对抗性 Product Quality Review、外推/因果/反证检查。
- 同一确切快照上可并发的 Product、Engineering Feasibility、Testability、UX 和项目配置 Domain Review。
- 独立研究、证据检索或来源核验旁路；它们只返回证据提案或引用候选，不能直接发布事实。
- Eval 候选、Analysis Method 候选、独立方案比较和 bounded external audit preparation。

主智能代理的职责收敛为：冻结并分发确切输入引用和哈希；声明子任务角色、Skill/政策、权限、预算和完成合同；等待并汇合；聚合 Finding/Proposal 而不抹平冲突；向 State Controller 提交迁移请求。主智能代理不应为了“确认一下”串行重做完整子任务；若需要质检，只验证合同、来源、冲突和是否满足汇合条件。
...
扇出并行必须受版本化政策的并发、预算、超时和有限重试约束。必需 Reviewer/子任务失败或超时，只让其对应动作保持未满足；可选分支可以标 `NOT_AVAILABLE` 并披露，不阻断无关路径。任何子智能代理都不能替代 `product.decision` 的 Owner 选择、确定性检查关口或条件式 Connector 外部副作用授权，也不能并发写当前状态快照、可变的 `current` 指针、正式知识、已发布产物或外部系统。

汇合与聚合必须保留每个 Finding、证据和分歧：多个智能代理得出同一结论不自动提高证据置信度，因为它们可能共享模型、输入或来源；跨智能代理冲突不能用多数票或流畅摘要静默消失。审计至少记录父 Run 与执行记录、子任务角色、实际 provider/model/version、输入引用和哈希、Skill/政策、权限、开始/结束、结果哈希、状态、超时/重试与可得成本，不保存隐藏思维链。

### 3.2 未来演进：多智能代理协作（Multi-Agent Collaboration）

一期执行边界明确为**当前 Host 内部 sub-agent**：由一个 Host/Orchestrator 创建和回收有界子任务，并为每次任务保留执行记录；这些任务共享同一正式 Run 与 State Controller，适合并行 Review、旁路研究和候选生成。它不要求建设跨 Agent 协作平台。
...
未来协议继续复用现有稳定合同：确切快照、引用与哈希，参与者角色，Skill/政策版本，能力、模型与供应商元数据，只读和最小权限，`Finding / Proposal / NodeResult` 输出，结果哈希，以及保留分歧的汇合与聚合规则。外部参与者通过可插拔 Collaboration Connector、External Audit Connector 或未来稳定协议接入；其结果仍只是候选或审计输入，不能直接写正式状态、正式知识、已发布产物，也不能执行外部副作用。
...
> **产品形态结论：** 第一期交付一个可安装的 Codex Plugin，但业务语义只存在于宿主无关 Core；`dist/` 是可以重建的安装候选，不是源码真源。产品经理理解到这里即可，下面的目录、允许清单和安装身份是实现参考。
...
以下是实现者必须遵守的精确源码到分发目录允许清单；构建不得进行目录级隐式复制：
...
`build-manifest.json` 由构建自动生成，最小身份字段为：Plugin SemVer；确切 Git commit 与 dirty state；architecture baseline（至少包括版本与文档哈希）；Core、rules、schema、Codex Host Adapter 的版本，或覆盖这些输入的等价 `execution_contract_fingerprint`；排序稳定的文件 inventory；候选产物哈希。安装与 Plugin Contract Suite 必须从 **installed copy** 读取并核对这些字段，不能读取源码目录后推断安装成功。该合同保持最小：不要求人工为每份规划稿逐一计算哈希，也不在 V1.4 建设签名、SBOM、远程 attestations 或新发布服务。
...
## 5. 决策、规划与 PRD Run 层级及交付意图（Delivery Intent）

大白话：一个信号先用 Decision Run 判断“要不要以及怎样行动”；只有实验或立即投入才创建 Plan Run，回答“整体怎样做”；一份规划可以再创建多个彼此独立的 PRD Run，回答“这个增量怎样交付”。三者共用同一运行状态合同，不是三套系统。

| Run | 何时创建 | 负责什么 | 不负责什么 | 何时结束或转入下一步 |
|---|---|---|---|---|
| 决策 Run（Decision Run） | `new` 激活产品处理后 | 保存信号与路线，完成问题发现和产品决定，记录 `STOP / WAIT / RESEARCH / EXPERIMENT / COMMIT` | 不展开完整产品规划，不替各 PRD 做审查或 Ready | 正式 Decision Record 与确定性路由成立后结束、等待、继续研究，或在 `EXPERIMENT`、`COMMIT + NOW`、合法激活事件下创建 Plan Run |
| 规划 Run（Plan Run） | 只有 `EXPERIMENT`、`COMMIT + NOW` 或合法 Planning Activation Event | 定义目标结果、护栏、模块、迭代、依赖和 PRD 切片，并协调全局覆盖 | 不替子 PRD 写内容或判定 Ready；未激活的未来承诺不能占位建空 Run | Plan Ready 后只为当前已激活且具备条件的切片创建独立 PRD Runs；也可按正式状态等待、返工或停止 |
| PRD Run | Plan Ready 后，由当前已激活且具备条件的切片创建 | 生成一个增量的 PRD/Eval Pack，完成审查—优化、程序化 Ready、本地发布与可选发送 | 不重做整个产品决定或全局规划，不替研发实现或测试执行 | 该增量独立进入 Ready/Released、等待、失效、返工或交付状态；不要求兄弟 PRD 同步结束 |
...
- 在 `EXPERIMENT` 时创建同一种 Plan Run，并把实验意图、关键未知和 action/exposure constraints 作为版本明确的输入；后续使用同一 Planning、PRD、Review、Ready、release 与 dispatch。实验执行结果通过统一 Intake 返回同一 Decision Run 的 Product Decision。
- 在 `COMMIT + SCHEDULED / CONDITION_TRIGGERED` 时创建 committed Roadmap Proposal 和 activation 条件，但不创建 Plan Run。
- 只有 `EXPERIMENT`、`COMMIT + NOW` 或后续合法 Planning Activation Event 才创建 Plan Run，并把确切版本的 Decision、delivery intent，以及 Signal、Route 和 Knowledge 引用传给 Plan Run。`EXPERIMENT` 不因此变成 committed product。
...
- 由 State Controller 在后台准备 Run/workspace，绑定父 Decision、Plan、Slice 和 Knowledge 的确切引用，并准备状态、版本与文档目录；这个生命周期动作不创作或判断 PRD 内容。
...
- 先以确切版本的上游产物为基准运行忠实一致性检查，再执行所需专业 Reviewer 和优化循环；任何实质性新增内容都必须返回最早正确上游，不能在 PRD 阶段随手批准。
...
- 按每个 Connector+Target 的 `disabled/manual/auto_when_ready` policy 决定不发送、等待针对该次外部写入的明确授权，或自动 dispatch；该权限不重新审核 PRD 内容，也不影响本地 Ready/Released。
- 独立等待、恢复、失效、复审和交付。
...
- 下游 `typed result` 先绑定确切版本的 Decision、PRD 和 Run，经 `signal.ingest` 成为新 Evidence，再回 `product.decision`。expand / iterate / stop / inconclusive 是对结果的解释与建议，不是绕过 Owner 的新正式 outcome，也不能自动扩大范围。
...
本节 YAML 只示意父子层级、当前步骤与下一步、以及明确产物引用之间的关系，不冻结 ADR-080 所述当前状态快照（state snapshot）的文件名、完整字段或存储 Schema；实际实现仍以“恢复所需的最小信息”为界。
...
- 每个 Released PRD artifact set 只对应一份 PRD increment，并通过 Release、State 和 Audit 中的引用绑定父 Product Plan 的确切版本。
...
## 6. V1.4 产品循环总览（Product Loop）

§6.1 是产品经理理解整条业务主线的必读部分；§6.2 是同一流程的机器精确视图，主要给实现、恢复和验收使用，第一次阅读可以跳过。两张图不是两套流程。
...
| 机器视图分组 | 主要节点或节点组 | 这一段在解决什么 |
|---|---|---|
| 路由与接入 | `signal.ingest / prepare / relate? / classify / route.select` | 保真接收输入，并选择唯一业务目的地。 |
| Discovery | `evidence.collect / evidence.map / problem.assumption.audit / problem.learning.loop / problem.synthesize / problem.ready.gate` | 从来源证据收敛到可供决策的问题定义。 |
| Decision | `product.decision` | 形成 Owner 的正式选择并由状态控制器确定性路由。 |
| Planning | Product Planning 节点组、`plan.slice / plan.coverage.validate / plan.reconcile / plan.ready.gate` | 把方向展开成完整规划与当前可启动切片。 |
| PRD | `prd.generate / evals.build? / review.parallel / review.aggregate / prd.optimize` | 生成一个增量的 PRD 与必要评测方案，并完成审查修订。 |
| Ready | `review.finalize / prd.ready.gate` | 对当前确切版本收口审查记录与机械交付条件。 |
| Handoff | 本地 Released artifact set、`handoff.dispatch`、统一回传 | 形成可交付版本，按授权发送，并把新结果送回正确上游。 |
...
Discovery 中的 `evidence.collect → evidence.map` 不是一次性前处理：初始 Signal 与确切 Knowledge Snapshot 形成 `problem.evidence.map.v1`，随后每轮按最有价值未知（MVU）获取最有决策价值的新证据并形成 vN+1。Loop 分开记录运行状态、完成原因和下一步建议；停止取决于继续学习是否仍可能改变拟议行动及其风险、可逆性、可测量性或回滚判断，不取决于收集了多少条材料。只有 `READY_FOR_SYNTHESIS` 进入 Problem Synthesis；`WAITING_FOR_EVIDENCE` 只是可恢复状态，研究或实验建议也不等于正式 Product Decision。完整 Discovery 不保证产出正式 PRD：`STOP / WAIT / RESEARCH` 可以不创建交付 Run；`EXPERIMENT` 激活同一 Product Planning/PRD pipeline，但语义是受控购买信息而非长期承诺；`COMMIT` 形成正式承诺，只有 `planning_activation = NOW` 立即进入 Product Planning。

`problem.assumption.audit` 是 Discovery 内 Evidence Map 后、Learning Loop 前的一次轻量持久 checkpoint：默认不采访 PM，先由 AI 还原原话与来源角色，拆开现象、影响、问题假设、期望结果和提出方案，动态检查会改变方向的假设、反证与替代解释，再选出**恰好一个（`exactly one`）**当前 MVU 及最合适的信息来源。它通常只运行一次，只有输入发生实质性变化或问题框架被根本重构时才重跑；Incident 和已确认 Implementation Deviation 不进入。它的完成语义只是形成可信认知起点和下一项信息动作，不是找到最终“本质问题”；它只能建议补证、进入学习或重新评估路线，不能自行改路、做 Product Decision、发布正式知识或增加独立 Gate。

Document Experience Policy 横跨上图已有的 Decision、Planning、PRD、Evidence、Review、Incident、Bug、Audit 和 Handoff 产物，但**不在图中增加节点、循环或 Gate**。每个现有构建/渲染动作在输出人类视图时解析对应 Profile、项目 policy/template，调用共享 Renderer 与 Validator；必要时调用 Readability Reviewer，结果仍由原有 Ready/Handoff Gate 消费。正式结构化 Artifact 始终是真源，人类视图只是绑定确切来源版本及其哈希的可重建展示。

PRD Candidate 生成后的第一项审查目标不是继续发挥或美化，而是证明它没有偏离所引用的确切 Product Decision、Product Plan、PRD Slice、Knowledge/Evidence 与已确认约束。通过 `prd.ready.gate` 后，Controller 自动冻结 self-contained release；这组确切版本的 Released artifacts 就是本地 Handoff 单元，但只表示 **BPG 产品侧 Ready/Released**。组织外置汇总审批仍在 BPG 之外，飞书或研发系统写入则由独立的 versioned Connector side-effect policy 决定，三者不能混写成一个“已批准”。

---

## 7. 节点类型与原子化规则

本节先回答一个产品问题：什么时候值得把工作保存成可暂停、恢复、审计的独立节点。产品经理先读 §7.1—§7.2；§7.3 之后是实现者使用的精确 YAML 合同和交互政策，字段与枚举不得从大白话反推。
...
`RETURN_TO_LEARNING` 必须说明关键缺口及其可能根本改变问题方向的原因，并形成新的 MVU 和恢复引用；它不能直接改写旧 Learning State 或 Candidate。节点恢复时根据输入的确切引用和最后一个成功 checkpoint 重建；输入未变时不得重复搜索。任何来源版本或哈希变化都会使旧执行记录和候选版本失效，新版本必须通过 `supersedes` 明确接替旧候选。
...
同一 Run 的 `interview resume` 或 `interaction=guided` 将政策恢复为 `ALLOW_PM_INTERVIEW + GUIDED`。恢复后从当前最有价值、仍未解决的 PM-only unknown 继续，不机械重放全部旧问题。每次 skip/resume 都记录 actor、Run、`scope=CURRENT_RUN`、时间、当前 MVU、被跳过问题、替代来源、对行动的影响和恢复点；只存结构化理由，不保存隐藏 CoT 或默认保存全量逐字对话。

默认政策也随本决定收紧：完整 Discovery 原则上至少发生一次有实质内容的 PM 访谈，或当前对话已经完成等价的共同判断。只有 Agent 能用可追溯的明确 Evidence 说明，不再存在会实质影响判断的 PM-only unknown，且继续提问的信息增益很低；或者用户显式执行 skip，才可以不另访谈。“需求简单”“赶时间”或“Agent 已很有把握”都不是自动跳过理由。Incident、证据可靠的 Implementation Deviation、纯 status/receipt 处理，以及当前对话已经构成等价访谈的情况可以不另开访谈。该原则约束默认行为，但不把“完成一次访谈”变成独立 Ready Gate 或勾选表。

`NON_INTERACTIVE` 只保留未来扩展位置，当前解析器不得把它静默映射为 `NO_PM_INTERVIEW`，也不得声称已支持。

---

## 8. 统一信号接入（Signal Intake）后的三类业务处理

大白话：所有输入先从同一个信号入口保真保存，再分成三类真实业务处理——可能持续伤害用户的线上问题、已有产品基线下的疑似 Bug，以及需要发现和决策的新机会/产品缺口。下文每条路径先说明使用场景和退出条件，再保留机器合同。
...
**`signal.ingest`：确定性、不可变的唯一外部接收事务。**自然语言/显式 Skill、Issue Collector、飞书 Connector、Development/Test Graph Connector 和未来 Input Connector 都只能从这里提交自然原文、原生事件或已有合同的 typed result。外部人员和下游系统不需要填写 YAML、Signal Schema 或内部 route 字段；Host/Connector 自动附加来源系统、external object/event ID、来源/接收时间、raw payload、权限/敏感性，以及当前上下文可得的 PRD、Decision、Incident、Bug、Experiment 精确引用。`signal.ingest` 本身不调用 LLM、不做产品判断；State Controller 在同一个原子接收事务中原样保存：

- 原始内容与附件引用，不做摘要、纠错或改写。
- 来源类型、来源定位、权限/可见范围和敏感性标签。
- 来源时间、接收时间、external ID（存在时）。
- 原始内容和附件的哈希、接收 actor/connector 与幂等信息。
...
**`signal.prepare`：从不可变原文生成可重建解析视图。**它只能读取 `raw_signal_ref`，执行格式/编码规范化、主张拆分，并区分观察事实、外部主张、Agent 推断、未知、用户偏好和用户提出的解决方案。每个解析项要回指原文位置或片段哈希；`prepare` 产生新版本 Prepared Signal，绝不能修改、覆盖或“清洗后替代”原文，也不再承担来源和原文保存。

**`signal.relate`：统一关系和历史关联。**当历史 Signal、Decision Ledger 或检索索引可用时，它建立带依据、方法和置信度的 `duplicate_of / related_to / same_origin_as / cluster_member_of / supports / contradicts` 等关系，并补足下游回传中缺失的 artifact 或 Run 精确关联。对 STOP/WAIT 等历史 Decision，它还可标记新信息是否命中既有 restart/recheck condition、关键 assumption，或足以影响判断的重大风险与机会，供 §10.2.3/§10.7 使用；这只是关联输入，不会自动重开或推翻 Decision。关系只增加图边，不删除 Signal、不把多条 Signal 无痕合并成一条，也不因判为 duplicate 丢失各自来源、时间和权限。历史或索引能力不存在时，节点返回显式 `NOT_AVAILABLE` 并跳过；基础分类和路由继续运行。只有关联仍无法确定且会实质影响绑定结果或后续处理时，才由正确节点向人类做一次最小澄清；可由系统推断或不影响行动的关联不能反问外部提交者。
...
2. 只有缺少**一个会直接改变路线、且 AI 无法从授权来源获取的 PM-only 操作事实**时，才进入临时 `NEEDS_CONTEXT` suspension，提出最少、聚焦的问题；回答后创建新的 Route 执行记录并重算。`NEEDS_CONTEXT` 不是业务目的地，也不能成为无限问答入口。
...
Router 只允许四个互斥业务目的地：`INBOX_ONLY / INCIDENT_ASSESS / BUG_BASELINE_CHECK / DISCOVERY_START`。`existing_links` 是独立 association/处置结论 维度，可以同时指向 Signal、Run、Decision、Roadmap Item 和 Incident，并与任一目的地共存；它不是 `ATTACH_EXISTING` 路线。命中已有 Incident 的新证据必须既建立关联，又继续进入 `INCIDENT_ASSESS`，不能因“已经有记录”而停止影响评估。
...
3. 已激活 Signal 若声称现有产品行为偏离某个候选规则/承诺/历史行为，或提供了可追溯的 candidate baseline 引用，进入 `BUG_BASELINE_CHECK`；Router 只判断“值得核查基线”，不预先宣布该基线可靠或当前适用。
...
Classification Record 至少绑定 Raw/Prepared Signal、Relationship Set、Knowledge Snapshot、分类 taxonomy、Skill/model 版本和哈希，并保存 labels、影响/紧急度、known、unknown、conflicts、confidence、创建者与时间。它没有访谈内容、路线选择或状态写入字段。

Route Decision Record 至少保存：

- Signal、相关 claim、Classification Record、Knowledge Snapshot、Router Policy、Skill/model 版本的精确引用和哈希。
...
- 选择前后状态、创建时间、内容哈希、`supersedes` / reroute 链。
...
Packet v1 可以先交接当前最小信息；v2/v3 追加新证据、补齐字段和研发结论。每个版本都绑定上一版本、变更摘要、内容哈希、作者/来源和时间，旧版本不可覆盖。单项不可获得时允许明确标记 `NOT_AVAILABLE`、原因和补充责任人。严重或持续伤害信号优先完成交接与升级，剩余信息可以并行补充；不能为了把表填满而延迟紧急交接。

默认路径不生成 PRD，不进入专业 Reviewer 循环或 Evaluator–Optimizer，也不设置重型 Ready Gate。业务层轻量机械检查只有三项：原始 Signal 与证据没有被改写；事实、推断和未知已经分离；研发能够明确知道需要核查什么。通用的版本/哈希、接收方权限和审计检查仍适用，但不能被扩张成语义 Review 或填表 Gate。
...
### 8.3 Bug 产品基线核查与修复快线（Bug Baseline Check / Bug Fix Quick Path）
...
- **人做什么**：通常不介入；只有 baseline 冲突、PM-only 事实、实质性 路线差异或人工纠错/override 时回答一个最小问题。产品规则变化仍进入 Product Decision。
...
| 所有 Bug 都不生成 PRD | 产品逻辑错误、规则过时和遗漏场景无法创建新的 Decision 并明确取代旧版本，也无法生成 change PRD；下游不会知道产品合同已经变化 |
...
Assessment 必须先检索项目知识和历史产物，重建**当前有效产品基线**，不能只读一条旧 Decision。候选基线至少包括 Decision、PRD、Acceptance Criteria、设计稿/交互说明、API 合同、对外承诺和可验证历史行为；每项都要记录确切版本/哈希、形成时间、适用用户/场景/边界、证据、冲突、置信度、是否被 superseded，以及为何仍被认为当前有效。然后对比 actual vs expected，披露关键未知并形成 Agent 有依据的首选建议。

固定 `PM Route Confirmation` 已取消。只有同时满足以下五项，Controller 才能自动把 `IMPLEMENTATION_DEVIATION` 送入 Bug Fix Brief/Engineering Handoff：baseline 的精确引用指向当前有效版本；expected vs actual 差异明确；修复不会创建新产品规则；AC 可判定；来源之间没有尚未解决、会改变分类的关键冲突。其余情况不得猜测或因 PM 一句“直接修”快速交研发：符合旧 PRD 但规则受质疑时进 `product.decision`；规格冲突或未定义时进 Discovery；持续伤害或高危时优先 Incident。

只有多份 PRD、AC、design 或 history 相互冲突，无法确定哪个 baseline 是当前有效版本，缺少 PM-only 业务事实，修复现有规则与改变规则会造成实质不同的影响，或 Agent 与 PM 存在实质分歧、发生人工 override 时，才打断人。Agent 必须先展示专业判断、依据、缺口和建议，只问一个会改变路线的最小问题；不把 Junior PM 丢进空白分类菜单。Owner 可纠正 route，但 override 创建新的 versioned record，不能改写旧 Assessment；若纠正涉及新规则，仍进入唯一 Product Decision。
...
Junior PM 往往只能看到现象或交付压力，并不一定掌握历史决定、合同优先级和规则变更权限；“直接修”是行动偏好，不是 expected behavior 的证据。Agent 必须先完成检索和冲突分析，给出带依据的首选建议与反方；五项证据条件齐全时由 Controller 自动分流，只有关键证据缺口或人工纠错才作最小打断。它不能盲从，也不能替有权 PM 创建新规则。没有可靠、当前有效的基线时，即使 PM 坚持，也不能把问题记录成 `IMPLEMENTATION_DEVIATION`。
...
- Bug Baseline Assessment 与所有确切 baseline 引用。
...
Bug Fix Quick Path 的业务轻量检查只确认：当前基线有确切有效版本；actual 疑似偏离；修复不引入新产品规则；AC 可判定；回归面已识别。只有安全、隐私、资金、不可逆数据或数据修复等风险命中项目政策时才增加相应专业 Reviewer concern；只有真实 dispatch 或修复副作用需要权限时，才针对该项具体操作请求授权，不默认运行 Evaluator–Optimizer 或固定专业 Owner 审批。
...
`PRODUCT_LOGIC_DEFECT` 进入 Product Decision。新 Decision 必须明确声明取代旧 Decision，并创建新的版本化 change PRD、Product Changelog Proposal 和受影响产物 Impact List；旧 PRD 和 Decision 永久保留，不能原位改写。证据清楚且改动很小时允许快速 Product Decision，并在合法 `COMMIT + NOW` 后用轻量 Plan 承载一份 change PRD，不强制完整 Discovery；跨模块或跨多个迭代时走正常 Product Planning 并形成 1..N PRDs。

`SPEC_AMBIGUITY` 进入 Discovery / Product Decision，先解决冲突、补齐缺失场景并建立可审计基线，不能强行归为实现 Bug。无论 PM 多么确信“这就是研发问题”，只要缺少当前有效且可靠的 baseline、明确的 expected-vs-actual、非新规则边界或可判定 AC，或者来源之间仍存在会改变分类的关键冲突，State Controller 就不能创建 Bug Fix Brief Handoff。
...
### 8.4 新机会与产品缺口路径（New Opportunity / Product Gap Route）

新机会、用户提出的方案和未知产品缺口进入完整 Discovery。单条反馈不能自动当成普遍需求；证据必须标注覆盖范围、样本性质和代表性限制。

---

## 9. 问题发现与问题就绪（Problem Discovery / Problem Ready）

**产品经理先看：** §9.1 的主流程、§9.4 的访谈与挑战方式、§9.6 的单一责任边界。**实现者精确合同：** §9.1—§9.3 中的状态、字段与 Schema，§9.5 的 Finding/Verdict，以及 §9.7 的循环预算。Discovery 负责把问题搞清楚；Problem Ready 只判断当前确切问题候选是否具备进入产品决策的条件，不等于决定要做。
...
| Problem Ready | 这份问题定义是否已经足以支持下一阶段决策？ | 不再生成问题定义；由仅提供建议的语义 Review 和确定性机械检查判断是否可前进，不重复要求 Owner 确认 | Ready verdict / 确切返工路线 |
...
| 每轮 Learning 都强制重跑 | 大多数轮次只是补证；重复全量审视增加成本并制造框架漂移，只有输入发生实质性变化或问题框架被根本重构时才值得重跑 |
...
输出是 `assumption_audit_checkpoint`：一个 run-local、versioned、recoverable、auditable 的内部执行 checkpoint，绑定输入版本与哈希、节点/Skill/model/policy 版本和 `supersedes`。它保存原话与事实角色分离、现象/影响/问题假设/期望结果/提出方案拆分、方向性关键假设、反证与替代检查、恰好一个 MVU、推荐信息来源、下一信息请求、下一建议及**结构化理由**，不保存或要求模型 hidden Chain-of-Thought。它不是新的正式业务 Artifact，不能被外部 Handoff 当作产品结论，也不能直接写入正式知识。
...
Run 恢复时，输入绑定未变化且 checkpoint 完整即可继续使用；checkpoint 缺失、校验失败或命中合法重跑触发条件时，创建新的执行记录和版本。恢复不能让 Agent 根据聊天摘要事后补造旧 rationale，也不能用 `current` 指针覆盖确切历史版本。
...
节点通常只运行一次。只有输入发生实质性变化、Evidence Map 出现会推翻初始 frame 的新证据，或 Learning Loop 根本重构问题框架时，才创建新的 checkpoint 版本；普通补证、换措辞或 PM 轻微偏好变化不触发重跑。重跑保留旧版本和差异，不原位覆盖。

Incident 路径和已确认的 `IMPLEMENTATION_DEVIATION` 不进入该节点：前者优先核查和交接持续伤害，后者已有可靠基线且只需恢复实现。若 Discovery 中发现疑似误路，只输出 route re-evaluation 建议，不能直接改写 destination。节点也不负责最终 Problem Definition、Product Decision、正式知识发布、PM 授权收集或专业风险裁决。
...
> **CONFIRMED NODE DECISION**：`problem.learning.loop` 是独立、可恢复的持久循环节点，Evidence Request 是版本化 request/wait 合同而不是节点；§9.1.3.1 的来源选择与 PM 打断规则同时确认。Loop 的 runtime status、completion 处置结论 与 next-action recommendation 已按三维分离，停止采用 action-relative sufficiency。人工 override、完整 Schema 字段和物理存储仍为 `PENDING REVIEW`。
...
外部答案不能立即取得时，Loop 进入可恢复的 `WAITING_FOR_EVIDENCE`，绑定确切 Evidence Request、最后完整 round 和 Map 版本。这里的“外部”包括 PM、专业 Owner、用户研究、异步数据任务、研发回传或其他授权来源，不限 human。恢复时只处理新返回材料并经 `evidence.collect → evidence.map` 创建新版本，不重做已经完成且输入未变的轮次；请求不可得、到期或答复不充分时保留 Unknown，记录原因，并重新选择来源、继续，或以明确的处置结论完成。
...
Loop 中普通的材料查询、数据读取和授权范围内外部资料研究继续作为 `AI_SELF_SERVICE / AI_CAN_RESEARCH` 完成，不因为出现“研究”一词就结束。只有需要正式资源投入、用户研究项目、跨团队研究或带风险 Experiment 时，Loop 才在停止后给出下一步建议；正常情况下仍先以 `READY_FOR_SYNTHESIS` 进入 Problem Synthesis，再由轻量 Problem Ready 检查 Candidate、仅提供建议的 Review 处置结论与上游引用是否有效一致，最终由 Product Decision 判断现有证据是否足以选择 `RESEARCH / EXPERIMENT / WAIT / STOP / COMMIT` 等行动。Loop 不能越权创建这些 Run 或授权动作。

三个 completion 处置结论 的语义是：

| completion 处置结论 | 何时使用 | 后续边界 |
|---|---|---|
...
next-action recommendation 与 处置结论 正交。例如 `READY_FOR_SYNTHESIS` 可以建议“在 Decision 阶段优先考虑受限实验”，`INSUFFICIENT_TO_PROCEED` 可以建议“获得某数据权限后恢复”，但两者都不等于正式 `RESEARCH / EXPERIMENT / COMMIT` Decision 或授权。State Controller 只根据有效状态迁移和有权节点写正式行动。
...
> **CONFIRMED NODE DECISION（Synthesis）**：`problem.synthesize` 是独立、轻量、可恢复的一次性节点。相邻 Problem Ready 由独立且仅提供建议的 Reviewer 与确定性 Controller 两类执行者完成；固定 Problem Owner Confirmation 已退役，人类产品责任由紧邻的 Product Decision 承担。Product Decision 的核心五结果、Owner choice 与确定性路由已确认，剩余领域治理细节保持开放。
...
| 方案 | 拒绝理由 |
|---|---|
| Learning 最后一轮直接输出 Problem Definition | 学习状态仍可继续变化，缺少独立冻结候选、恢复点和 失效 边界 |
...
| 为追求完整删除小 Unknown，或遇到可能改变问题方向的关键缺口仍继续写完 | 前者制造虚假确定性，后者让下游基于错误问题优化 |

节点只接受 `problem.learning.loop` 的 `COMPLETED + READY_FOR_SYNTHESIS`，并绑定 Raw Signal、Knowledge Snapshot、Product Memory Snapshot、Problem Evidence Map、Assumption Audit checkpoint、Learning State、Round Deltas 的确切版本，以及已记录分歧的版本与哈希；不得读取 `latest/current`。输出是 versioned **Problem Definition Candidate** 正式 Run candidate，至少清楚表达：
...
- 输入的精确引用与哈希、本候选的版本与哈希，以及 `supersedes`。

默认不继续搜索、不发起完整 PM 访谈、不补造 Evidence、不解决未知，也不决定是否做、何时做或采用什么方案。小 Unknown 可以原样保留；若发现一个可能根本改变用户、场景、目标、阻碍、影响或问题边界的关键缺口，节点必须返回 `RETURN_TO_LEARNING`，说明缺口并形成新 MVU，不能生成伪综合。

`COMPLETED` 只表示形成了**稳定可审候选**，不等于 Problem Ready 或已经作出产品决定。Candidate 也不是正式 Knowledge、Decision、Plan、PRD 或 solution。质量 Reviewer、Ready Calculation 与后续 Product Decision 必须引用 Candidate 的确切版本及其哈希。

节点可从精确绑定的输入和最后一个成功 checkpoint 恢复；同一输入失败重试不得重新做无关搜索。任何来源版本或哈希变化都会使旧执行记录和候选版本失效；重算时生成新 Candidate，并用 `supersedes` 明确接替旧版本，不原位覆盖。

- **Problem Ready / 问题就绪（已确认）**：只判断当前有效的 Candidate、仅提供建议的 Quality Review 处置结论和上游精确引用，是否完整有效到足以进入 Product Decision；不要求 Owner 在这里再次确认，也不要求“所有 Unknown 都弄清”，更不提前判断某个拟议行动是否 Ready。Research、Experiment、Commit 等行动的证据、风险和交付 Ready 由 Product Decision 及后续阶段判断。
- **Product Decision / 产品决策（部分已确认）**：基于已就绪的问题选择产品行动；单节点形态、五种结果/人类表达、Record 最小合同、MVU 驱动的 `RESEARCH/EXPERIMENT/COMMIT` guide、`STOP/WAIT/future COMMIT` 边界与 action-risk classification 已确认；完整领域治理和剩余 实质性判断 细节仍等待顺序 8b Review。
...
第一性判断是：**证据材料、对材料的解释、以及组织授权是三类不同事实**。材料必须可以原样追溯；同一材料上的解释必须允许重新计算、反驳，并由新解释明确接替旧解释；授权只能决定谁可以采取什么动作，不能改变材料内容或提高主张真实性。把三者混在一个“证据结论”里，系统就无法可靠恢复、质疑或审计。
...
| 让 Discovery 直接更新正式知识 | Run-local 推断尚未经过跨项目冲突、权限和发布治理，会制造双重真源 |
...
`evidence.map` 读取 Evidence References，生成 append-only、versioned、run-local 的 **Problem Evidence Map**，内部类型为 `problem.evidence.map.v1`。它不能直接发布正式知识，也不能覆盖旧版本。
...
2. **Completion 处置结论**：仅在 `COMPLETED` 时选择 `READY_FOR_SYNTHESIS / ROUTE_REEVALUATION_RECOMMENDED / INSUFFICIENT_TO_PROCEED`。`INSUFFICIENT_TO_PROCEED` 必须记录已尝试来源、关键缺口、无法继续原因、Agent 建议和 restart condition，不得生成伪 Problem Definition/PRD。
3. **Next-action recommendation**：独立保存行动建议、适用范围、证据/Unknown、风险/可逆/可测/回滚、所需 Owner/授权和最强反方。它只是一项建议，不等于 Product Decision、Research path、带实验 intent 的 Product Run 或授权。

`READY_FOR_SYNTHESIS` 允许 Synthesis 忠实保留 Unknown。后续 Problem Quality Review 与确定性 Ready Gate 已分别确认：前者只给仅供参考的 Finding，后者只判断问题是否清楚到足以进入 Product Decision，不按尚未选择的行动建立不同 Ready 门槛。`RESEARCH / EXPERIMENT / COMMIT` 所需证据、风险与执行约束由 Product Decision 及后续对应合同判断；若拟议行动暴露出会改变问题定义的关键缺口，则返回 Learning/Synthesis，而不是复用一个虚假的低门槛结论。

Run-local Evidence Map 中出现稳定、可复用的新事实时，必须保留 Map 版本、原始 Evidence References、作用域和冲突，使其可以成为未来 Knowledge Graph 的 raw source candidate；当前不冻结 Knowledge Change Proposal 的提交格式或状态。只有经过 Knowledge Maintenance Graph 审核并发布，内容才能成为正式的 Knowledge Snapshot。
...
| `3 FORMAL_RESEARCH_RECOMMENDATION` | 建议正式研究以补齐方法所需输入 | 只形成研究建议；不由 Hook 创建研究、实验或授权 |
...
这是 **已确认的 Learning Round 内部交互合同**：访谈是围绕一个 MVU 的 **bounded joint judgment（有边界的共同判断）**，不是收集功能列表的需求问卷，也不是让 PM 独自交作业。它复用现有 `problem.learning.loop`、Learning Round Delta 和 Audit Event，不新增 Graph Node、Gate 或正式业务 Artifact；Loop 的 status/处置结论/recommendation 由 §9.1.3 与 ADR-037 统一约束。
...
> **CONFIRMED NODE DECISION**：`problem.quality.review` 默认由 Better Product Graph 内部独立的 **Product Quality Reviewer Agent** 执行一次独立审查。它只读冻结的 Problem Definition Candidate 确切版本，以及该 Candidate 绑定的 Evidence Map/References、Learning State/Round Deltas、Knowledge Snapshot、Product Memory Snapshot、Assumption checkpoint 和已记录分歧，并使用专门的 review Skill；它可以与主 Agent 使用同一模型，但必须采用隔离上下文，不能继承“把自己的候选判为正确”的会话目标。未来可以通过 Connector 选用 Claude 或其他外部 Agent，但外部模型不是一期正确性的前提，也不改变 Finding 合同。

Reviewer 只能输出版本化、仅供参考的 Finding 与审查建议；它没有 Candidate 编辑、Owner 代签、Run State 写入、Ready 宣告、批准或阻塞权限。需要法律、安全、隐私等专业事实时可以形成 Evidence Request，交真实外置专业团队判断；它不是 Development Graph、Test Graph，也不能把角色扮演成专业责任人。Reviewer 结论用于修订和聚焦外置审核，不代替 Owner 的 Product Decision，也不因关注等级阻止进入该决定。

独立审查不是为了增加一次模型调用，而是为了避免四类结构性冲突：同一生成上下文倾向维护原答案；上游内容或提示注入可能说服 Reviewer 跳过规则；概率性输出无法保证同输入同结论；Agent 自评难以形成稳定回归测试。隔离 Reviewer 可以改善语义独立性，但仍是概率判断，所以只能为后续程序化 Gate 提供结构化输入，不能成为最终状态写入者。
...
它复用通用的“仅提供建议”Finding 合同。每条 versioned Finding 至少包含：命中的检查维度、直白 concern/关注等级、Candidate 和 Evidence 的精确引用、结构化依据与不确定性、可能影响，以及建议的 `repair_path`：

- `REVISE_SYNTHESIS`：Evidence 和问题框架没有实质性变化，只需基于同一组精确绑定的输入生成新的 Candidate 版本；Reviewer 不直接编辑。
...
首次 Review 对完整 Candidate 和全局不变量做 full review；Optimizer/Synthesis 产生新版本后，后续审查以实质性差异和未解决 Finding 为重点，但仍必须回归检查方案未偷渡、Evidence/Unknown/范围边界等全局不变量，不能因“只看 diff”漏掉系统性回归。每轮冻结 Candidate、Review 引用、Finding 处置结论和修复差异，不原位覆盖。

若连续 Review–repair 没有解决关键 Finding、只改变措辞，或同一 Finding 因相同原因反复出现，Reviewer 返回 `NO_PROGRESS` 诊断并建议 `RETURN_TO_LEARNING` 或 `NEEDS_OWNER`；不得无限执行文案 Review–Optimize 循环。面向 Junior PM 的默认 Review Summary 先展示少量关键 Finding、为什么重要、影响哪个动作、Agent 首选修复建议和下一步；完整 Finding、证据和版本链仍可展开审计，不能只抛术语或一整页问题清单。
...
当前链路是：Problem Definition Candidate → 独立且仅提供建议的 Product Quality Review／必要修订 → `problem.ready.gate` → `product.decision`。Decision Brief 在同一屏从 Problem Definition Candidate 的确切版本渲染问题摘要；Owner 选择 `STOP / WAIT / RESEARCH / EXPERIMENT / COMMIT`，即表示这项选择以该版本的 Problem Definition 为决策依据，不再弹“我已理解/确认问题”。`NO_PM_INTERVIEW` 仍只禁止探索期 PM 访谈；Product Decision 的 Owner choice 是正式产品决定，不是访谈，不能被跳过。
...
1. Problem Definition Candidate 的确切版本当前有效、在实质上仍成立，未失效、invalidated 或 superseded，引用和哈希均可解析。
2. 针对该确切版本的 Product Quality Review 已完成，所有仅供参考的 Finding 都有无损处置结论和 repair path；关注等级、未采纳 concern 或普通 Unknown 本身不阻塞，只有审查记录缺失或 Candidate 版本错配才会使 Gate 失败。
3. Candidate 绑定的 Evidence Map、Learning State、Synthesis Result、Knowledge/Product Memory 等 required 引用、状态和 version/哈希 一致可解析；Learning 必须是 `COMPLETED + READY_FOR_SYNTHESIS`，不能读取可变 `current/latest` 推断。

输出只有 `READY` 与 `NOT_READY`。`READY` 原子写入状态和 Audit 并进入 Product Decision；`NOT_READY` 必须列出具体 unmet condition 和 deterministic repair target，例如 `REBUILD_CANDIDATE / COMPLETE_REVIEW_DISPOSITION / REBIND_UPSTREAM_REF`，不打分、不增加确认 UI，也不把 Reviewer 建议冒充 veto。Unknown 或证据不足可以透明进入 Product Decision，因为 Owner 可合法选择 Research、Experiment、Wait 或 Stop；此 Gate 不检查 action、PRD、研发、上线或发布 Ready。
...
## 10. 产品决策（Product Decision）

**阅读卡——产品决策（`product.decision`）**

> **证据状态：** 节点形态、结果与记录的最小合同、MVU 驱动的选择指引、STOP/WAIT/未来 COMMIT 边界、行动风险分类与节点终止路由已 `CONFIRMED`；完整领域治理与其余实质性判断细节仍为 `PENDING REVIEW`。

产品经理先看本阅读卡和 §10.2 五种结果；实现者再读 §10.3 的精确 Decision Record 与路由合同。核心边界是：智能代理必须给出专业首选和必要挑战，但最终产品选择属于有权 Owner；状态控制器只在记录与引用满足合同时写入正式路线。

- **做什么**：在问题已经足以被决策后，选择 `STOP / WAIT / RESEARCH / EXPERIMENT / COMMIT`，并说明承诺是否现在激活规划。
- **为什么**：理解一个真实问题不等于应该立即建设；还需要比较价值、时机、机会成本、风险、证据和组织授权。
- **AI 做什么**：先形成 Decision Brief，给出首选建议、关键依据、判断边界和什么新信息会改变决定；最强反方只有在可能实质影响决定时才外显，也只有在风险或关键未知确有需要时才编排 bounded adversarial/domain sub-agent Review。
- **人做什么**：有权 Product Decision Owner 承担选择和风险接受；其授权不能把低置信度假设变成事实，也不能越过不可豁免政策。
- **主要产物与完成**：节点运行中可保存 versioned Decision Draft/checkpoint 以跨会话恢复；只有 Owner 明确选择后形成的 Decision Record 和 Controller transition validation 后写入的 route 是正式边界。五种结果、通用/结果专属最小记录、MVU guide、STOP/WAIT/future COMMIT、R0—R3 轻量 action-risk classification 与终止路由已确认；完整领域治理和剩余 实质性判断 扩展仍为 `PENDING REVIEW`。
...
每个 human-facing 结果必须同时回答：**为什么是这个选择、判断边界/最大不确定性是什么、接下来会发生什么、什么条件会让决定改变**。推荐不能退化为五个按钮或一句“请选择”：默认完整建议句为“我的首选是『中文结论』，因为……；当前判断边界是……；下一步是……；如果……，建议改判为……”。最强反方只有在会实质影响判断时才补一句。这是一种可审计的结构化表达，不要求暴露 hidden Chain-of-Thought。
...
Machine enum 仍可在 human view 中作为辅助标识，例如“现在不做，结束当前方向（`STOP`）”，便于研发、日志和 API 对照；但 code 不能替代结论。Human Renderer 必须从同一个 Decision Record 的确切版本生成这些表达，不单独保存第二份“人话决定”，也不能让翻译或模板改变正式 outcome。
...
最强反方只有在会实质影响判断时才增加一句；没有可信且会影响责任边界的反方时，不为模板完整强制制造。完整 Evidence、其他合法选项、完整 Cognitive Analysis、风险和反方、历史 Decision/Roadmap 与 Audit 作为按需展开层，默认一屏可以定位并打开，但不重复保存内容。

认知基座和 Analysis Method 在内部可以运行得更重；默认只外显 **一个确实改变决定** 的发现。其余可审计内容保存为结构化 rationale、引用与事件，不保存或重建 hidden Chain-of-Thought。默认视图、展开层和 Audit View 都从同一个 Decision Draft/Record 的确切版本及其引用渲染，不创建新的业务 Artifact、Graph Node 或第二真源。
...
#### 10.2.2 `RESEARCH / EXPERIMENT / COMMIT` 的统一判断指南
...
Owner 可以接受或选择其他 outcome；当选择在实质上不同时，复用 §10.3.2 的一次 bounded substantive challenge，而不无限争论或静默迎合。对证据充分度、研究或实验信息价值、市场窗口和实现成本的判断都只提供建议。`EXPERIMENT` 的确定性产品合同仍要求说明验证问题、可观察结果、exposure、停止/回滚和伤害边界；缺项时相应 PRD 尚不完整，但这不是 Reviewer 的 veto。安全、隐私、合规等 Reviewer 对高风险 action 只形成重点 concern 并交外置团队最终审核，不能把自己的角色建议扩张成整个 Run 的 blanket block。
...
`STOP` 和 `WAIT` 都必须进入 immutable Decision Record/Ledger。除 §10.3 的共同字段外，系统保留当时 Problem、Evidence、Knowledge snapshot 的精确引用，关键 assumptions、选择理由、Agent recommendation、最强且会实质影响判断的反方、Owner choice、scope、机会成本、被拒 alternatives、restart/review condition、监控 signal（如有）与 next action。它们是对当时认知和责任边界的完整记录，不是“没做所以不记录”。STOP 新事项通常不占 active Roadmap；WAIT 只投影为未承诺的 `exploring / candidate`；只有未来 COMMIT 才进入 `committed`。

外部环境、Knowledge、typed result 或新 Signal 到来时，仍通过统一 `signal.ingest` 与 `signal.relate`/`existing_links` 关联 historical Decision 的确切版本。只有新信息命中 restart/recheck condition、可信挑战 key assumption，或带来会实质影响判断的风险或机会时，才给 Owner 展示一屏重审提醒：“当时为什么停/等、什么条件会触发、现在出现了什么、Agent 是否建议改判”。普通支持性信息进入阶段摘要，不周期性无差别唤醒全部 STOP/WAIT，避免 alert fatigue。

重审永不改写旧 Record。Owner 维持原决定时追加 review result、理由与下一条件；改变时创建新的 immutable Decision Record/version，通过 `supersedes` 明确接替旧决定，并更新 Decision Ledger、Roadmap、Product Changelog 与精确 Impact。新结果仍可为五种 outcome 中任一种。Core 一期不建设常驻 watcher/service：无 Connector 时由自然语言/显式 Skill、`status/resume` 或人工 inspection 带入新信息；未来 scheduler/Connector 只能提交 Signal 或触发检查，不能自动推翻决定。
...
1. **AI Decision Brief**：绑定 Problem Definition Candidate、Evidence/Unknown、Knowledge/Product Memory、历史 Decision/Roadmap 的确切版本与当前约束，形成 Agent 首选建议、关键依据、判断边界和会改变建议的信息；最强反方只有在会实质影响判断时才外显，不能只罗列选项。
...
5. **确定性路由**：State Controller 校验 Decision Record 的确切版本、Owner identity/authorization、适用 policy/constraint 与目标边后，唯一写入正式状态和 route；Agent、Reviewer 或 Draft 不能自行路由。

这些是内部动作或能力，不是新的 Graph Node。节点可把 AI Brief、Reviewer 结果、讨论分歧和未完成选择保存为 versioned、run-local `Decision Draft/checkpoint`，在通用 Run State 声明的等待、暂停或可重试失败状态下跨会话恢复；checkpoint 必须绑定精确输入、版本与哈希，并记录取代关系，但不是正式 Decision Record、承诺或 route。只有 Owner 明确选择后冻结的最终 Decision Record，以及 Controller 基于该 Record 的确切版本写入的 route，才构成正式持久边界。

价值、可用性、可行性和商业合理性仍可作为内部风险/方向透镜，评估的是**待选方向**，不是审核 PRD 文档。是否调用某个透镜或专业 Reviewer 由当前风险和未知驱动，不固定为全量流水线。五种结果、MVU guide、STOP/WAIT/future COMMIT、Decision Record 通用最小合同和 outcome 最小补充已经确认；完整领域治理与剩余 实质性判断 扩展仍待后续确认。
...
Agent 先根据 Decision Draft 的确切版本预填，Owner 在一屏中确认或修改。Owner 对具体 outcome 和 scope 的清晰自然语言表达本身即可构成 choice；系统据此冻结 Record，不再追加第二次“是否确认”点击。Owner 默认只需关注五项：
...
当 Agent recommendation 与 Owner choice **在实质上不同**时，确认页才按 §10.3.2 额外显示 disagreement；没有实质分歧时，不为模板整齐占用默认视图。§10.2.1 的“一屏决策”是推荐和讨论层的信息预算；本节是 Owner 最终确认层，必须优先呈现 applicability scope 和明确选择的决定。二者都从同一 Decision Draft/Record 渲染，不是两份产品决定。

以下字段由系统从精确绑定的输入、Host identity、Controller 事件与下游引用自动填充，不要求 PM 重复录入：

- Decision ID、version、timestamp、actor identity。
- Problem Definition Candidate、Problem Evidence Map、Knowledge Snapshot 和 Review 的精确引用与哈希。
- Agent recommendation 与 Owner result；有实质分歧时绑定 §10.3.2 的 challenge 与完整条件字段。
...
- Roadmap、Experiment、Product Plan 与 PRD downstream 引用。
- 完整 cognitive rationale 与 Audit 引用；只保存结构化理由和引用，不保存 hidden Chain-of-Thought。

Owner 清楚表达 choice 后，系统生成 immutable、versioned Decision Record，不再要求一次重复确认。任何会改变 chosen decision、适用范围、关键理由、判断边界、下一动作或 outcome 最小补充的实质性变化，都必须创建新版本，并通过 `supersedes` 明确接替旧版，不能原位覆盖。默认 Human View、渐进展开和 Audit View 都从这一个 Record 及其精确引用渲染；视图不是第二真源。

#### 10.3.2 Agent 与 Owner 的实质性分歧（disagreement）

> **CONFIRMED DISAGREEMENT CONTRACT**：原则是**不迎合，也不阻挠**。Agent 保留专业意见，Owner 承担权限范围内的最终产品决定，Record 保留真实证据状态与责任边界。

Agent 必须先给出明确首选及理由。Owner 选择在实质上不同的 outcome 时，Agent 只进行**一次有界、实质性的挑战**：指出最关键的证据缺口或风险，解释为什么专业首选不同，以及当前选择会把什么不确定性或责任带到下一步。只问“你确定吗”不算挑战；同一证据下反复换措辞、争论或阻塞也不允许。挑战完成后，有权 Owner 可以坚持其选择；无权角色的坚持不能改变正式 Decision。

仅在存在实质性分歧时，Decision Record 和 Human View 才按条件保存并展示：
...
#### 10.3.4 按是否会改变决定及风险扩展（候选，待后续 Review）

以下是此前累积的扩展候选。它们不是 Owner 通用五项确认表，也不是所有微小决定的强制字段；哪些由 实质性判断、risk 或具体 outcome 触发，仍待后续 Review：
...
- 关联 Eval 和其他受影响对象的确切版本引用；Plan、PRD、Experiment 与 Roadmap 的通用 downstream 引用由系统自动维护。
...
Controller 只基于精确绑定的 records 和 versioned rules 重算：

1. 当前有权 Owner 的确认绑定 确切、immutable Decision Record。
...
6. Problem Definition、Evidence、Knowledge/Product Memory、Review 和历史 Decision 的引用指向当前有效的确切版本，可解析，且没有因实质性变化或矛盾而失效。
7. 实质性分歧已按 §10.3.2 完成一次实质挑战并记录；适用的 accepted uncertainty/risk、authorization 和 recheck/stop condition 完整。

全部满足后，Controller 才写入确定性 route；否则 Run 保持在当前 `product.decision`，不创建下游 Run 或承诺，并用直白语言列出每项 **unmet condition + deterministic repair target**。不能只返回裸 `NOT_READY`，也不使用分数、概率或“总体差不多”判完成。修复后复用同一节点重新提交 transition request，不增加 Reviewer，也不要求 Owner 对未变化的明确选择重复确认；Record 发生实质性变化时仍按版本规则重新确认。
...
### 10.4 `EXPERIMENT`：同一产品流程（Product Pipeline）的可逆实验模式
...
- 明确的 key unknown / hypothesis，以及本次 action 要回答的 Decision 问题。
...
结果可以被解释为 expand / iterate / stop / inconclusive，但这些只是回到 Decision 的结果处置结论和建议，不是新的正式 Decision enum：expand 仍需 Owner 选择 `COMMIT` 或新的受限 `EXPERIMENT`；iterate 需要新的 Decision/PRD version 且不能移动旧成功标准；stop 保留停止原因；inconclusive 不能包装成软成功。Better Product Graph 不执行实验、流量投放或测试 runner，也不能用下游 result 自动改写 Decision。
...
一期不建设 Experiment Portfolio，也不把它作为 Roadmap、Decision 或运行状态之外的正式业务真源。单个或少量实验使用既有 Decision、Plan/PRD、Run State、Audit、exposure/measurement/stop 条件与精确链接即可。
...
### 10.6 产品决策与路线图记忆（Product Decision / Roadmap Memory）
...
|---|---|---|---|
| **Decision Ledger** | 正式决定了什么、为什么、适用于哪里、谁作出决定、证据/Unknown 是什么、为何/何时可能改判 | 每个正式 Decision（包括 `STOP`）对应一份精确、不可变且版本化的 Decision Record；通用最小合同见 §10.3，另保存 Agent/Owner 分歧、授权、重审条件、`supersedes` 和下游精确引用 | Roadmap、Product Changelog、Audit Log |
...
| **Product Changelog** | 相对上一具名产品状态，哪些产品意义、承诺、规则或发布边界发生了实质性变化 | 实质性新增、改变、延期、暂停或停止，及其理由、source Decision/Roadmap/release 引用、批准者、影响对象和是否需要复审或 re-ready | 文档 CHANGELOG、全部 Decision 的副本或执行日志 |
...
|---|---|---|---|
| `STOP` | 必须记录，包括理由与 restart condition | 新事项通常不进 Roadmap；若本决定停止的是既有 Item，则把该 Item 的确切版本更新为 `stopped` | “没做所以没有 Decision”或新建虚假未来事项 |
...
这里的 Product Changelog 记录用户行为、产品规则、路线图承诺和交付或发布边界中**具有实质产品意义的变化**，不等于本仓库 `docs/architecture/CHANGELOG.md`，也不复制每条 Decision。它应尽量从 Decision 的确切版本、Roadmap transition 与 released artifact 自动生成 Proposal/entry，人工只补充“为什么这对产品有意义”，避免四处重复录入。普通 Classification 修正、reroute、澄清、证据追加或执行重试只进各自 history/Audit；只有它同时改变正式 Decision、Roadmap 承诺、产品规则或发布边界时才进入 Product Changelog。
...
**所有产品决策都必须记录，但记录深度随影响与风险增加。**共同最小合同统一使用 §10.3.1 与 §10.3.3，实质性分歧另按 §10.3.2：Owner 只确认五项与 chosen-outcome 补充；系统自动维护 ID、版本、状态、actor、authorization 与 epistemic 边界，以及 evidence、knowledge、review 的精确引用、recommendation/result、关联对象、supersession 和 rationale/audit 引用。“维持现状”等完整备选、机会成本和专业 Risk/Domain Review，只在决定的重要程度或风险命中扩展条件时增加，不强塞进每次 Owner 确认。微小、局部、易逆的决定可以使用紧凑 Decision Record 并内嵌在所属产物中；跨模块、外部承诺、高风险、改变既有规则或影响多个交付物的决定必须扩展完整备选分析、Risk/Domain Review、Roadmap/Product Changelog Proposal 和 Impact List。是否扩展由版本化的重要程度与风险政策判断，不由 Agent 为省事自行降级。

分级只决定扩展字段和 Review 深度，不决定“记不记录”：`local/reversible` 使用最小记录；`material/cross-artifact` 增加完整影响、路线图和产品变化说明；`critical/high-risk` 强化恢复方案、仅提供建议的专业审查和外置团队关注。所有已确认 Decision Record（包括 `STOP`）都立即进入 BPG 本地 append-only/versioned Ledger；重要程度较低的项目可以批量进入团队共享发布，但不能因不阻塞而被省略。一期不在此引入专业 Owner approval、Waiver 或 Reviewer 硬 Gate。
...
一期用版本化 YAML/Markdown 文件、不可覆盖 artifact 目录、`current` 指针和结构化索引实现四类语义投影。**Knowledge Maintenance Graph 未接入时，BPG 仍可在项目本地完整运行**：保存完整 Decision Ledger、Roadmap、Product Changelog 和 Audit records，并保留未来 KMG 消费 raw source 所需的精确引用；Connector 缺失本身不能让本地 Decision、Planning 或 PRD 生成瘫痪。未来接入后，Knowledge Maintenance Graph 负责团队共享、治理和发布，但不能重新替 Owner 作决定、改写其 outcome 或无痕生成第二份事实。四类视图始终通过精确引用和哈希关联同一来源；Audit Ledger 记录实际发生的提交、发布、消费和失效事实，但不替代产品语义。提交、Impact 和同步合同仍待 Knowledge Requirements 反推。

### 10.7 新证据对历史决定的影响（Decision / Evidence）

**本节规则已确认；它不新增 Router destination。**新 Signal 仍先执行现有 Intake/Router：可能持续重大伤害进入 `INCIDENT_ASSESS`，有可信行为基线且疑似偏离进入 `BUG_BASELINE_CHECK`，其余按 activation 进入 `INBOX_ONLY` 或 `DISCOVERY_START`。同时，`existing_links` 可以关联 historical Decision、Roadmap Item、PRD（含实验 intent）或 Incident 的确切版本；“命中历史对象”只建立关系和后续影响输入，不能吞掉本次路线判断。

Agent 将新 Evidence 对 historical Decision 确切版本的影响分为四类：

| 影响类别 | 含义 | 默认动作 |
|---|---|---|
| `SUPPORTS` | 在原 scope 内支持原决定，没有改变关键假设、风险或执行边界 | 以 append-only evidence link/阶段摘要关联原 Decision，不修改旧 Record、不打断 Owner |
| `NON_MATERIAL_DELTA` | 增加局部信息或缩小、补充某个边界，但不足以改变当前 outcome/action | 记录明确 scope 与局部影响，必要时更新摘要；不重开 Decision、不全量重跑 |
...
| `HITS_KILL_RECHECK_OR_MATERIAL_RISK` | 命中既定 kill/recheck condition，或使即将执行的动作出现足以改变判断的风险或高关注领域风险 | 精确标记 affected actions/artifacts；Agent 给出暂停、停止、缩小或重决策的首选建议，Owner 决定产品方向，并把重点 concern 交外置团队最终审核。疑似持续伤害仍优先 Incident |

一屏提醒只在新信息可能改变核心假设、scope、预设 trigger、重大风险或即将执行内容时主动出现；普通支持性证据和不会实质影响判断的聚合信息进入阶段摘要，避免 alert fatigue。提醒默认展示：原决定的确切版本、当时关键依据、新信息与 provenance、究竟改变了什么、Agent 首选建议，以及精确标记的 affected Plan、PRD（含 delivery intent）、Eval、Ready、Handoff。授权与 Evidence 继续分开：Owner 说“维持原决定”是有效决定，不会把冲突证据变成无效。

Owner 可以选择：维持原决定、补充 Evidence、设计受限 Experiment、创建新的 Decision 并明确取代旧版本，或缩小 action/scope。若在实质性挑战后仍维持原决定，必须记录理由、accepted uncertainty/risk 和下一 recheck/stop condition。旧 Decision 永久 immutable；改判只能创建新 version/Decision，并通过 `supersedes` 明确接替旧版，不能回写历史依据让旧决定“看起来一直正确”。

Impact List 绑定 new Evidence、historical Decision 的确切版本和 affected artifact 哈希，只标真正受影响的对象，最小状态语义为 `UNAFFECTED / REVIEW_REQUIRED / INVALIDATED / PAUSED / WAITING_DECISION`；不得因一个新 Signal 默认全量重跑，也不得把受影响的已派发 Handoff 隐藏在 Run 级总状态中。新 Evidence 若形成可复用项目事实，应保留为带 provenance 的未来 Knowledge source candidate；在 Knowledge Maintenance Graph 审核并发布前，不能静默覆盖正式 Snapshot。具体提交格式、证据冲突合并、跨团队 Impact delivery/ack 和批量提醒阈值保留到后续原型，不在本轮提前冻结。

对历史 STOP/WAIT 的重审另受 §10.2.3 约束：只有命中 restart/recheck condition、可信挑战 key assumption，或出现会实质影响判断的风险或机会时才主动提醒；维持原决定只追加 review result 与理由，改判则新建 immutable Decision Record，并通过 `supersedes` 接替旧版。Core 不周期性全量重审，也不让 future scheduler/Connector 自动改写 outcome。

---

## 11. 结果优先的产品规划、二维拆解与覆盖检查（Outcome-first Planning / Coverage）

**阅读卡——产品规划**

> **证据状态：** 双向深化原则已 `CONFIRMED`；当前 Planning 活动、节点组及持久 Machine Name 仍为 `PENDING NODE REVIEW`。

产品经理先看本阅读卡、§11.1 的结果定义和 §11.2 的二维拆解；实现者再读 §11.3—§11.6 的覆盖、协调、校验与 Plan Ready 精确合同。
...
- **人做什么**：Owner 已在 Product Decision 选择目标方向；Planning 中只有目标、边界、取舍、风险或阶段发生超出该 Decision 的实质性变化时，才返回同一个 Product Decision 重新判断。语义 Reviewer 只给模块、迭代和切片建议。
- **主要产物与完成**：实质性规划变化必须留下 versioned Product Plan checkpoint、changelog、取代关系和 impact；内部 Refinement 稳定后，Module、Iteration、Dependency 视角，PRD Candidate Slice Map/List、Coverage 处置结论与 Reconciliation change 共同进入 Plan Candidate 的确切版本，再由独立 formal Review–Optimize 审查。Review 已收尾且轻量 Plan Ready 的机械合同满足后，只为当前 activated+eligible slices 创建 1..N 个独立 PRD Runs。一页摘要是按需 Human View，不是 Owner Confirmation。三个 `plan.*` 是 `product.planning` 内部动作，不新增顶层节点。
...
同一 Product Plan snapshot 的确切版本可交给 bounded sub-agents，并行检查不同模块、流程和迭代；它们只返回 Findings、Proposals 和精确引用，不并发写正式 Plan、当前指针或 state。主 Agent join 后保留分歧并生成统一候选；涉及文件修改时复用 ADR-045 的独立 branch + worktree 和主 Agent diff review。多个 sub-agent 同意不自动提高 Evidence confidence。

发生实质性变化时，必须创建新的 Product Plan version/checkpoint，记录 change summary、reason、`supersedes`、受影响模块、迭代、依赖、候选 PRD 与 downstream Impact；旧版本不可覆盖。生命周期尽量复用 §20.5 的 immutable/version guard 和 §12.3 的重要 checkpoint/archived 原则，但结构化 Product Plan 仍是唯一规划真源，不另存一份可漂移的“深化报告”。
...
|---|---|---|---|
| **Planning Refinement** | 主 Agent 生成、探索、拆解和 global reconciliation；bounded sub-agents 对选定部分给 Findings/Proposals | 从 v0 或最近 checkpoint 的确切版本产生 versioned checkpoints，稳定后冻结 Product Plan Candidate | sub-agent 不写正式 Plan/state；允许推翻结构。结束只表示形成 stable Candidate，不是 Review PASS/Plan Ready |
| **Formal Planning Review–Optimize** | 独立、只读 Reviewer sub-agents 对冻结 Candidate 的确切版本按 `product-plan` profile 审查；Optimizer/主 Agent 按 repair path 修复 | Finding/Verdict 绑定 candidate 版本和哈希；每次修复生成新的 Candidate，并保留差异和取代关系 | Reviewer 不编辑 Plan、不写 state；适用的审查记录已完成或如实标记不可用，Finding 均有处置结论且 Review 已确定性收尾后，才交给既有 Plan Ready 流程 |

探索阶段可以按模块或流程调用仅提供建议的 partial review，帮助决定下一步 Refinement，但这些结果不能冒充 formal verdict。正式 Review 必须在 stable frozen Candidate 上运行；Candidate 发生实质性变化时，旧 verdict 立即失效。
...
连续循环若只改措辞、重复同一根因或没有实质进展，必须返回正确上游 checkpoint、Decision、Discovery 或 Owner，不能靠继续 Optimizer 消耗轮次。该机制复用一套通用 Review–Optimize engine：`product-plan / prd` 使用不同 Reviewer 组合、rubric 和 repair policy；`EXPERIMENT` 只是相同 PRD profile 上的条件化检查，不增加独立 profile。它们共享冻结输入、Finding/Verdict、版本差异、repair routing、定向复审、global invariant regression 和 no-progress 合同，不新增多套 runtime、业务 Node 或 Gate。
...
Selector 由 Agent 根据当前 Decision 的确切版本、Product Plan v0、模块与迭代数量和耦合、依赖面、风险与 Unknown、跨团队或跨会话需要及真实深化结果提出选择，再由现有 policy/Controller 记录合法 profile transition。它不要求 PM 填复杂度问卷，也不能在 Signal Intake 时永久决定；规划开始后如果发现依赖扩大、出现多个阶段或跨团队协调，可 `LIGHT → STANDARD → PROJECT_SCALE`，范围被验证为更小或复杂性被移除时也可降级。每次实质性升降级都要记录触发证据、对 checkpoint、Review 和后续工作的影响及继续点，不能为了省步骤静默降级。
...
三个 profile 的深度不同，质量底线不变。`LIGHT` 仍必须能定位 Target Operating Outcome 与当前增量、Observable Evidence、关键 Assumption/Unknown、范围与依赖、Acceptance/验证、风险/Guardrail、Owner/Decision 引用和审计链；它只是把步骤一次完成或内联、减少非必要 Reviewer 或交互，不得跳过会导致错误交付的核心内容或正式 Review/Ready 约束。
...
输出是 Product Plan 中 versioned 的 **PRD Candidate Slice Map/List**，每项至少包含：目标/问题、预期用户结果、所属规划阶段、涉及产品模块、依赖、验证方式、切分/合并理由及 source Plan/view 引用。它不是最终 PRD 或 PRD Run；后续 `plan.coverage.validate` 检查候选之间及其与整体 Plan 的遗漏、重复、断层和依赖冲突。
...
核心原则是：**覆盖完整不等于本轮全部实现，而是每件重要事情都有明确去向。**未进入当前 PRD 不得静默消失；每个 Planning Item 使用稳定 ID，并有一个当前 处置结论：
...
对应的人类语义是：本轮实现、后续迭代、先实验、等待明确条件、明确不在范围、已决定不做、尚未解决。`UNRESOLVED` 不是丢弃箱，必须附 Owner、影响、临时处理和复查/触发条件；处置结论 改变需通过 `plan.reconcile` 形成新 Plan version，而非覆盖旧记录。
...
5. **上游一致性**：切片是否仍符合 Product Decision 的确切版本、Roadmap 承诺及待验证 Assumption，而没有把实验偷换成正式投入。

检查深度随 Planning Profile 自适应：`LIGHT` 由主 Agent 在同一 bounded pass 内快速内联；`STANDARD` 形成简短的 item→处置结论和依赖关系并检查遗漏；`PROJECT_SCALE` 可针对 Plan Candidate 的确切版本派独立只读 sub-agent 做覆盖挑战。任何档位都不要求每次召集完整 Reviewer 组。

`plan.coverage.validate` 只诊断和记录 Finding，不静默修改 Product Plan、切片或 Roadmap，也不因发现缺口自动输出 BLOCK。关键 Finding 必须有明确影响、建议去向、责任与后续 `plan.reconcile` 处置；检查结果优先内嵌在 Product Plan、版本差异和 Audit 中，不新造庞大独立 Coverage 文档或第二真源。例如发现“高风险消息 PRD 缺少误判处理”时，本动作只说明遗漏、对用户的影响和命中的候选；是否加入当前 PRD、放到后续迭代或先实验，由协调动作决定。
...
所有 `dropped_as_unnecessary` 项必须列出理由、判定人和 Evidence/Decision 引用；Agent 不能通过缩小分母自证 100%。但数字不是 Gate 分数，`unresolved_items > 0` 也不自动 BLOCK；真正标准是去向、影响、责任和复查条件透明，且不会使当前候选建立在失效或自相矛盾的 Plan 上。
...
`plan.reconcile` 接受 `plan.coverage.validate` Findings、Planning Refinement 的局部发现和新 Evidence/Impact，重新对照 Target Operating Outcome、Guardrails、确切 Product Decision 与 Roadmap。它区分两类处置权限：
...
这些是已有 Plan Run 内的 repair route 或既有上游 route，不注册四个新审批。回到上游后生成确切的新版本，记录取代关系和 Impact，再重新执行受影响的切片、覆盖与协调；不得把旧 Review/finalize 结果搬到发生实质性变化的新版本。

允许灰度收敛：不要求所有 Unknown 和冲突在 Planning 阶段消失。暂不能解决但不使当前行动失真的冲突可以保留，前提是记录临时处理、影响、风险承担者、复查时间或条件，以及什么新 Evidence 会触发调整；会实质影响行动的冲突不得被隐藏为“以后再说”。按 Profile，`LIGHT` 可与覆盖检查在一次 pass 中完成，`STANDARD` 在每轮关键 Planning Refinement 后协调，`PROJECT_SCALE` 在 Wave 边界、重大切片变化或新 Evidence 出现时协调；复杂档可让只读 sub-agents 提供不同方案，但只有主 Agent 整合。新的实质性产品取舍必须返回 `product.decision`，再由 Controller 和版本守卫写正式记录。
...
考虑过但拒绝的方案：让 Coverage Validator 直接修复会让诊断者越权且无法解释为什么改；逐 Finding 局部打补丁会制造组合后矛盾；所有变化都要求 Owner 审批会让非语义同步过重；发现上游失效仍强行推进会把错误固化到 PRD。`plan.reconcile` 只更新既有 Product Plan version、PRD 候选关系、变更记录和必要 repair route，不创建独立大文档、第二状态系统或新的审批 Gate。具体自动/Owner 实质性判断 阈值和 `plan.iteration.map` 内部合同继续以真实运行验证，不在本轮扩张。

### 11.5 Deterministic Validators 与 Semantic Planning Review

确定性 Validators 检查：

- 当前 Product Plan Candidate 绑定 v0 或前序 checkpoint 的确切版本、实质性变更摘要、`supersedes` 和 impact 引用；不存在并发写入或从可变 `latest` 推断 source。
...
`plan.ready.gate` 只能按 §11.6 检查所需 Validator、formal Semantic Planning Review/finalize、Coverage/处置结论、依赖冲突，以及涉及实质性产品选择的 Decision 引用是否齐全并绑定当前版本；它不替代语义判断，也不要求 Owner 再确认整份 Plan。正式 Semantic Planning Review 按 §11.1.2 由独立只读 Reviewer sub-agent 针对冻结 Planning Candidate 的确切版本执行，并保留审查记录；探索期仅提供建议的 review、主 Agent 自审或绑定旧 Candidate 的结果，都不能冒充当前审查已完成。本轮不新增独立 Review Graph Node；已确认的 `plan.slice / plan.coverage.validate / plan.reconcile` 仍是 Planning 内部动作而非三个新 Gate。
...
Human Renderer 可从同一个 Product Plan Candidate 的确切版本按需生成一页规划摘要：
...
该摘要不是审批、acknowledgement 或 Ready 前提，Owner 无需逐字段打勾。固定 `plan.owner.confirm` 已取消，不保留未实现 alias/event。只要 Planning 忠实展开 Product Decision 的确切版本，Agent 可以自动完成模块与迭代拆分、普通依赖、切片、已确认规则的场景、异常和 AC 补全，以及不改变产品承诺本质的协调。

只有 Planning 发现新的实质性产品取舍时才打断 Owner：改变目标用户或目标结果；删除或推迟已承诺核心能力；改变重要优先级、时间或 Roadmap 承诺；接受明显新风险或体验损失；改变 `COMMIT / EXPERIMENT` 边界；或作出其他超出 Decision 确切授权范围的选择。此时不是“确认整份 Plan”，而是从 `plan.reconcile` 或 Planning Finding 返回 `product.decision`，展示“发现 → 受影响承诺 → Agent 首选 → 不改后果/最强反方/翻转条件”，由 Owner 形成 Decision amendment 或新 Decision version 后再回 Planning。普通忠实展开不得包装成新 Owner 选择。

程序化 Deterministic State Controller 随后执行 `plan.ready.gate`。它只重算以下确定性条件，不做语义产品判断，也不替代 Reviewer 或 Product Decision：

- 当前 Product Plan Candidate 的确切版本和哈希可解析，且该版本当前有效、在实质上仍成立。
- `plan.coverage.validate` 已对当前 slice set 完成；重要 Planning Items 均有显式 处置结论，未静默消失。
- Planning Profile 要求的、仅提供建议的 formal Review 已完成，`review.finalize` 已记录处置结论，并绑定当前 Plan 的确切版本；LIGHT 可以更轻，但不能拿旧 Review 或主 Agent 自审冒充完成。
- 无法形成有效子 Run 的机械依赖或引用冲突已由 `plan.reconcile` 处置；所有会改变产品方向、承诺、重要优先级或时间、风险承担或 delivery intent 的实质性选择，都有当前有效的 Decision 精确引用。普通、仅提供建议的 concern 和 Unknown 可透明携带。

Gate 输出 `READY`，或输出带具体 unmet condition 和 deterministic repair target 的 `NOT_READY`；它不评分，也不发明第二次确认。Planning Profile 只调整 Refinement、Review 和 Coverage 的执行深度，三档共用同一基础 Ready 合同。
...
`plan.ready.gate` 通过后，State Controller **只为当前已 activation 且 eligible 的 PRD Candidate Slices** 创建 PRD Runs，记录父 Plan 的精确引用、`planning_views_ref`、slice 版本和哈希、delivery intent、依赖，以及 carried Unknown/constraints。未来阶段继续留在 Roadmap/LATER 处置结论，等待条件项保持等待；`EXPERIMENT` slice 使用同一种 PRD Run 并继承实验条件，不创建 Experiment Run。不得为全部未来 Roadmap 预生成容易失效的 PRD。v0、单个局部深化结果、未完成 Coverage/Reconciliation 的候选，以及未获当前 activation 的 slice 都不能创建 PRD Run。

考虑过但拒绝的方案：要求“所有规划完全确定”会阻止可逆迭代；完全省略 Ready 会把全局矛盾复制到子 PRD；固定要求 Owner 阅读一页摘要并确认会重复 Product Decision 与外部审批，却不提供新的责任边界；为所有未来事项先写 PRD 会制造版本债务。待原型验证的重点是 LIGHT 是否仍能稳定发现机械冲突、实质性选择是否可靠返回同一 Product Decision、Unknown 边界与 eligible activation 计算是否可重复；本节不扩写后续 PRD Run 内部逻辑。

---

## 12. PRD Run 与审查—优化循环（Review–Optimize Loop）

**阅读卡——PRD 内容、模板与审查优化**

产品经理先看本阅读卡、§12.1 的主流程和 §12.6 的人类介入边界；实现者再读 §12.2—§12.5 的评测、Reviewer、Finding、优化轮次与 Ready 前置合同。这里的“审查”默认提供建议并留下处置记录，不等于组织审批。

- **做什么**：`prd.content.build / prd.render` 先组织一个确定增量的完整产品语义，再按项目模板写成同一份 PRD；随后先检查它是否忠实于上游的确切版本，再由 `review.parallel / review.aggregate / prd.optimize` 反复审查、修复和定向复审。
...
- **人做什么**：只在存在会实质影响判断的歧义或上游产品决定、需要不可替代的专业 Domain 授权，或 Connector policy 要求外部写权限时介入；不为每份 PRD 固定确认理解或重复批准内容。
- **主要产物与完成**：当前 PRD、必要 Eval Pack、Fidelity Findings 和 Review 结果绑定同一个确切候选版本，以及上游 Decision、Plan、Slice、Knowledge 引用；程序化 PRD Ready 通过后自动形成 self-contained BPG Released artifact set，直接作为本地交付单元。它不等于组织外置审批已通过、Connector 已发送、研发已接收或测试通过。
...
进入 `prd.generate` 前，State Controller 在后台执行“准备 PRD 工作空间”的确定性生命周期动作，内部 action/event ID 为 `prd.workspace.initialize`，不在 Graph Manifest 或人类 Overview 中与 `prd.generate` 并列为智能产品节点。该动作只允许：创建 Run/workspace，绑定父 Plan、Slice、Decision、Knowledge 的精确引用，登记状态与版本，准备 `archived/`、`released/` 及恢复位置，以及执行必要的保存、流转与机械校验。它**绝不生成、判断、补写或修改** PRD 的语义内容。第一版没有已发布运行消费者，不实现旧名 alias。

`prd.generate` 是独立、可恢复的 Agent 节点，内部按需保存执行记录和 checkpoint，但不把三个动作注册成三个用户可见 Graph 节点。`prd.content.build` 在不依赖具体排版顺序的情况下组织完整产品语义；确定性 `template.resolve` 解析 project Template Profile；`prd.render` 再由 Agent 按模板编写与组织 PRD。对简单需求，三个动作通常在**同一次 Agent 执行、同一用户动作**中连续完成，不增加一次确认或新的 HITL；内部能力可分开测试，不代表必须产生多个节点或多份正式文档。第一版直接使用这些正式 machine names，不实现 `spec.build` alias。

一期不默认持久化与 PRD 同等完整的独立 Product Spec。`prd.content.build` 的中间结构默认只是当前执行过程中的 runtime object；最终 frozen/released PRD 才是面向研发的人类交付合同。只有复杂跨会话写作、同一语义需输出多个模板、多个 Agent 分章节协作、模板发生重大迁移，或必须区分“产品语义错了”与“模板表达错了”时，项目才可按配置保存 versioned recoverable content checkpoint。该 checkpoint 必须绑定父级引用的确切版本和 PRD candidate，不能进入 `released/`、不能单独 Handoff，也不能被发布为与 PRD 竞争的第二份正式需求。
...
`template.resolve` 是 `prd.generate` 内的确定性步骤，不独立成为 LLM 节点。`prd.generate` 首次完成时把 versioned PRD Candidate 写入 `archived/`；之后沿用 Review–Optimize、Ready 与 release promotion，使通过的确切版本进入 `released/`。§12.4.4 的 `<PRD-ID>_<需求短名称>_v<版本>_<YYYY-MM-DD>` 目录和主文件同 stem 规则是已确认的人类定位合同；示例中的具体 ID、标题、日期和 v0.x/v1.x 递增方式不是唯一值。不可变、self-contained、精确引用与哈希，以及 Candidate→release 生命周期同样是强约束。

本轮只确认上述职责纠正、正式 Machine Name 与产物边界；`prd.render` 的完整内部写作逻辑、初始化的完整字段、失败状态和重试策略仍随 PRD Run 实现设计讨论，不因命名自动冻结为新 Schema、Gate 或 HITL。
...
忠实一致性检查绑定同一个冻结的 PRD Candidate、条件式 Eval Pack，以及 Product Decision、Product Plan、PRD Slice、Knowledge/Evidence、共享合同和确认约束的确切版本，重点寻找：
...
Agent 可以做不改变语义的清晰化，也可以把已确认的规则展开为必要场景、分支、异常和可判定 AC；这些属于忠实展开。任何实质性新增内容都必须从当前 Candidate 删除或显式标为 `PROPOSAL`，并按最早正确修复点返回 Product Plan/Slice、Product Decision 或 Problem Learning。**PRD 阶段的 Owner 不能用一次随手确认把未决规则变成正式决定。**

实现保持轻量：Validator 确定性检查精确引用和哈希、source validity、slice membership、Planning 处置结论、模板映射与遗漏的结构化约束；Product Reviewer 把 semantic fidelity/alignment 作为首要 rubric，优先由绑定同一份确切快照的独立只读 sub-agent 执行。它只输出既有 Finding、Verdict 和差异并进入 Review/Audit，不新增顶层 Node、独立报告、固定人工确认或第二真源。Reviewer 发现实质性偏离时必须给出对应的最早 repair path；Optimizer 只修允许在当前 PRD 修复的表达或遗漏，不能把越界内容合理化。

Review–Optimize 收敛后，Human View Renderer 可以根据同一个 Candidate、Review 和 Audit 的精确引用生成一页最终摘要，默认只展示：最终交付是什么、相对上游或上一版本发生了哪些实质性变化、关键 Review 修复、残余风险或 Unknown，以及下一去向。它不是审批记录、第二真源或“必须读完才能 Ready”的新 Gate；用户可在需要时查看，source 变化后按既有失效规则重渲染。
...
`evals.build` 是 PRD Run 内部按需、可恢复的子节点，不是第五条业务路线，也不是独立 Run Profile。选择子节点而非一次 Prompt 的理由是：它有独立的缺输入/等待、专业 Review、版本恢复和 失效 边界，并能与 PRD 模板写作并行；但它没有脱离当前 PRD 的独立产品目标，所以不升级为顶层 Graph Node/Route。

当 `prd.content.build` 形成稳定 content Candidate 的确切版本后，PRD Run 冻结该来源版本及哈希，然后允许 `template.resolve → prd.render` 与条件触发的 `evals.build` 并行读取同一版本。`evals.build` 内部依次编排 `evals.scope / evals.generate / evals.review`，但三者不显示为三个用户节点，也不增加三次 HITL。两支 join 时必须证明 PRD 与 Eval Pack 都绑定同一 source content version；产品语义变化按 Impact 使相关 case、rubric 和 traceability 失效，纯措辞、排版或不影响判断的模板变化不触发 Eval Pack 全量重做。
...
- 到具体 PRD rule 和 AC 的 traceability。
...
未来的 `evals-generator` 是我们后续要建设的 Better Product Graph **内部原子能力实现**，不是现有产品、顶层 Graph Node 或 Connector；一期可以由主 Agent 冻结 PRD/content snapshot 的确切版本、派发 bounded sub-agent 生成候选并聚合结果，再由现有 Review 和权限边界处置。下列只保留候选输入输出边界，不在本轮提前冻结完整生成逻辑：
...
如果未来由 Claude 或其他外部 Agent 执行生成，外部调用可以通过 Connector，但 Core 中的 `evals.generate` 内部能力合同和 Ready 语义不改变。V1.4 只冻结合同和挂载位置，不声称 `evals-generator` 已存在；在它完成前，项目可以用人工或通用 Agent 按同一合同产出候选 Eval Pack。`REQUIRED` 且 fulfillment 未到 `REVIEWED` 时可以继续改 PRD/Eval 内容，但 PRD Ready 必须失败并返回具体缺口。
...
- Product Decision 的确切版本，以及 chosen outcome、scope 和 Guardrails。
- Product Plan 的确切版本，以及 Target Operating Outcome、Current Iteration Outcome 与 activated scope。
- 当前 PRD Slice、用户结果、Module/Iteration/Dependency/Shared Contract 与 处置结论。
- Evidence 和 Knowledge 的精确引用、Assumption、Unknown 和适用 System Acceptance Baseline。
- 当前冻结的 PRD Candidate、条件式 Eval Pack、Template/Profile/Policy versions 与 content 哈希。

上述目标已经在 Problem、Decision 和 Plan 阶段由有权 Owner 决定，Reviewer 不再要求 Owner 逐项确认“这是不是目标”。任何 source 只提供 `current/latest`、哈希不一致或无法解析时，当前 Review 不能伪装成可信目标基准，应要求修复为精确引用；这仍不是新增 Owner HITL。
...
**独立首轮与 join。** 所有 reviewer/sub-agent 接收完全相同的 frozen Candidate、Goal Fidelity Review Packet、Evidence/Knowledge snapshot 和 output contract。首轮结果提交前不得互看其他角色的结论；Reviewer 只读，不能编辑 Candidate、state、当前 pointer 或 released Artifact。Host 不支持物理并行时可以隔离上下文顺序执行，但必须分别保存首轮结果，主 Agent 只能在所有 required 首轮结果返回或按 policy 明确失败后开始 aggregate。这样得到的是审查独立性，不是“多 Agent 同意就提升 Evidence confidence”。
...
`confidence` 必须由 `confidence_basis` 对 evidence directness、freshness 和 conflict 给出简短可审计解释；该字段只记录支撑等级的关键依据，不是重复 Finding 的长推理。Validator 必须拒绝没有可解析 `confidence_basis` 的非空 `confidence`，也不得依据模型自信、Reviewer 数量或多数一致提高置信度。最小人类输出必须能一眼看懂：具体 concern、关注等级、确切 依据、可能影响、专业建议/建议返工点和当前 处置结论。

**分歧与交叉验证。** Aggregate 可以在机器层使用以下既有 Skill 语义，但 Human View 必须显示直白中文：

| Internal status | 人类视图 | 处置 |
|---|---|---|
| `confirmed` | 多个角色独立发现同一问题或有更强证据支持 | 合并 Finding，保留每个角色和全部 evidence 引用 |
...
分歧不按多数投票。直接、可复现、当前有效、面向用户承诺和关键路径的 evidence 优先；仍无法裁决时保留冲突、说明可能影响和 Agent 首选建议，供外置团队审核。Aggregate 不能把所有合法建议都塞进 PRD，也不能以“多数未发现问题”抹去一项会实质影响 Goal 或 Scope 的 concern。
...
**行动结果与修复验证。** Human View 不显示百分比分、A—F 或加权总分，而是从 Finding 处置结论和 repair target 渲染直白建议：**建议按当前版本继续，并披露关注事项**；**建议修改当前 PRD**；**建议退回正确上游**；**证据或专业判断不足，保留给外置团队判断**。这些都只是建议，不是四个 Gate 枚举；Agent 可以采纳当前 PRD 范围内的清晰建议并触发 Optimize，也可以保留争议或未采纳项及理由后继续完成内部审查。Reviewer 不能因为自己的建议未被采纳而无限占住 Run。

Optimizer 每次生成新 Candidate 后，优先只复审实质性差异和未解决 Findings，同时回归全局目标、范围、Guardrail、Shared Contract、AC/Eval 等关键不变量。每个上一轮 Finding 记录：`FIXED`（已解决）、`IMPROVED_BUT_OPEN`（有改善但仍未关闭）、`PERSISTENT`（仍存在）、`REGRESSED`（修复引入或加重问题）、`INSUFFICIENT_EVIDENCE`（证据仍不足）；Human View 分别显示“已解决 / 有改善但未关闭 / 仍存在 / 出现回归 / 证据不足”。连续两轮只改措辞、没有实质进展或同一根因反复出现时，停止机械重写并显示卡点；Agent 可建议返回 Plan、Slice、Decision 或 Learning，也可把未解决 concern 与理由保留给外置团队。不得无限优化，也不得因 Reviewer 不满意而重启轮次预算。

**明确不吸收。** 每份 PRD 不运行完整七阶段 Product Goal-Based Audit；不重新要求人类确认上游承诺；不采用 95%/80% 覆盖率目标、百分比分、加权分或 A—F；不创建每 PRD `.product-audit/` 文件套件；不默认生成 Shell 审计脚本；不要求每条 Finding 双审、每轮人工确认或无限优化。完整 Skill 继续用于 §20.7 的架构、版本和里程碑级审计，其结果仍需形成处置结论，不能自动成为 PRD 修改指令。

### 12.4 Review–Optimize Loop

Review–Optimize 是一套可复用 engine/profile 机制，不是 PRD、Product Plan、Experiment 各自复制的一套 Runtime。不同 profile 可以使用不同 Reviewer 与 rubric，但一期全部 Reviewer 都只提供建议。流程仍必须冻结 Candidate 的确切版本；独立只读 Reviewer 只提交 Finding 和建议，只有 Optimizer 或主 Agent 才生成新版本；后续定向复审同时回归全局不变量，no-progress 时停止机械重写。Product Plan 的 Refinement 与正式 Review 边界见 §11.1.2；PRD 与 Experiment 进入本节流程时，也不能让生成它们的同一上下文自报审查完成。
...
**为什么保留独立 join point。** `review.parallel` 可能产生并行、逻辑并行、晚到、失败、`STALE` 或外部审计结果，高影响 Finding 还可能触发 §12.3.1 的 bounded review-of-review。若不保留一个可恢复的汇总边界，主 Agent 在中断后就难以证明已收到哪些结果、哪些无效、哪些尚未返回。因此保留 `review.aggregate` 作为轻量内部节点和聚合 join point，中文名为“审查意见汇总”。LIGHT 可在同一用户动作中自动完成，但仍保存可恢复的 aggregate 执行记录。这是机器内部恢复边界，不是 PM 交互步骤。
...
Aggregate 只提交合法汇总结果；不修改 PRD/Eval Pack，不生成新 Candidate，不写当前状态快照或当前指针，不执行 Gate 或授权，也不得宣布 `READY`。`review.finalize` 如何检查完整性并进入唯一 `prd.ready.gate`，见 §12.4.2；两者不得合并成“Agent 汇总并自行批准”。

**语义聚合与确定性校验分工。**

- 主 Agent 负责语义聚类、关联同一根因，判断 `confirmed / complemented / conflicted / unique / unsupported`，隔离缺少上游依据的未来线索，并建议最早的 `repair_target`。它不能为了显得完整而把不同问题强行合并，也不能把无上游承诺的建议放入当前修复清单。
- 确定性状态控制器只检查以下条件：必需 Reviewer 的审查尝试已齐全或已按政策明确失败；所有有效结果绑定同一个已冻结 Candidate 和 Goal Fidelity Review Packet；Finding 最小字段可解析；晚到或失效的结果已标识；每条原始 Reviewer Finding 在聚合结果中都有可追溯的处置。条件满足后，控制器才原子写入审查尝试、状态和审计记录。程序不判断哪个专业观点更正确。
- 必需的首轮结果未齐时，可以保存“等待/部分到达”恢复点，但主 Agent 不得完成语义汇总；绑定旧 Candidate 的晚到结果保留为 `STALE`，不混入新版本。

**恢复与输出。** Aggregate 结果是现有 Review Record 中带版本的汇总尝试或章节，引用现有 Finding、Verdict、Disposition、分歧账本与 Audit Event；`review_summary` 只是按需生成的人类视图，不是新业务 Artifact 或第二真源。最小可恢复记录需绑定：

- `aggregate_attempt_id/version/status` 与上一次汇总尝试的引用。
- 确切的 `candidate_ref + hash` 与 `goal_fidelity_packet_ref + hash`。
- 必需和可选 Reviewer 的审查尝试引用，以及完成、失败、晚到或失效的分类。
- 每条原始 Finding 到 Finding 聚类、分歧、无上游依据线索或其他处置结论的无损引用。
- 按 Finding 或聚类绑定的关注等级、建议 `repair_target`、当前处置结论，以及是否需要有界的二次审查。

审计账本按实际发生追加 `REVIEW_AGGREGATE_STARTED / REVIEW_AGGREGATE_PARTIAL_SAVED / REVIEW_RESULT_MARKED_STALE / REVIEW_AGGREGATE_COMPLETED / REVIEW_AGGREGATE_REJECTED` 或等价事件，并绑定汇总尝试、确切输入、Reviewer 审查尝试引用、结果哈希与规则版本；事件只证明汇总生命周期，不替代 `review.finalize` 结果或 Ready。

人类视图按需只显示：总体建议、重点关注、存在的分歧、建议返回的上游、不属当前范围的建议，以及下一修复位置。机器记录仍保留完整的确切引用；摘要不能通过省略消灭 Finding 或分歧。Aggregate 默认不打断 PM；真正需要上游产品判断时，由既有 Product Decision 或 Planning 节点处理。未解决的专业关注项进入同源内审意见供外置团队审核，不在 Aggregate 增加 HITL。

失败包括：静默丢失 Finding；把不同问题错误合并；把所有建议塞入 PRD；用 Reviewer 数量或多数票裁决分歧、提高证据置信度；把缺少上游依据的线索变成当前修复；在必需的首轮结果未齐且未明确记录不可用时完成汇总；读取不同或可变的 Candidate；修改 Candidate 或状态（向控制器提交合法结果除外）；声称批准或 Ready。高影响的 `unique / conflicted / weak-no-concern` 继续按 §12.3.1 触发有界二次审查，不在 Aggregate 内用多数投票快速“消除分歧”。

#### 12.4.2 `review.finalize / 审查收尾`

**形态与取舍。** 当前阶段退役 `review.gate`：所有 Reviewer 都只提供建议，继续保留 Gate 名会误导读者以为它们拥有批准或阻塞权；但把 Aggregate 直接折叠进 Ready，又会丢失审查尝试、Finding 处置结论和配套视图的完整性。因此使用 `review.finalize / 审查收尾`，由确定性状态控制器在合法 Aggregate 后立即执行。它是轻量内部迁移或动作，不是业务 Graph Node、Gate、Agent、Artifact 或 HITL，也不重做语义审查。

Finalize 只做四项确定性检查：

1. 本轮配置为适用或必需的 Review 审查尝试已完成；确实不可用的审查尝试已明确记录 `NOT_AVAILABLE`、原因和影响，而不是伪装成完成。
2. 每条 Finding 都有无损处置结论：`accepted + fixed`、`not accepted + reason`，或 `unresolved for external review`；Reviewer 的关注等级、数量或不满意本身不决定状态。
3. Aggregate、Finding、处置结论和修复差异都绑定当前确切 Candidate 及其哈希；旧版或失效结果不能混入。
4. 由同源 Review Record 渲染的“内审意见”配套视图已生成，并绑定同一 PRD ID、版本和哈希；主 PRD 的相对链接和关注项数量与之相符。

若主 Agent 决定采纳的当前 PRD 修改仍未完成，Finalize 把控制交回 `prd.optimize`；若主 Agent 接受某项上游修复建议，则返回现有最早修复点。若没有 Agent 决定继续采纳的修改、其余关注项均已有处置，或已达到既有轮次或无进展上限，Finalize 用人话记录“内部审查已完成”或“内部审查已完成，并保留 N 项关注供外置团队判断”，然后进入唯一 `prd.ready.gate`。它不要求所有 Reviewer PASS，也不因 Reviewer 不可用、存在未解决关注项或关注等级而阻塞；相同的确切输入和规则必须给出同一完成结果。

Finalize 不能修改 Candidate、裁决专业观点、批准内容、解释模糊建议、创建新修复，或把外置团队尚未审核写成“已通过”。输入失效或缺失、Finding 丢失、处置结论缺失或配套视图版本不一致时，只返回确切的流程未满足条件与修复位置；这属于审查记录完整性失败，不是 Reviewer 否决。

`prd.ready.gate` 随后只检查既有确定性产品合同、引用、version、template、REQUIRED Eval、Document Experience 和 changelog 等机械条件。Reviewer 指出的缺口只有被独立 Validator 证明违反既定合同，才因该合同缺失返回 NOT_READY；原因是 Validator 规则，不是 Reviewer 权力。正式 Reviewer Block/Review Gate/Domain Policy/Waiver 仅保留为未来无人值守研发/发布的 Roadmap seam，本期不定义实现。

#### 12.4.3 `prd.optimize / 根据审查意见修订 PRD`

**形态与启动边界。** `prd.optimize` 保留为可恢复的 Agent 节点，是既有通用 Review–Optimize 引擎的 PRD 配置，不是程序写 PRD，也不是可以重写一切的“自由优化”。只有主 Agent 对 Aggregate 中某项当前 PRD 建议作出“采纳并修复”的处置结论后，节点才能启动；根因在 Problem/Evidence、Product Decision 或 Plan/Slice 时，必须返回最早的正确上游，禁止在 PRD 里用更流畅的文案“圆回来”。未采纳或有争议的建议保留理由并进入内审意见，不要求继续改到 Reviewer 满意。

**确切输入与产出。** 每次优化尝试只读取确切 Candidate、已聚合且被主 Agent 处置为当前 PRD 修复项的 Findings、每条 Finding 的 Evidence、受影响范围与修复要求，以及确切 Decision、Plan、Slice、Guardrails、Template 和 Eval 引用。一轮批量修订完成时输出：

- 一个新的 versioned PRD Candidate，不覆盖旧版。
- 简短的实质性变化说明和 `finding_id → change/ref` 映射。
- 未修改项、理由及建议的正确上游/等待点。
- 因变化而需要失效或重审的 PRD 章节、Eval 和 Reviewer 范围。
...
一轮精确定义为“Aggregate 一批 Findings → 统一修订一个 Candidate → 定向复审一次”，不按 Reviewer 数量或 Finding 数量计数。LIGHT 最多 2 轮修订，STANDARD 默认最多 3 轮，PROJECT_SCALE 或高风险最多 4 轮；只有上一轮仍有实质性进展且余下问题确实能在当前 PRD 修复时，才允许第 4 轮。State Controller 维护 `loop_id / round / current_candidate_ref+hash / profile_budget / no_progress_count / legal_transition`，Agent 不得通过节点改名、重启审查尝试或生成新 loop ID 规避预算。一期不建 token、费用或墙钟预算系统。

**提前停止和恢复。** 连续两轮没有实质性进展，包括只换措辞、同一根因反复出现、重点关注项未减少、同类问题回归，或事实证明根因在上游，必须提前停止，不等硬上限。达到上限或无进展时不得宣称“全部解决”；应保存最新 Candidate 和恢复点，直白显示已用轮次、已解决与未解决项、停止理由、Agent 的上游建议，以及保留给外置团队的关注项，并复用现有 `NO_PROGRESS / RETURN_UPSTREAM / WAIT` 语义，不新增业务路由或无必要枚举。

**Candidate 版本与 Review Attempt 必须分开。** Agent 在一轮中的草稿或临时工作过程不版本化；只有批量修订完成且实际重新送审，或形成被引用的实质性检查点时，才在 `archived/` 产生一个新 Candidate。每次 Review 或复审使用独立轮次和审查尝试记录，并引用 Candidate，不复制 PRD 文件。同一 Candidate 只新增 Reviewer 结果、复核、处置结论或证据且内容未变时，只增加 Review Attempt，不增加 PRD 版本。

`archived/` 只保留首次完整送审的 Candidate、每轮批量修订后实际送审的 Candidate、经过人工实质性编辑的 Candidate，以及发布前实际引用的实质性检查点；不保存每个 Reviewer 稿、每条 Finding 中间稿、自动保存稿、排版缓存或无内容变化的复审稿，也不新增 `working/` 目录。正常 Run 中出现 2—4 份过程稿是实践预期，不是 Gate 数量限制。
...
现有 §20.5 已规定正式产物不可变、变更必须创建新版本并记录 `supersedes`，但还缺少 PRD Review–Optimize 的具体保存点、附件定位和发布生命周期。ADR-039 的早期物理方案让每个 PRD 包根目录共享一份 `assets/`，并让 `archived/`、`released/` 只保存 Markdown；这会使人无法从某个确切 PRD 直接定位附件，也会让兄弟版本共享可变素材。ADR-075 只取代它的物理目录和打包方式，继续保留不可变、Document Changelog、取代关系与确切 Handoff 语义。V1.4 的当前合同是：**每个 Candidate 和 release 都是自包含目录。**
...
目录与主文件的稳定命名是 `<PRD-ID>_<需求短名称>_v<版本>_<YYYY-MM-DD>`；派生导出文件使用同一文件名前缀，不使用 `prd.md / final.md / 最新版.md`。同源内审意见使用 `<PRD-ID>_<需求短名称>_v<版本>_内审意见.md`，并通过视图头部绑定确切的主 PRD 引用、哈希和日期；它随 PRD 版本更新，不单独快速递增版本。PRD ID 在版本和标题微调间保持稳定；只有范围已经成为新的独立产品增量时才分配新 ID。

过程目录采用 `archived/`；正式目录采用 `released/`。不使用 `final/`，因为 release 后仍可能产生 v1.1；不使用 `published/`，因为 PRD 可正式交接但未对外公开；不使用 `approved/`，因为批准本身不代表 Ready、文档验证和 Handoff 条件全部成立。`released` 的唯一语义是：该版本已绑定所需忠实一致性检查、专业 Review、Ready 与 Document Experience validation，成为可被正式 Handoff 精确引用的不可变 BPG PRD 产物集。它不表示组织外置汇总审批已经通过，也不表示 Connector 已发送。

`archived/` 保存实质性检查点：首次完整送审的 Candidate、每轮 Optimizer 批量修订后实际重新送审的 Candidate、经过人工实质性编辑的版本，以及因模板、政策或配置迁移而实际被 release 或 Review 引用的 Candidate。候选版本引用附件时，使用与 release 相同的单候选自包含目录；正式审查完成后，同目录生成与该确切 Candidate 绑定的内审意见视图。无附件或导出文件时不创建空目录。它不保存每个 Reviewer 稿、每条 Finding 中间稿、token、按键记录、自动保存稿、排版缓存或无内容变化的复审稿。

`released/` 保存**所有**正式发布版本，通常从 v1.0、v1.1 递增，不是“只放最新版”。每个 release 目录必须能独立复制、发送、归档和定位，不依赖兄弟版本共享可变素材；主 Markdown 只用 `./assets/...` 相对路径，已经被 release 引用的 Markdown、内审意见和 assets 都不得覆盖。每个 release 只渲染一份当前内审摘要：默认只列仍未解决、存在分歧或需外置团队判断的关注项，包含关注等级、具体事项、确切依据或引用、可能影响、Agent 建议和处置结论或未处理原因；完整 Findings、已修复项和审查尝试留在 Review、Audit 与 Changelog 中。旧 release 后续被 `SUPERSEDED / INVALIDATED / REVOKED` 时仍留在 `released/`，不移动到 `archived/`、不删除、不改写。

主 PRD 正文不复制详细关注项，只保留相对链接和短状态，例如“内部审查已完成，尚有 N 项重点关注；详见同目录内审意见；外置团队审核尚未完成”。没有遗留项时写“截至当前版本，内部审查未保留需要外置团队特别判断的未解决事项”，不能写“无风险/已批准”。单文件导出配置可以把同源内审意见附在 PRD 末尾；正式 Markdown 职责仍分离，合并导出不是第二份手写真源。

`artifacts/prds/DOCUMENT_CHANGELOG.md` 是只追加的 PRD 文档生命周期账本，以 `prd_id` 区分各 PRD 的事件；它不被复制进 release 或 Handoff。它**不同于**记录产品语义变化的 Product Changelog，也不同于仓库架构版本的 `docs/architecture/CHANGELOG.md`。每个实质性版本至少记录：

- `prd_id / document_id / version / lifecycle_status`。
- `created_at / created_by`。
- 确切的 `source_prd_candidate_ref + hash`、父级 Decision、Plan、Slice 和 Knowledge 引用、可选的 `source_content_checkpoint_ref + hash`，以及 Template、Policy 与 Profile 的确切版本。
- `supersedes` 或 `source_draft_ref + hash`。
- `change_type / change_summary / reason`。
- Review、忠实一致性 Finding 和处置结论、Ready 与 Document Experience validation 引用；若历史或条件式流程实际发生 Owner、Domain 或 Connector 授权，则记录对应的确切事件或引用，不把它写成当前默认前置。
- 主 Markdown 的 `content_hash`，以及该 Candidate 或 release 实际引用的 assets 与可选派生导出文件的确切引用和哈希；派生导出文件还要回指源 Markdown 与 assets 哈希。

状态变化用新事件或新记录追加，不能修改历史行。既有可变 manifest 或当前导航如果实现，必须能由不可变目录、Changelog 和 Audit Ledger 重建；它们不能创造产品事实或成为第二真源，也不会被复制成一份独立 Handoff manifest。

Review–Optimize 每一轮按以下方式落档：

1. 将本轮送审内容及其引用的 assets 冻结为 `archived/` 中确切、自包含的审查 Candidate 目录，并让所有 Reviewer 和 Findings 引用其版本和哈希。
2. `review.aggregate` 保存本轮 Review Attempt 引用；`prd.optimize` 基于确切 Candidate 统一修订该批 Findings。只有修订后的内容实际送入下轮 Review，或成为被引用的实质性检查点时，才生成一个新的 archived 版本，记录 Finding 到变更的映射、未修改项及理由、变化内容、`source_draft_ref`，以及失效与复审范围；不得改写旧候选。
3. 人类对范围、规则、AC 或其他实质性内容的编辑，也必须先返回正确上游形成合法新语义，再生成新的 archived Candidate，并按影响使旧 Verdict 或 Ready 失效；不能在 PRD 阶段用临时批准替代上游 Decision、Plan 或 Slice 变更。
4. 只有确切 Candidate 的忠实一致性结果、所需 Review 处置结论、PRD Ready 和 Document Experience validation 全部有效时，才从它生成不可变、自包含的 released 目录；release 必须绑定 `source_draft_ref + hash`。
5. Handoff 只消费确切的 released 产物集；“候选已生成”“已批准”“Ready 计算中”或“导出文件已生成”都不能被写成 released 或已交接。

文档呈现变化与产品语义变化必须分开：模板、排版、术语或可读性修订即使不改变产品语义，也创建新的 released 版本并保留旧版；若范围、产品规则、流程、AC、Eval 语义或承诺发生变化，必须创建新的 archived PRD Candidate；若变化还改变上游 Decision、Plan、Slice 或共享合同，应先生成相应的新版本、变更记录与影响分析，再重新 Review、Ready 和 release，不能只改 released Markdown。**本地或 Git 中的正式真源，是确切的 released Markdown 及该 Markdown 引用的 assets。** 同名 DOCX、PDF 或 ZIP 仅由 Renderer 或 Export Adapter 按需派生，不要求每个 release 全部生成；它们不改变产品语义，也不成为 Ready 默认条件或第二真源。

本节复用现有 `prd.render / review.aggregate / prd.optimize / artifact.version.guard / prd.ready.gate`。第一版直接把确切的 release 作为本地交付单元，不注册 `handoff.package.build`、物理复制、独立打包器、manifest、兼容事件、Graph Node、Gate、HITL 或第二真源。

### 12.5 Reviewer 仅提供建议及其与外置审核的边界

**一期合同已确认。** 所有 BPG Reviewer 都只提供建议：它们可以是 Product、UX、Engineering Feasibility、Testability、AI Behavior、Security、Privacy、Compliance、Legal、Finance 或其他专业逻辑角色，也可以由内部子智能代理执行；但任何 Reviewer 都没有正式 `BLOCK`、否决、批准或豁免权，也不能因为使用了“专业 Reviewer”名称就冒充真实组织责任人。当前每份需求仍由外置团队作最终审核，BPG 的职责是提高候选质量、保存分歧，并把关注事项清楚地交过去。

Finding 的最小人类合同只有：直白的关注事项、关注等级、确切依据或引用、可能影响、专业建议或建议返工点，以及处置结论；结构化细节见 §12.3.1。关注等级只用于排序，不赋权。旧 `PASS / PASS_WITH_CONDITIONS / NEEDS_OWNER / BLOCK_RECOMMENDED`、`severity`、`blocked_actions/still_allowed` 或 Waiver 字段若出现在历史设计或记录中，只保留为历史解释，不进入一期当前状态路由、Ready 或 release 条件。

主 Agent 对每项建议作出透明 处置结论：

- **采纳并修复**：建议清晰、有上游依据且最早修复点在当前 PRD 时，进入有界 `prd.optimize`；Reviewer 后续复审修复状态。
- **建议返回上游**：确实动摇 Problem、Decision、Plan 或 Slice 时，Agent 提出最早返工点；是否形成实质性产品变化仍由对应 Owner 或节点决定，不由 Reviewer 直接改状态。
- **不采纳并记录理由**：建议不适用、证据不足、超出当前范围或会引入无依据功能时，保留原关注事项与 Agent 理由。
- **保留给外置团队判断**：存在合理分歧、专业事实不足或达到无进展或轮次上限时，停止机械改写，在同源“内审意见”视图中披露。

Reviewer 冲突不按多数投票，多个 Agent 同意也不增加证据置信度。有界的二次审查只帮助澄清高影响关注项，不产生批准。`review.finalize` 只检查审查尝试、处置结论、确切版本和配套视图是否完整；`prd.ready.gate` 只执行独立、确定性的产品合同检查。未解决的建议性关注项必须披露，但本身不是 `NOT_READY` 原因，也不能让 Reviewer 无限占住 Run。
...
PRD 阶段只在两类情况打断人：存在会实质影响结果的歧义，或上游产品决定必须由 Product Owner 判断；带版本的 Connector 副作用政策要求对外部写入取得一次权限。前者返回相应上游形成新决定，后者只授权确切的 Connector、目标、产物版本与哈希，以及具体动作，不重新审核 PRD 语义。安全、隐私、合规、资金等 AI Reviewer 的意见进入内审意见并交外置团队，不在 BPG 内另建 Domain Owner 批准步骤。
...
2. 以确切版本和内容哈希绑定 `decision_refs`、`roadmap_snapshot_ref`、`product_plan_ref` 和 `knowledge_snapshot_ref`，并关联 Module Map、Iteration Map、PRD Matrix 和 Slice；不接受仅指向“当前/latest”的可变引用。
...
13. 适用 Reviewer 的审查尝试已完成或已明确记录不可用；所有 Finding 都有处置结论，未解决或存在分歧的关注项已在同源内审意见中披露。关注等级与 Reviewer 建议不构成 BPG 批准或阻塞，外置团队审核仍独立。
14. Evals Applicability 与 Fulfillment 分开，且都有直白的人类视图；`NOT_NEEDED` 理由成立，`RECOMMENDED` 的建议与放弃影响透明，`REQUIRED` 时确切 Eval Pack 已完成所需 Review，不存在被旧 `DEFERRED` 隐藏的 `BLOCKED_MISSING_INPUT`。
15. 可以通过确切、自包含的 Released 产物集独立交付；只有客观上不能独立发布时，才作为 Batch 中边界独立的成员交付，并披露共同回滚或停止条件与耦合风险。
16. 忠实一致性检查证明当前 PRD 没有超出确切的 Decision、Plan 或 Slice，没有偷带未来范围、编造事实或遗漏关键约束；任何会实质改变产品语义的 Proposal 都已返回正确上游，而不是靠 PRD 阶段临时批准变成正式规则。

### 13.1 模板可配置

Core 由 `prd.content.build` 先组织完整产品语义，再由 `prd.render` 按项目模板组织表达；两项能力默认连续完成并产生一份 PRD candidate，而不是两份并列正式文档。

V1.4 没有把“领域无关的 general 模板必须先通过人工 Review 才能成为默认”冻结为当前硬前置。当前启用模板和回退模板由带版本的 Template Profile 配置解析；在配置尚未明确把其他候选提升为默认模板时，一期可继续使用以下确切的上游回退版本：
...
`template.resolve` 使用带版本的 **Template Profile / PRD 模板配置**，不使用易与 Host Adapter、Connector 混淆的“Template Adapter”。选择优先级固定为：

1. 项目显式配置的当前 Template Profile。
2. 从受信项目知识中可确定识别的当前模板及版本。
3. Better Product Graph 通用默认模板。

只要能确定确切的有效模板，Agent 就不能询问 PM。只有多个候选互相冲突，而且通过项目配置、知识版本和有效期仍无法判断哪个当前有效时，才允许一次聚焦询问；该询问属于 `prd.generate` 的恢复上下文，不增加独立 HITL 节点或模板 Router。

项目可以换模板，但不能降低 Ready 标准。每个 Template Profile 必须声明必要产品语义到模板章节或字段的显式映射；模板没有对应栏目时，按 Profile 把内容放入约定扩展或附录，默认直接渲染进同一主 Markdown。第一版不实现未发布 `ready-supplement.md` 的兼容输入；只有确实无法表达或绑定必要语义时，才由 Validator 判定模板不兼容，不能因为栏目名不同就拒绝，也不能静默丢失。
...
一期只为当前过渡使用的 Better-Product-Plan 回退模板补足以下通用合同，不另造一份完整 Spec：父级 Product Plan、Slice 和 Decision 的确切引用，PRD Increment / Increment Contribution，`CONFIRMED / ASSUMPTION / UNKNOWN / VERIFY` 边界，Shared Contract 与跨 PRD 依赖，Evals applicability 与 Eval Pack 引用，以及文档版本、Review 和 release 状态。模板字段可以使用不同名称或编排顺序，但仍须映射 Target Operating Outcome、Observable Evidence、Non-sacrificable Guardrails、Current Iteration Outcome、Module 和 Iteration。Futu、moomoo、券商、投教等项目专属检查项继续属于项目配置或 Profile，不升级为通用产品语义底座。
...
该 general 模板目前只是 Draft/Bootstrap 候选，不是质量完备或已经激活的通用默认模板。V1.4 只冻结以下边界：Template Profile 可配置；使用确切版本；支持回退、项目覆盖、固定版本与回滚；不得静默丢失语义。模板内容优化、版本升级提示、迁移、何时提升为默认并取代上游回退模板，以及未来是否增加前端、后端或服务类 Profile，都留在 Roadmap 单独推进。提升为默认模板时可以使用实现期兼容性与真实消费者证据，但当前不设置固定人工 Review Gate。模板升级通过配置发生，不新增 Graph Node、模板 Router 或每份 PRD 的固定人工确认。

### 13.2 文档体验政策（Document Experience Policy）：从表达原则到横向执行机制

大白话：正式结构化产物负责“事实可验证、流程可恢复”，人类视图负责“读者能迅速看懂结论、未知、责任和下一步”。两者不能各写一套事实；人类视图必须从确切正式产物与哈希重建，来源变化后旧视图失效并重渲染。下面才是实现者使用的 Policy、Profile、Renderer 与 Validator 精确合同。
...
| **artifact-specific Profile** | 按产物用途、受众和行动风险决定展示重点、密度，以及是否需要语义 Review | 新 Run Profile、Graph 分支或产品 Artifact |
| **Human View Renderer** | 从正式 Artifact 与确切配置生成可重建的人类视图，并嵌入 Human View Metadata | 新真源或允许手工漂移的副本 |
| **Document Experience Validator / 按需 Readability Reviewer** | 前者确定性检查最低理解项、绑定和新鲜度；后者在 Profile 或风险需要时提出仅供参考的 Findings | Reviewer 自行批准或阻塞，或一套新的 Ready 状态机 |

默认 Policy 由 Core 提供。项目可以配置 `language`、章节顺序、展示密度、模板和加强要求，但不得删除最低理解项。**模板决定怎么排，Policy 决定至少让人理解什么**；Profile 决定某类产物需要多重的表达。每次渲染必须记录 Policy、Profile 和 Template 的确切版本，不能只引用 `current/latest`。

> **产品经理可在这里停止。** 读懂上面的大白话和四个概念，就足以参与文档体验的产品讨论；§13.2.3 补充不同产物的表达轻重，§13.2.4—§13.2.5 是实现者使用的精确合同，首次阅读可以跳过。

共同最低理解项是：当前结论、适用范围、证据边界、关键依据与未知项、Owner 和权限、当前状态、正式来源的确切版本与哈希，以及下一项允许动作。某项确实不适用时使用 `NOT_APPLICABLE` 并说明理由；源材料确实不可得时按业务合同使用 `NOT_AVAILABLE`，不能静默省略或由 Renderer 编造。

#### 13.2.3 映射现有环节，而不是增加持久节点

| 已有生产/查看环节 | Human View Profile | 重点 |
|---|---|---|
| Product Decision | `decision` | 默认一屏：一个中文首选、最多三个关键依据、最多一个会实质影响决定的认知提醒、一个定性判断边界、一个具体下一步；有实质影响的最强反方可加一句，完整证据、选项、分析、风险、历史和 Audit 渐进展开 |
| Product Planning | `product_plan` | 运行结果、模块/迭代、依赖、覆盖、Owner 和承诺边界 |
| `prd.render` | `prd` | 产品增量、范围与非目标、规则与流程、AC 或 Eval、风险、确切的上游绑定与 Handoff 边界；Review 收敛后可从同一来源按需渲染一页最终摘要 |
| `evidence.map` 的人类视图 | `evidence` | 来源与形成过程、主张、冲突、未知项、MVU 和版本变化 |
| `review.aggregate / review.finalize` | `review_summary / internal_review` | 完整机器记录的按需摘要，以及与确切 PRD 同目录的未解决事项、分歧或需外置判断事项的配套视图；不复制成手写第二真源 |
...
| `audit.view` | `audit` | 确切的事件与产物链、结构化理由、人工改判、变化与权限过滤 |
| `handoff.dispatch` 准备的视图 | `handoff` | 交付对象、确切来源、边界、约束、所需动作与真实回执状态 |

Run State、Schema、Audit Event 和内部索引等纯机器记录保持原始 JSON/YAML，不为可读性扩写真源；用户有权限且确有解释需要时，才通过 `audit` 或对应的按需人类视图呈现。`audit.view` 仍读取原 Audit Ledger，不复制第二份账本。
...
这里没有新增持久节点：例如，`prd.render` 在原执行尝试内完成 PRD 人类视图；Decision、Planning、Incident、Bug 和 Handoff 由各自已有的构建或准备动作调用同一组件。`document.experience.validate` 是共享 Validator 名称，不是 Graph Node ID。Orchestrator 必须向调用传入 `artifact_type / profile_id / profile_version / policy_version / template_version / audience / language / source_artifact_ref+hash`，Agent 不能自行省略或降级 Profile。State Controller 在相关 Ready 或 Handoff 迁移时重新解析绑定并运行 Validator；Agent、Renderer 或 Reviewer 都不能自报“可读性已完成”。

`decision` Profile 还必须确定性检查面向人的输出不是只有代码，并执行 §10.2.1 的默认信息预算：一项结论、最多三项关键依据、最多一项会实质影响决定的认知提醒、一项定性边界和一个具体下一步；最强反方只有确实影响决定时才按需增加一句，`COMMIT` activation 使用直白句子。Owner 确认视图另按 §10.3.1 和 §10.3.3 检查五项内容、系统字段自动绑定与仅针对已选结果的 `outcome_details`；存在实质性分歧时，再按 §10.3.2 检查双方结果与理由、授权依据、接受的不确定性与风险、复查或停止条件、执行约束，以及一次有界的实质性质疑记录；无分歧时不得生成占位块。当 §10.1 的行动风险分类会改变下一步时，视图必须用直白语言说明等级、原因和后续影响；`RISK_PENDING` 不得被渲染成低风险。缺少最低项、按需展开入口或来源绑定时，`document.experience.validate` 失败。数量是默认层级预算而非死字数：重大安全、合规、资金、隐私或不可逆风险必须在默认视图充分披露，即使超过一屏。Renderer 只能从同一个确切的 Decision Draft 或 Decision Record 及其引用和枚举、激活、风险字段渲染，不能创建或手改第二份产品决定真源。

`prd` Profile 的一页最终摘要不是 Owner 批准视图。Renderer 只能从确切的 PRD Candidate、上游引用、Review Findings 与变化记录，以及 Audit 中生成最终交付内容、实质性变化、关键修复、残余风险或未知项和下一去向；该摘要按需查看，不是 `prd.ready.gate` 的阅读确认项。`internal_review` 配套视图从同一 Review Record、Aggregate 和 Findings 确定性渲染，只保留未解决事项、分歧或需外置团队判断的关注项，并绑定确切的 PRD ID、版本和哈希；主 PRD 只生成相对链接与短状态，单文件导出可附上同源内容。Validator 检查二者的确切版本、数量和链接一致，且不得把 BPG `READY/RELEASED`、`READY_TO_DISPATCH` 或 Connector `SENT` 写成组织外置审批通过，也不能用漂亮摘要隐藏 REQUIRED Eval 缺口。

Validator 确定性检查：最低理解项是否存在；来源、Policy、Profile 和 Template 是否绑定确切版本与哈希，且仍为当前允许版本；人类视图是否因来源或依赖变化而失效；结论、范围、证据边界与未知项、Owner 与权限、状态、下一步是否可定位；Handoff 是否区分“已生成、待发送、已发送、已接收”。缺少会导致读者执行错误动作的最低理解项时，结果可以让**原有**对应 Ready 或 Handoff Gate 不通过；这不是第五种概念或新增 Gate。

Readability Reviewer 只检查确定性规则不能判断的语义问题，例如术语堆叠、结论埋藏、文本流畅但隐藏未知、重复导致边界模糊、把授权写成事实、面向错误受众。它只产生仅供参考的 Finding，不能批准、否决或阻止 Ready；当前阶段真正可执行的约束只来自已确认的确定性 Document Experience 和 Artifact 合同，并由 State Controller 重算，专业责任留给外置团队。
...
- 机器记录只提供按权限生成的按需视图，不改写或扩写原始 JSON/YAML。

每个人类视图头部都嵌入：`source_artifact_ref/hash`、`policy_version`、`profile_id`、`profile_version`、`template_version`、`rendered_at`。来源、Policy、Profile、Template 或其声明依赖的版本变化后，旧视图立即标记 `STALE/INVALID`，并从新绑定生成新的渲染版本；旧视图保留用于审计，不得被原位手工修改来掩盖漂移。人类注释若需要保留，必须作为回指来源或视图的独立反馈或 Finding，不改变视图和正式 Artifact。
...
第一期只保证有真实消费者的最小 Profile 集：`product_plan / prd / internal_review / decision / incident / bug_fix / handoff / audit`。其中 `product_plan` 只先支持 §11.6 的按需一页规划摘要及确切来源绑定，不借此冻结全部规划展示模板；`internal_review` 是 Review Record 的配套人类视图，不是业务 Artifact。`evidence / review_summary` 等映射继续保留稳定语义，随着对应真实生产者和消费者成熟再冻结合同。不能为了架构完整一次性冻结所有 Profile，也不因此增加独立 Runtime、Service、MCP 或 CLI。

---

## 14. 核心合同与结构合同（Core Contracts / Schema）
...
本节给合同、Schema 和 Validator 实现者看。主要结论是：一期先用三个最小逻辑合同跑通状态与审计，其余合同随真实业务切片成熟；下面列出的是边界清单，不表示都已实现或已经冻结完整字段。
...
三个逻辑合同从 `v0alpha` 开始，通过真实端到端实现切片后再冻结兼容规则；这不等于每个 Run 保存三套机器账本。按 ADR-080，一期每个 Run 的持久机器记录只有当前状态快照和有意义的事件流两类；Node Result 是原子能力提交给 Controller 的执行合同，被消费后只引用正式 Artifact、更新快照或形成必要的 Audit Event，不另建全量永久流水。文件名与完整字段仍待实现，不把示例 `state.yaml/state.json/events.jsonl` 冻结成 Schema。
...
- Raw Signal Envelope / Prepared Signal / Signal Relationship Set / Classification Record / Route Decision Record / `existing_links` / Signal Ledger / 当前路由指针。Raw Envelope 由 Host 或 Connector 根据自然原文或原生结果自动构建，并附来源与形成过程；不是要求外部人员填写的 YAML 表单。Route Contract 只接受四个互斥去向，关联维度或下游结果类型不得伪装成第五条路线。
- Evidence Reference / Problem Evidence Map（`problem.evidence.map.v1`）/ Learning State / Learning Round Delta / Evidence Request / Learning 运行状态 / 完成处置结论 / 下一步建议。Evidence Reference 保存不可变的来源与形成过程；Map 保存只追加、仅限当前 Run 的关系判断，两者不得互相替代。Learning Round Delta 是 Learning State 内部检查点章节；Evidence Request 是绑定 Run、Round、MVU、来源、有效证据、决策影响、等待与恢复条件的版本化请求和等待合同，二者都不是新的顶层 Graph Node 或正式业务 Artifact。Learning State 将 `ACTIVE / WAITING_FOR_EVIDENCE / PAUSED / COMPLETED / CANCELLED`、`READY_FOR_SYNTHESIS / ROUTE_REEVALUATION_RECOMMENDED / INSUFFICIENT_TO_PROCEED` 和仅提供建议的下一步分开；只有 `COMPLETED` 可有完成处置结论。七类 `source_resolution_type`，以及 PM 访谈的中断原因、核心问题或必要澄清、主张类型、挑战强度、Agent 建议与最强反方、同意或分歧、权限、假设变化，以及下一步或停止原因，都复用 Learning State 或 Round Delta，不升格为新 Artifact、Graph Node 或业务路线；默认不把全量逐字对话当作合同必填。人工改判的完整权限合同仍待 Review。
- Analysis Method Hook / versioned Method Card：只作为 Learning、Synthesis 或 Planning 的内部扩展合同；Card 至少声明 `method_id / question_answered / applicability / non_applicability / required_inputs / output / limitations / cost / skill_version`，调用记录绑定 Level、五问结果、输入 Evidence 引用和方法版本。它不是业务 Artifact、Evidence、Graph Node、Gate 或集中 Registry。
...
- Assumption Audit Checkpoint：只作为 `problem.assumption.audit` 带版本、仅限当前 Run 的内部检查点，保存输入绑定、原话与来源角色分离、五类初始表达拆分、动态关键假设、反证与替代检查、唯一一个 MVU、推荐信息来源、下一信息请求、结构化理由和下一步建议；不提升为正式业务 Artifact、Problem Definition 或正式知识。`next_learning_information_request` 也不替代 Learning State 的三维完成合同、人工改判或 Evidence Request 全部字段。
- Problem Definition Candidate / Synthesis Result：`problem.synthesize` 只接受确切的 Learning State=`COMPLETED` 且完成处置结论=`READY_FOR_SYNTHESIS`，并绑定 Raw Signal、Knowledge Snapshot、Product Memory Snapshot、Problem Evidence Map、Assumption Audit 检查点、Learning State、Round Deltas 与已记录分歧的确切版本和哈希。带版本的 Candidate 至少保存用户、场景、目标、阻碍、影响、期望改变、证据边界、假设与未知项、范围与非问题，以及用户提出方案与问题的关系。Result 只能是 `COMPLETED / RETURN_TO_LEARNING / FAILED`，同时保存来源绑定、输出版本与哈希、失效与取代关系。它是正式 Run 候选，不是正式 Knowledge、Decision、Plan、PRD、解决方案或 Problem Ready；`RETURN_TO_LEARNING` 必须携带会实质影响结果的缺口与新 MVU，但不能伪装成已完成的新 Learning Round。
- Problem Quality Review Attempt / Finding / Disposition / Problem Ready Calculation：审查尝试只读取确切、已冻结的 Candidate 及其绑定的 Evidence、Learning、Knowledge 和 Product Memory，并与生成上下文隔离；首次完整审查，后续针对变化范围复审，同时回归全局不变量。Finding 保存 Evidence 引用、关注事项、等级、影响，以及 `REVISE_SYNTHESIS / RETURN_TO_LEARNING / NEEDS_OWNER / ROUTE_REEVALUATION` 建议修复路径；它不编辑内容或写状态。固定 `problem.owner.confirm`、`PM_ACKNOWLEDGED` 与 `OWNER_CONFIRMED` 已取消。Ready Calculation 只绑定当前确切 Candidate、确切的 Review 审查尝试和处置结论、上游 Evidence、Learning 和 Synthesis 引用、规则版本、`READY | NOT_READY`、未满足条件、确定性修复位置、迁移与 Audit 引用，仅可由程序化 State Controller 正式写入。Owner 的产品责任由随后同屏展示确切 Problem 的 `product.decision` 选择承担；Agent 模拟 Gate 只能为 `ADVISORY_ONLY`。

#### 评测与审查：运行收口

- Sub-agent Attempt / Join-Aggregate Contract：绑定父 Run 与执行尝试、子任务角色、确切输入引用与哈希、Skill 和政策、最小权限配置、请求与实际使用的模型配置、提供方和版本、分发数量、并发、预算、超时、重试，以及统一的 `NodeResult / Finding / Proposal` 结果哈希与状态。子智能代理不得写状态、当前指针、正式知识、released 产物或外部副作用；必需任务失败只使相关动作未满足，可选任务失败可记为 `NOT_AVAILABLE`。汇合时保留分歧和每份原始结果，多个 Agent 同意不提升证据置信度；它是现有节点的执行合同，不是新业务 Artifact 或 Graph Node。
- Review Finalize Result：`review.finalize` 的带版本 Controller 迁移记录，绑定确切的 Aggregate 尝试与哈希、Candidate 引用与哈希、适用或必需的审查尝试及不可用原因、每条 Finding 的处置结论、同源 `internal_review` 视图引用与哈希、规则版本、未完成的已采纳修复或上游修复位置，以及 Audit 引用。它是现有 Review Record 或 Run 迁移的收尾记录，不是新 Artifact、Gate 或节点；输入失效或缺失、Finding 丢失、处置结论缺失或配套视图版本不一致时，保持当前状态并列出确切的流程修复位置。Reviewer 的关注等级不进入阻塞计算。
- PRD Optimize Loop/Attempt：复用 Run State、Review Record 与 Document Changelog，保存 `loop_id / planning_profile / round / round_limit / no_progress_count / current_candidate_ref+hash`，以及允许在当前 PRD 修复的 Aggregate 或 Finding 引用、声明已修复项到变更的映射、实质性变化、未修改项及理由、失效与复审范围、新 Candidate 引用与哈希，以及停止、恢复或返修路由。Review Attempt 独立引用 Candidate，不复制 PRD；Agent 临时草稿不记录版本，每轮实际送审的批量修订才生成一个 archived Candidate。这些是现有引擎和状态的恢复字段，不是新业务 Artifact 或 Loop Node。
- PRD Ready Calculation/Assertion：绑定确切的 archived Candidate 与哈希、同版本的必需 Review、Aggregate 和 `review.finalize`、同源内审意见视图、当前关键上游引用、Eval Applicability、Fulfillment 与 Pack 引用、Template、Document Experience、版本与 Changelog 校验、规则版本，以及 `READY | NOT_READY`。建议性关注项可以披露，但不因未关闭而阻塞；若 Reviewer 指出机械缺口，只有 Validator 独立命中既定合同才返回 `NOT_READY`。`NOT_READY` 保存确切的未满足条件、受影响引用与版本、修复位置和恢复点；`READY` 原子关联 release promotion 与确切、自包含的 Released 产物集引用。

#### 规划与 PRD

- Product Plan / Target Operating Outcome / Current Iteration Outcome / Planning Item / Product Plan Candidate：同一 Product Plan 合同按版本承载 v0、`planning_profile` 及选择或变更理由、可选的 Wave 或 Round 检查点、实质性深化或协调检查点、稳定且已冻结的 Candidate、变更摘要、`supersedes` 和影响引用，不另建可漂移的“深化报告”Artifact。Refinement 是 `product.planning` 内部可恢复 Loop；正式 Review 审查尝试绑定确切 Candidate 与哈希，并复用通用 Review–Optimize 引擎和 Profile，Reviewer 只读，Optimizer 生成新 Candidate。
- Planning Views / Internal Action Contract：Module / Iteration / PRD Matrix / Dependency & Shared Contract 可独立或内嵌；`plan.slice` 在同一 Product Plan 版本中保存 PRD Candidate Slice Map/List，包括目标或问题、用户结果、阶段、模块、依赖、验证、切分理由和来源引用，不冒充 PRD；`plan.coverage.validate` 保存每项处置结论、五类 Finding、Profile 或模式，以及责任和修复引用；`plan.reconcile` 保存处置、是否实质影响的判断、Decision 引用、Plan 变化、上游修复路由与未解决冲突条件。Plan Ready Calculation 只保存确切 Plan 与激活集合、Coverage 和处置结论、仅提供建议的 Review Finalize、依赖与冲突、实质性 Decision 引用、规则版本、`READY | NOT_READY`、修复位置与已创建子项引用，不要求 Owner 确认 Plan。按需一页摘要只是同源人类视图。前三者可在 LIGHT 内联，但确切的来源与输出版本、哈希和审计语义不能丢失。
- `EXPERIMENT` 条件化内容不形成独立 Artifact 合同：它复用 Product Plan、PRD Candidate/release、PRD Eval Pack、Review/Ready/Dispatch 与 typed-result binding，并在同一正式产物内携带 §10.4 的 key unknown、exposure、measurement、mapping、guardrail、stop/rollback 和 result-return 引用。一期不建立 Experiment Plan/PRD/Eval/Ready/Handoff/Portfolio 产物族。

#### 问题与决策

- Product Decision Draft/checkpoint / Decision Record / Planning Activation Event / Decision Ledger / Roadmap Registry Snapshot / Research Request / Roadmap Change Proposal / Product Changelog Proposal / Product Memory Impact List：Draft 或 checkpoint 是 `product.decision` 带版本、仅限当前 Run 的恢复合同，绑定确切输入、AI Brief、按需 Review 引用、分歧、Owner 讨论状态与取代关系，不是正式决定、承诺或路由。Decision Record 的确认层固定为已选决定、适用范围、最多三个关键理由、最大未知项及翻转、停止或重启条件、下一步与检查点或触发条件；只有存在实质性分歧时，才增加双方结果与理由、分歧、授权依据、接受的不确定性与风险、复查或停止条件和执行约束，并证明已完成一次有界的实质性质疑。系统层自动维护身份与时间戳、确切的 Problem、Evidence、Knowledge 和 Review 引用、建议与结果、取代关系、下游引用，以及理由和 Audit 引用，并按适用性内嵌限定行动或暴露范围的 `R0—R3 | RISK_PENDING` 分类、理由和对允许动作的影响；该分类不是独立 Artifact、Node 或 Gate。只生成已选结果的条件化 `outcome_details`，不产生五套模板或未选项空字段。节点结束时复用 State Controller 的迁移校验，并将通过或拒绝记入既有 Audit Event；该计算不是 `decision.ready`、业务 Artifact 或新 Gate。只有 Owner 作出确切选择后的不可变、带版本 Decision Record 与 Controller 路由是正式边界。Ledger、Roadmap、Changelog 和 Audit 从这份 Record 和后续确切事件产生不同语义投影，不复制结果真源；新 Evidence 对历史 Decision 的分类和逐产物 Impact List 绑定确切的 Evidence、Decision 和 Artifact 引用。

#### PRD 与文档体验

- PRD Candidate / released PRD，以及可选的 PRD Content Checkpoint：Candidate 与 released PRD 是同一交付合同的生命周期版本；Content Checkpoint 仅在配置命中复杂恢复场景时保存，绑定确切的父级引用、Candidate 与哈希，不进入 `released/`、不单独 Handoff，也不成为正式业务真源。PRD Review 合同把 Fidelity 和 Alignment 作为 Product Reviewer 的首要检查准则，并由确定性检查绑定 Decision、Plan、Slice、Knowledge、Evidence 和约束的确切引用、Slice 成员关系与处置结论；结果继续使用既有 Finding、Verdict、变化记录与 Audit，不生成独立 Fidelity Report。第一版只使用 `prd.content.build`，不注册旧名别名。
- PRD Template Profile：保存 `profile_id/version`、模板的确切来源、版本与哈希、选择来源与优先级、可信知识或项目配置引用、适用与不适用范围、必要语义到章节、字段或扩展的映射、`incompatibility reason` 和 `resolved_at`；它是 `prd.generate` 的配置与解析合同，不是 Template Adapter、业务 Artifact、Graph Node 或模板 Router。general v0.1 保持 Draft/Bootstrap 候选；何时升级为当前默认模板由未来带版本的配置与 Roadmap 决定，V1.4 不冻结人工 promotion Gate。
...
- Eval Applicability Decision / Eval Fulfillment State / Product Eval Strategy / PRD Eval Pack：Decision 绑定确切的 PRD、Plan 和 Eval Policy 引用、`NOT_NEEDED | RECOMMENDED | REQUIRED`、人类结论与理由；Fulfillment 独立记录 `NOT_STARTED | GENERATING | GENERATED_PENDING_REVIEW | REVIEWED | BLOCKED_MISSING_INPUT`、缺失输入及其 Owner 和修复方式，以及确切的 Pack 与 Review 引用。`evals.build` 另绑定确切、稳定的内容来源，范围、生成和审查尝试，Pack 版本与哈希，PRD 规则或 AC 的可追溯关系，覆盖与缺口、失效影响和汇合结果，但内部三个动作不注册顶层节点；最小语义见 §12.2.1，本期不冻结庞大 Schema。第一版不实现未发布旧值的兼容迁移。TDD-ready Test Design Contract 只是未来扩展接口，正式测试用例、代码、执行和 Verdict 不进入该合同。
- Reviewer Finding / Finding Disposition：PRD goal-fidelity Profile 复用同一项仅提供建议的 Finding，并补充 `reviewer_role/profile / concern / concern_level / basis_refs / upstream_commitment_refs / affected_scope / possible_impact / professional_recommendation / confidence / confidence_basis / cross_check_status / repair_target / disposition`；交叉检查使用 `confirmed / complemented / conflicted / unique / unsupported`，修复验证使用 `FIXED / IMPROVED_BUT_OPEN / PERSISTENT / REGRESSED / INSUFFICIENT_EVIDENCE`。关注等级只排序，不赋权；这些是既有 Review 或 Audit 字段，不生成独立审计 Artifact、分数或 Gate。
...
- Product Released Artifact Set / Incident Verification Packet（`incident.verification.packet.v1`）/ Connector Side-effect Authorization：所有 PRD，包括 `EXPERIMENT` intent，在同一个 PRD Ready 后直接使用自包含 release，不另行构建 Product 或 Experiment Handoff Package。Authorization 只绑定确切的 Connector、目标、动作、产物版本与哈希、政策、责任主体和有效期，决定能否发送，不重新判断 PRD 语义。线上问题核查包使用只追加的内容版本与轻量机械检查，不复用 Ready Assertion。
- Dispatch Attempt / Receipt（复用 State 与 Audit 记录，不新增业务 Artifact）：至少保存执行尝试 ID、政策与版本、`PRD ID + Released Version + Connector + Target + Action` 幂等标识、确切来源目录以及 Markdown 和 assets 哈希、执行主体与预授权、开始与结束时间、`PENDING / SUCCEEDED / UNKNOWN / FAILED`、远端 ID、URL、版本与回执、查询、协调与重试引用，以及来源后续失效或被取代的影响链接。Receipt 只证明该目标的可验证远端动作，不能替代 PRD，包括实验 intent、Incident 或 Bug 语义合同，也不能替代外置审批。
- 外部回传不定义独立的通用 Feedback Artifact：纯生命周期状态复用 Dispatch 或 External Status；带类型的结果先绑定已有 Incident、Bug、Handoff、带 `EXPERIMENT` intent 的确切 Decision、PRD、Run，或未来 Test Result 记录。只有新的产品事实、冲突或挑战才派生带确切引用的 Product Signal。研发 Incident 回传仍追加为同一 Packet 的后续版本，返工影响复用现有 Impact List、Decision 取代关系与 Audit。
...
## 15. 确定性状态控制器（State Controller）

本节给状态机、Validator 和恢复机制实现者看。主要结论是：智能代理、Skill、Reviewer 和 Connector 都只能提交候选结果或迁移请求；状态控制器必须基于当前确切记录重新计算规则，并且是唯一能原子写入正式状态的组件。

### 15.1 唯一正式状态写入者

Orchestrator、原子 Skill、Reviewer、Connector 和人类批准动作都只能提交候选结果或事件，不能直接写当前状态快照或提升正式产物。

唯一写入者是 Plugin 中的确定性 State Controller。

一期它只是 Better Product Graph Plugin 内的本地模块或程序库，维护每个 Run 的当前状态快照、只追加的有意义事件流和独立版本化的产物；`state.yaml`、`state.json`、`events.jsonl` 只是可选实现示例，文件名和完整字段本轮不冻结。它不是独立 Service、MCP、CLI、daemon 或常驻进程。它的“确定性”来自带版本的规则、明确输入、纯规则计算、CAS 或原子写入和自动化测试，不来自“代码天然正确”；规则版本、测试和迁移行为必须与有审计价值的状态事件一起可追溯。

若早期原型暂时用 Agent 模拟 Controller 计算，输出必须显式标为 `ADVISORY_ONLY`，不得写正式 Ready、提升 Artifact 或授权 Handoff；正式 Handoff 前必须由程序化约束重算。子智能代理不能替代正式 Controller：模型有概率波动，可能被当前上下文或注入内容说服，既生成又自审存在职责冲突，也难以让同一输入在回归测试中稳定得到同一状态迁移。
...
Controller 自己不做产品语义判断；它只检查当前规则要求的确切产物、Review 审查尝试与 Finding 处置结论、版本与哈希、Validator 结果、Owner Decision 或 Connector 副作用授权是否齐全有效。Reviewer 关注项不进入一期阻塞计算；未来的自主治理能力不得以占位字段偷渡到当前规则。

计算 Problem Ready 时，Controller 只执行 §9.6 的三类机械规则：Problem Definition Candidate 是确切、当前有效且未因实质性变化失效的版本；同版本、仅提供建议的 Quality Review 已完成，且所有 Finding 都有明确处置结论；上游 Evidence、Learning、Synthesis、Knowledge 和 Product Memory 的引用、状态、版本与哈希一致且可解析。它不再检查 `problem.owner.confirm`、`PM_ACKNOWLEDGED` 或 `OWNER_CONFIRMED`，也不重新阅读文案、“凭感觉判断是否是好问题”或评估尚未选择的行动。结果只有 `READY`，自动进入 Product Decision；或 `NOT_READY`，并带确切的未满足条件与确定性修复位置。Reviewer 关注等级、普通未知项或未采纳建议本身不阻塞。Owner 随后在 Product Decision 对同屏展示的确切 Problem Definition 选择结果；实质性纠错必须先返回 Synthesis 或 Learning。

Product Decision 结束时，Controller 复用同一迁移顺序执行 §10.3.5，不调用 `decision.ready` 或第二个审批流程：重算经过 Owner 确认的确切 Record、合法结果、通用与结果专属字段、适用的行动风险与约束、当前上游与历史引用，以及实质性分歧和已接受风险。通过后才按 STOP、WAIT、RESEARCH、EXPERIMENT 或 COMMIT 的语义原子写入路由；失败则保持当前 Decision 状态，记录具体未满足条件与修复位置，并用大白话呈现。它不打分、不创建下游 Run，也不要求未变化的选择重复确认。Controller 不决定 Research 的内部节点形态、正式 Roadmap 发布或任何下游 Ready。
...
- AI Reviewer 试图把关注项、旧 `BLOCK_RECOMMENDED` 或关注等级直接写成正式阻塞、批准或豁免，或仅凭 Reviewer 意见要求阻塞整个 Run。
- 以 Sponsor 授权替代证据、提高认知置信度或移除证据缺口的请求。
...
- 在扩大流量、修改指标或 Guardrail，或改变行动范围后，未经 Impact 检查就复用原 PRD Ready 或 release 的请求。
- 实验的带类型结果未绑定原 Decision、PRD、Run 与数据版本，成功标准被事后改写，或只提交选择性结果却要求扩大范围的迁移。
...
- 已批准、引用或交付的 Decision、Roadmap 或 Product Changelog 被原位覆盖，或实质性的 Product Memory Impact 尚为 `IMPACT_PENDING / REVIEW_REQUIRED`，却请求受影响行动继续的迁移。
...
PM prompt 不是由 Agent 自由发送的旁路。Orchestrator 每次准备发出 PM 产品访谈问题时，必须把 Run、当前 MVU、prompt 目的、`source_resolution_type` 和问题集合提交给 State Controller；Controller 读取当前 Run 确切的 `interaction_policy`：
...
**目标是准确继续工作，不是重放整段会话。**一期每个 Run 只需要两个最小机器记录：一个 当前 state snapshot，以及一条 append-only meaningful event stream。`state.yaml`、`events.jsonl` 只是便于讨论的例子；具体文件名和完整 Schema 留到实现时按真实消费者确定，不在架构阶段扩张成 Run Registry、数据库或事件平台。业务 Artifacts、PRD archived/released 文件仍按各自版本合同保存，不算第三份 Run 状态副本。

当前 state snapshot 只需让恢复过程回答：

- **正在做什么**：Run/事项、来源 Signal 和当前目标。
- **走到哪里**：当前 step、last completed step、next allowed step。
- **以什么为准**：当前有效 Decision/Plan/PRD/Eval/Review 等产物的精确引用与版本。
- **还缺什么**：未解决项、等待对象、pause/failure 原因和恢复条件。
- **已经对外做过什么**：Connector side effect、执行尝试/receipt 和 UNKNOWN/reconcile 状态，防止重复执行。
- **在哪里继续修改**：仅在并行文件工作实际需要时保存 branch/worktree identity；不为此建立重型 registry。

事件流只追加**有审计价值、会影响恢复或责任判断**的变化：节点进入与完成、正式 Candidate 或实质性检查点、Owner 决定与实质性分歧、重要 Finding 打开与关闭、外部副作用与回执、暂停、失败与停止，以及子智能代理分发、结果与失败。它不记录隐藏思维链、每次工具调用、没有状态变化的内部尝试、每次措辞编辑、自动保存稿或每份工作草稿的哈希。Node Result 是被 Controller 消费的执行合同，可按需引用 Artifact 或形成上述有意义事件；它不是每个 Run 的第三条永久流水。
...
| Run State | 现在到哪里、下一步是什么、哪些精确引用当前有效、等待什么 | 文档内容历史、关键状态为什么改变 |
...
Resume Brief 面向人必须用完整直白句说明“正在处理什么、已经完成到哪里、在等什么、建议下一步是什么、哪些变化可能阻止直接继续”，不能只显示 machine enum。若确切的 Decision/Plan/PRD、外部结果或工作分支发生实质性变化，Controller 不得盲目沿旧 `next` 继续，必须先调用既有失效/Impact/conflict 处理；纯格式变化不得无故重跑全部节点。Machine enum 可以保持稳定，但 Agent 只提出语义状态和下一步建议，State Controller 才能原子追加 meaningful event、更新 snapshot 和宣布节点完成。
...
## 16. 知识快照、影响与重建（Rebase）

本节给知识接入和状态恢复实现者看。主要结论是：Run 读取确切版本的知识快照；新知识只让真正受影响的产物失效并回到最早修复点，Better Product Graph 本身不能发布正式知识。

### 16.1 共同合同

> **CONFIRMED SOURCE COVERAGE / DEFERRED INTERFACE**：未来 Knowledge Graph 的 Raw Data / Source Corpus 必须把**所有正式 Product Decision Record 及其演化**列为必要候选素材，而不只是 Released PRD。`STOP / WAIT / RESEARCH / EXPERIMENT / COMMIT` 的原始 Record、精确来源引用，以及后续维持、复查、推翻和 `supersedes` 关系都要可被未来 KMG 消费：STOP/WAIT 保留证据、假设、拒绝理由和 restart/review condition，防止组织失忆；RESEARCH/EXPERIMENT 保留认知缺口、验证路径与结果；COMMIT 保留承诺和 scope；Decision evolution 则说明项目认知如何改变。不能先压缩成只有“最终结论”的摘要再冒充完整来源。

这只确认**来源覆盖要求**，不确认 Product Graph 的提交 Schema。Node 17 继续 `DEFERRED / PENDING_KNOWLEDGE_REQUIREMENTS`：未来应先定义 KMG 对原始来源语料与派生或压缩知识两层的真实消费、权限和治理需求，再反推采用复制、引用还是索引，何时提交，如何自动化，以及如何采纳、发布、保留、去重和同步。Decision Ledger 仍是 BPG 的产品决定真源，不能与正式 Knowledge 存储合并，也不能复制出第二真源；本节不新增 Node、Gate、Artifact 或 HITL。

### 16.2 当前冻结的最小合同

一期只冻结四项边界：

1. Better Product Graph 可以读取项目配置指定的确切、带版本、只读 Knowledge Snapshot；快照缺失或陈旧时必须披露对当前判断的影响，不能编造正式知识。
2. Better Product Graph 可以在本地保存 Decision、Plan、PRD、Evidence 与原始材料的确切引用；这些都是未来 Knowledge Graph 的来源语料候选，但不是已经提交或发布的共享知识。
3. Better Product Graph 永远不能直接发布正式知识；未来只允许向 Knowledge Maintenance Graph 提交候选素材或变更建议，是否采纳、压缩、处理冲突和发布由后者负责。
4. Knowledge Maintenance Graph 未接入时，Better Product Graph 仍以本地快照和记录独立运行，并明确 `local-only / not team-published`；它不能把“未共享发布”误写成“没有产品决定”，也不能把本地记录伪装成 KMG 回执。

Snapshot 完整度的精确状态、原始材料的复制、引用或索引方式、Proposal/Impact Schema、Connector 方法、Outbox/Inbox 目录、检查时机、rebase/ack、离线同步与跨团队权限均留在 Roadmap 的 Knowledge Requirements 工作中。等真实 KMG 消费者需求明确后，再创建独立版本设计并回写 BPG Connector 合同；V1.4 不把这些历史草案当作一期实现要求或 Ready Gate。

---

## 17. 交接合同（Handoff Contracts）：产品发布产物、线上问题核查包与 Bug 修复说明

本节给交付消费者、Connector 实现者和验收人员看。主要结论是：交接必须绑定确切版本、哈希和清单；Product、Incident 与 Bug 保留各自合同，任何本地 Ready/Released 都不能被外推为对方已接收、已批准或已实现。

**阅读卡——交接（`handoff.dispatch`）**

- **做什么**：Ready 后，确切且自包含的 Released PRD directory 直接成为本地交付单元；再按每个 Connector+Target policy 决定不发送、等待人工外部写权或自动写入研发/飞书，并记录真实的发送尝试与回执。
...
- **人做什么**：只在 side-effect policy 没有预授权外部写入、或专业 Domain/Incident/Bug policy 明确要求时授权确切动作；不重新审批 PRD 语义。任何交接授权都不能外推为自动回滚、数据修复或对外沟通。
- **主要产物与完成**：Product 交付确切版本的 Released PRD artifact set；实验 intent 复用同一 Plan/PRD/Ready/Released/Handoff，只在同一 PRD 中增加条件化实验内容；Incident/Bug 发送其确切 Packet/Brief 版本。BPG Ready/Released 只证明产品侧交付物成立；只有远端 receipt 或人工确认才能声明已发送/接收，外置组织审批仍有自己的状态。

供人阅读的 Handoff View 必须按 §13.2 从同一 Released PRD、Release/State/Audit 引用 按需渲染；它不产生包内副本或新增 Handoff 业务 Artifact。`handoff` Profile 必须把 `GENERATED / READY_TO_DISPATCH / SENT / RECEIVED / RESULT_RETURNED` 以及独立外置组织审批状态分开，Renderer 不得根据“文件/导出已生成”“BPG Released”或“Connector 已发送”写成“下游已接收/组织已批准”。source 或依赖版本变化后，旧视图 失效 并重新渲染和校验。
...
Ready 后不再复制 Product Plan、Decision、Evidence、Review 等十几个文件组成另一套物理 Handoff Package，也不创建独立 Handoff manifest。现有 Release/State/Audit records 直接保存确切的 released directory、主 Markdown/assets 哈希、同源内审意见的引用与哈希、`source_draft_ref/hash`、上游重要引用、派生视图，以及每次 dispatch 的发送尝试与回执。需要审计时按精确引用展开原记录；不把它们复制成第二份真相。

`READY_TO_DISPATCH` 只表示这个确切的 Released PRD artifact set 已可供某个 Connector、Export Adapter 或人工流程消费，不表示“物理打包完成”、已发送、已导入或已接收。同一 release 发往多个目标时复用同一唯一正式来源；每个目标只生成独立的 dispatch 发送尝试与回执，不复制正式内容目录。Handoff 不读取 `current/latest` 猜版本；release 后被 invalidated/revoked 时原目录保留，状态、Impact、替代关系和后续 dispatch 通过 records 表达。

第一版不实现 `handoff.package.build`、旧多文件 package 目录、legacy event 或兼容回放；Product Plan/Decision/Evidence/Review 引用 供外置汇总审批按 Release/State/Audit records 读取。外部审批的结论、会签和组织责任由其自身系统记录，Better Product Graph 不复制该审批流。Incident Verification Packet 与 Bug Fix Brief 仍保留各自轻量正式交接合同，本决定不把它们并入 PRD release。

### 17.2 `EXPERIMENT` Intent 的同一 Released PRD 交付

`EXPERIMENT` 不生成独立 Handoff Package、manifest 或目录。它使用 §17.1 相同的 self-contained Released PRD directory；主 Markdown 的条件化实验 section 和 Release/State/Audit 引用明确标示 key unknown、exposure、measurement、结束时间、kill/rollback、Guardrails、Owner 与 result-return binding。接收方只在这些确切边界内执行。

同一个 release 可以发送给多个目标，但仍复用同一份正式 Markdown 与 assets，只增加各目标的 dispatch 发送尝试与回执。任何扩流、延期、改变成功或无结论标准、取消 Guardrail、扩大用户范围，或把学习性 action 转为长期产品，都属于实质性变化：必须返回 Product Decision，并生成新的 Plan/PRD/release version；不能通过 Connector 参数或下游口头确认无痕改变。
...
Engineering Incident Handoff 只发送一个确切 Packet 版本及 transport envelope，不拆出一组重复内容文件。业务层机械检查只验证三项：原始 Signal/证据未被改写；事实/推断/未知分离；`engineering_questions` 让接收方明确知道核查什么。通用传输边界另外检查版本/哈希、接收方和权限，但不是专业 Reviewer 或 Ready Gate。
...
`IMPLEMENTATION_DEVIATION` 使用 `bug.fix.brief.v1` 作为正式交接内容，不包装成 Product Plan/PRD。Handoff envelope 至少引用：当前 Bug Baseline Assessment、Agent Route Recommendation、Controller Route、必要 clarification/override record、Bug Fix Brief、适用的 Bug Eval Pack 或 `NOT_NEEDED` 理由、版本/哈希、接收方/权限与回传合同。来自 Incident 的事实和证据继续引用 Incident Verification Packet 的确切版本，不复制为第二份真源。
...
下游人员或系统只需提交自然原文或已有协议的原生结果，不填写 BPG YAML、内部 route 或返工节点。研发 Graph、测试 Graph、飞书和人工回传都先进入唯一 `signal.ingest`；Connector 自动附加来源系统、external object/event ID、时间、raw payload、身份/权限，以及当前上下文中可得的 PRD/Decision/Incident/Bug/Experiment 精确引用。缺少关联时先由 `signal.relate` 推断；只有仍无法确定且会实质改变绑定或处理时才做一次最小澄清。
...
来源协议明确声明的 event kind 是可验证的传输层事实，Core 可以确定性识别“这是某个 dispatch receipt 或某份 Incident Result”；它不授权 Connector 把内容判断成 Bug、Incident、Discovery 或 Planning 问题。普通自然语言由 Agent 识别。纯状态和没有新产品事实的执行结果不创建 Product Signal、不启动新 Product Run，也不进入 `route.select`；typed result 包含新的产品事实、冲突或挑战时，Graph 才创建一条引用原 result 与确切上游合同的关联 Product Signal。Router 仍只有四个业务 destination，不增加“研发退回”或“测试失败”路线。

**影响判断不是“默认退 PRD”。**Agent 先把输入区分为新可核查证据、执行结果、个人意见/建议或重复信息，并给出专业判断：它是否影响既有结论、最早影响哪一层、依据和最强反方是什么、什么新证据会升级或翻转。轻量 实质性判断 至少回答：

1. 能否绑定确切的 PRD / Decision / AC / result；
...
当存在 实质性 产品影响时，返回**最早被新证据实质影响、且当前仍有效的环节**：

| 新证据真正改变的层 | 处理位置 | 不允许的捷径 |
|---|---|---|
| Implementation Deviation | 研发核查/修复，引用 Bug Fix Brief/Incident Packet | 不改 PRD 来掩盖实现偏差 |
| PRD 遗漏/歧义，但 Decision、Plan、Slice 仍成立 | 生成新 archived PRD Candidate，定向 Review 后以新 release 取代 | 不覆盖 Released PRD，不全链重跑 |
...
返回上游不是由普通意见自动触发。任何人和下游系统都可以提交反馈，但不能直接把 Decision/Plan/PRD/Roadmap 标为 invalid、reopened 或自行选择返工点。Agent 负责专业语义判断；State Controller 只按 versioned policy、确切 引用 和已有权限执行状态动作。机械且边界清楚的 result update/local repair 可自动执行；只有 实质性 改变既有 Product Decision、承诺范围或 Roadmap 时，才需要当前有权 Owner 形成新决定。当前一期操作者按 ADR-044 视为 Owner，外置汇总审批仍在 BPG 之外；本节不新增内部审批 Gate。更加细化的领域 实质性判断 policy 仍需真实回传样本验证，不能由 Connector 私有实现。

Audit Ledger 复用现有事件合同，至少能追溯 `EXTERNAL_INPUT_RECEIVED / LIFECYCLE_STATUS_UPDATED / TYPED_RESULT_BOUND / PRODUCT_SIGNAL_DERIVED / IMPACT_ASSESSED / STATE_CHANGE_APPLIED` 的 actor、精确的 source/result/artifact 引用、判断与规则版本、建议、Owner 决定（若需要）和 Controller transition；事件只证明发生了什么，不替代 Result、Decision、Impact List 或 Product Signal 的业务语义。

所有已 Released PRD 都不可原位覆盖。实质性 PRD 变化必须形成新 archived Candidate 和新的 取代旧版的 release，保留旧版本、change reason、触发它的下游输入/结果、Impact List、受影响范围和 downstream 引用。未改变产品事实的发送状态、执行状态、重复信息或普通意见不使全部 Decision/Plan/PRD/Eval/Handoff 失效，也不触发全链重跑。
...
带 `EXPERIMENT` intent 的执行结果以 typed result 进入统一 `signal.ingest`，先绑定确切的 Decision/Released PRD/Run、Eval 版本、实际受众/流量、运行窗口、数据版本、Guardrail/kill 事件和已知干扰；再成为新 Evidence，并由适用 Reviewer 检查完整性与解释后返回 Product Decision。Connector 或执行方不能直接写 expand/`COMMIT`，也不能用一次结果无痕覆盖原 PRD/Decision。

---

## 18. 连接器挂载规则（Connector）

本节给外部系统集成人员看。主要结论是：连接器只能在声明的挂载点运行；读取外部材料和执行外部写入是不同权限，写入还必须满足确切目标、动作、版本与副作用授权。

| Connector | Mount Point | 作用 | 前置条件 |
|---|---|---|---|
| 所有入站 Connector（Issue Collector、飞书、Development/Test Graph、未来 Input Connector） | `signal.ingest` | 传输/解析原生协议，提交自然原文或 typed result，并自动附加 provenance | Connector identity/permission、raw payload、source system、external object/event ID、time、哈希/幂等信息和可用的精确引用；外部提交者不填 BPG 内部 Schema，Connector 不作产品分类/路由 |
| Knowledge | `context.snapshot.load`（一期）；未来提交与影响接口待 Knowledge Requirements 反推 | 读取项目配置指定的确切、带版本、只读 Knowledge Snapshot | BPG 不发布正式知识；原始与派生内容提交、Impact、确认、离线同步与权限合同继续延后 |
| External Audit | `review.external.request` | 请求 Claude 等 Agent 审计固定版本 | 返回结果经本地 Validator，不能写状态 |
| Feishu Project | `handoff.dispatch` | 创建或更新产品需求（包括实验 intent）、线上问题核查单或 Bug 修复单 | PRD 对应同一个 Ready；Incident/Bug Fix Brief 对应各自轻量检查；versioned side-effect policy 预授权或确切的外部写入授权；profile、幂等/去重策略和回执 |
| Development/Test Graph | `handoff.dispatch` | 提交 Product Released artifact set（可含实验 intent）、Incident Verification Packet 或 Bug Fix Brief | PRD 对应同一个 Ready；Incident/Bug 分别对应三项/五项业务轻量检查；versioned side-effect policy 预授权或确切的外部写入授权；通用版本/权限检查 |

Connector 不可用时：

- 任一入站 Connector 不可用时可降级为自然语言或显式 Skill 的手动输入，Core Intake 和后续 Product Run 仍可运行。
- 一期 External Audit 始终是 建议性；不可用时记录 `NOT_AVAILABLE`、原因和影响，不因其意见形成语义阻塞。Blocking profile 只属于未来无人值守治理。
- Feishu/Development Connector 不可用时保留确切版本的 Released PRD artifact set；可手工传递或按需生成 DOCX 后人工导入，但没有远端 receipt/人工可验证证据时不得声称已发送、已导入或已接收。
- Knowledge Connector 未接入时使用项目本地的确切快照，并标记 `local-only / not team-published`；snapshot 缺失、陈旧或与当前行动明显冲突时，按现有 Evidence/Unknown/Decision 规则披露影响或返回补证，不能假装已完成 KMG Impact 检查。严重或持续伤害的 Incident Verification Packet 仍按轻量政策优先交接。
...
有副作用的 Connector 必须解析 versioned side-effect policy。Policy 至少将 connector、target/project、action、artifact type/version/哈希、授权 identity/capability、有效期、幂等与 receipt 要求绑定在一起；Product dispatch 的默认/自动规则见 §18.1。该授权只允许相应外部写动作，不提高 PRD 的语义置信度，不替代 Domain Owner，也不代表组织外置汇总审批通过。Connector 未安装或不可用时，本地 Released artifact set 仍然完整，状态不得伪装成 `SENT`。

### 18.1 `handoff.dispatch / 产品交付发送`

`handoff.dispatch` 是现有 `handoff.dispatch` mount 上的**可选、可恢复 Connector boundary 执行尝试**，不是 Product Loop 本地完成的必经节点。无 Connector、Connector=`disabled` 或权限不可用时，确切 Released PRD+assets 仍是完整本地交付物，可人工传递或使用 §18.2 的 DOCX fallback；外部能力不得撤销或阻止 `READY / RELEASED`。

Dispatch 不打包、不重新生成或改写 PRD、不做语义 Review，也不重复 Owner 内容审批。它只消费确切且自包含的 Released artifact set，经目标 Adapter 执行必要的渲染/写入，并把发送尝试、远端结果和回执追加到现有 State/Audit records。Development/Test Graph Connector 同样不能回写正式 Product artifact。
...
| `manual` | 可以发送，但每次需明确外部写入授权 | 首次或未配置时默认；自然语言“把这个需求提到飞书”可触发一次确切的 manual 发送尝试 |
| `auto_when_ready` | 本地 Ready 后可自动发送 | 仅当项目级 versioned preauthorization 明确覆盖确切的 connector、target、action、artifact type/scope、identity 和有效期时成立 |
...
每个 执行尝试 的幂等 identity 至少稳定绑定 `PRD ID + Released Version + Connector + Target + Action`。调用前先记录 pending 执行尝试；明确成功时必须保存远端 ID/URL/revision/receipt 或人工可验证的等价证据。超时、连接断开或响应不确定时状态为 `UNKNOWN`，必须先 query/reconcile 远端状态，再决定是否重试；无法查询时保持等待，禁止盲重试制造重复飞书文档或项目单。具体 API 幂等能力须在 Connector 原型阶段验证，通用 Graph 不假设飞书一定支持客户端幂等键。
...
`SENT` 只由远端 receipt/ID/URL/revision 或人工可验证等价证据支持，不得外推为 `RECEIVED / ACCEPTED / APPROVED`。同一 release 向多个目标发送时复用同一唯一正式来源，每个目标分别保存发送尝试与回执；一个目标失败或 UNKNOWN 不影响其他目标和本地 Ready。source 后续被 invalidated/superseded 时，原发送记录与回执不删除；任何外部状态/结果回传统一由 `signal.ingest` 接收，并按 §17.6 更新绑定记录、派生 Product Signal 和计算 Impact，Dispatch 自身不选择返工点。
...
现实 fallback 是从确切的 released Markdown 与 assets 按需派生同 stem DOCX，由人手工导入飞书。DOCX 不是唯一正式真源，生成 DOCX 只说明 `EXPORT_GENERATED`；除非用户或远端返回可验证链接、ID、revision 或等价确认，否则不能标为 `SENT / IMPORTED / RECEIVED`。后续 Feishu Connector 与 Export Adapter 都必须从同一唯一正式来源派生，不能反向改写 Released PRD；当前只记录合同和验证点，不实现 Connector、Exporter 或新的 Handoff packager。

---

## 19. 审查者、校验器、检查关口与就绪（Reviewer / Validator / Gate / Ready）

本节给审查政策、校验器和 Ready 实现者看。主要结论是：Reviewer 负责提出语义 Finding，Validator 负责机械检查，Gate 负责计算允许边，状态控制器负责正式写入；四种职责不能互相冒充。
...
- **AI 做什么**：Reviewer 提交仅提供建议的 Finding、证据和建议，不能自行写正式状态或宣布批准/阻塞。
- **人做什么**：实质性 产品判断返回唯一 `product.decision`；外部副作用按 Connector policy 条件授权。State Controller 是唯一正式状态写入者。
...
Document Experience 不增加一类 Gate。共享 Validator 与按需 Readability Reviewer 把结果提交给现有 Problem/PRD/Handoff 边界；实验 intent 仍由 PRD Ready 消费。State Controller 重新解析确切来源及 policy/profile/template binding，并重算确定性结果。Readability Finding 仅提供建议；只有 Validator 独立命中既定最低合同缺口时，相应 Ready 才因该机械缺口失败。

### 19.2 PRD Ready 条件

`prd.ready.gate / PRD 最终就绪检查` 是 State Controller 的最终确定性 validation/transition boundary。它在 `review.finalize` 完成后立即执行，不是 Agent/Subagent、人工审批，也不是需要另外等待/恢复的独立智能 Graph Node。Finalize 只保证审查尝试（attempt）、处置结论和 companion view 完整；Ready 只确认即将成为 self-contained release/Handoff 单元的确切 PRD Candidate 及引用是否机械有效，不重新做语义 Review。

一期只检查六类必要前提：

1. **当前确切 Candidate**：当前对象是 `archived/` 中不可变、materially valid 的确切 Candidate、版本与哈希，不是 scratch、`current/latest` 推测或尚未生成的 release。
2. **同版本 Review 与收尾**：适用 Review 执行尝试、Aggregate 和 `review.finalize` 都完成、有效且绑定该 Candidate；每条 Finding 有 处置结论，同源内审意见 view/引用/哈希 与主 PRD 一致。旧版结果不可复用。
3. **Reviewer 与机械合同分离**：仅提供建议的 concern 即使未解决也只需披露，不阻塞本地 release/Handoff；若 concern 指向必需字段、精确引用或模板映射缺失，必须由对应 Validator 独立复现该既定机械缺口后才使 Ready 失败。
4. **关键上游精确引用可解析且当前有效**：Decision、Product Plan、activated Slice、Knowledge/Evidence 及适用的 Roadmap/Planning/Guardrail/Shared Contract 引用与 Candidate 语义一致；实质性 Product Memory Impact 已有合法处置结论。
5. **Evals 按 Applicability 履行**：`NOT_NEEDED` 有可审计理由时不受影响，`RECOMMENDED` 默认不硬阻塞但披露影响；`REQUIRED` 时，确切版本的 Eval Pack 与必需 Review/Ground Truth provenance 必须达到当前 policy 要求并绑定同一 stable content version，`BLOCKED_MISSING_INPUT` 或失效 Pack 不能 Ready。
6. **呈现与版本机械合法**：Template Profile/mapping/Supplement、Document Experience validation、Artifact version record/哈希、`DOCUMENT_CHANGELOG.md`、Audit 与 Ready Policy 检查通过；必要语义无法承载时不得静默丢失。适用项可使用 `N/A + 理由 + policy/profile 依据`，字段填满率不是 Gate。

本 Gate **不检查或要求** PRD-stage Owner 再确认、组织外置汇总审批、Connector 可用性/外部写入授权、研发或测试已完成、所有建议均已关闭，或任何百分比/加权分数。按需一页最终摘要如已渲染则必须绑定同一 source，但用户是否打开/阅读不是 Gate 条件。

机器结果保持 `READY | NOT_READY`。`NOT_READY` 必须列出确切的 unmet condition、受影响 artifact/引用/version/finding、deterministic repair target 和 resume point，不得只输出“请完善 PRD”。同一组确切输入与哈希，加上相同的 rules/policy version，必须得到同一结果。

`READY` 后，State Controller 无需 Owner 二次确认，自动执行 §12.4.4 的 release promotion：从确切的 archived Candidate 生成 immutable self-contained Released PRD directory，追加 Document Changelog/Release/State/Audit 与当前导航记录。该确切 artifact set 直接成为本地交付单元，不再构建独立 Handoff Package。若 Candidate/哈希、关键引用、Review/route、Eval、Template/Document Experience 或 version record 变化，旧计算失效并重算。随后是否 dispatch 只按 §18.1 的目标 policy/预授权决定；无 Connector 或未授权不阻止本地 `READY/RELEASED`。BPG Ready/Released 不等于组织外置审批、研发实现或测试通过。
...
1. Product Decision 明确为 `EXPERIMENT`，授权与 epistemic confidence/evidence gaps 分开，并绑定确切的 Decision、Plan/PRD 与 Knowledge 引用。
...
5. applicable Review 对同一 Candidate 检查了忠实性、可观测、可停止/回滚、scope/exposure 与风险，Finding 已有 处置结论 并在需要时披露；Reviewer concern 本身不是 Ready block。

满足这些条件后仍只生成普通 PRD Ready Assertion 和同一个 self-contained Released PRD directory。范围、流量、时长、指标、Guardrail、风险或长期承诺发生实质性变化时，旧 PRD Ready/release 按既有 Impact/version 规则失效或需要复审；不得创建 Experiment Ready 来旁路。

### 19.4 Ready Assertion 绑定

- 普通 PRD profile：`archived/` 确切 review candidate、可选 PRD Content Checkpoint、Supplement、确切 `decision_refs`、`roadmap_snapshot_ref`、`product_plan_ref`、`knowledge_snapshot_ref`、Target Operating Outcome、`planning_views_ref`、Slice、适用 Batch 和 Eval Pack 的版本及哈希；Ready PASS 后的 release record 再引用该 Assertion 与 candidate 哈希，不原位修改 Assertion。
- 当 `delivery_intent=EXPERIMENT` 时，同一 PRD Ready binding 额外包含确切 Decision/Knowledge 引用、key unknown/hypothesis、受众/暴露、具体变化、测量/解释边界、风险/授权、监控/停止/回滚和 typed-result return binding；不生成另一份 Experiment Ready Assertion。
- Graph Manifest、Skill、Schema、Validator 和 Gate policy 版本。
- 知识快照、Document Experience Policy、artifact Profile 和项目模板的确切版本；适用 human view 的 source 哈希、rendered_at 和 Validation Result。
- authorization、epistemic status、risk classification、仅提供建议的 Finding 及其处置结论、同源内审意见，以及实际发生的 Connector side-effect authorization records；PRD 内容审批不得成为当前 Ready 隐性前置。
- Audit Ledger head 哈希 与 enforcement mode。

---

## 20. 审计账本、回放与版本管理（Audit Ledger）

本节给审计、恢复和版本管理实现者看。主要结论是：审计记录关键执行事实但不替代正式产品产物；已被引用或交付的版本不可原位覆盖，回放也不能重新执行外部副作用。
...
V1.4 的当前合同是**轻量、有意义事件流**：当前 state snapshot 是恢复当前状态的唯一真源；Audit 只追加会改变恢复、责任、正式版本、Review 处置结论、外部副作用或失败判断的事件，并解释重要状态为何改变。它不记录每次工具调用、每次无状态的执行尝试、每个措辞草稿、每项 Validator 细节或隐藏 Chain-of-Thought，也不承诺用事件完整重建所有中间 state。事件足够时可以校验迁移历史并辅助修复损坏 snapshot，但不能成为第二状态真源。

最低事件类别只有：节点或路线的重要迁移；实质性产物检查点、release 与取代关系；Owner Decision 与实质性分歧；Review 审查尝试、Finding 处置结论与 Finalize；等待、暂停、失败和恢复；Connector 副作用与回执；以及实际发生的子智能代理分发、结果与失败。每条只保存确切引用、执行主体与规则、结构化理由、前后状态和必要回执；具体事件名与完整 Schema 留到实现时按真实恢复消费者冻结。

<details>
<summary>V1.2 事件名设计清单（已退为非规范设计历史，不属于一期实现要求）</summary>

以下清单只记录此前考虑过的事件语义，不能覆盖上面的轻量合同，也不能被实现者解释为“每次执行尝试都必须记录全部字段”。

每个 signal-scoped Intake 或 Run 的执行尝试曾考虑记录：

- Run、节点、执行尝试、Host 和会话标识。
- Graph、Skill、Schema、Validator 和 policy 版本。
- 输入、知识快照、输出和哈希。
- 工具、Connector、权限和 approval。
- Reviewer 审查尝试、仅提供建议的 Findings、concern level、Finding 处置结论、分歧和同源内审意见引用；未来 autonomy policy/waiver 不写入一期当前状态路由。
- Validator/Gate 逐项结果。
- 状态迁移、重试、错误和外部回执。

分类与路由至少使用 `CLASSIFICATION_CREATED / ROUTE_RECOMMENDED / ROUTE_ASSOCIATIONS_RECORDED / ROUTE_CLARIFICATION_REQUESTED / ROUTE_CLARIFICATION_ANSWERED / ROUTE_SELECTED / ROUTE_OVERRIDDEN / ROUTE_REROUTED / ROUTE_SUPERSEDED` 事件，并引用对应 Record 版本/哈希。事件和 Record 只保存结构化理由、证据、未知、政策命中和授权，不请求、保存或通过 `audit.view` 暴露模型隐藏 chain-of-thought。

Discovery Evidence 路径至少使用 `LEARNING_ROUND_STARTED / EVIDENCE_REFERENCE_CAPTURED / PROBLEM_EVIDENCE_MAP_CREATED / MVU_UPDATED / EVIDENCE_REQUESTED / LEARNING_STATUS_CHANGED / LEARNING_WAITING_FOR_EVIDENCE / LEARNING_RESUMED / LEARNING_ROUND_COMPLETED / LEARNING_COMPLETION_DISPOSITION_RECORDED / LEARNING_NEXT_ACTION_RECOMMENDED / KNOWLEDGE_PROPOSAL_SUBMITTED`（按实际发生使用）。事件必须引用 Learning Round/State、Evidence Reference、`problem.evidence.map.v1`、Evidence Request、Knowledge Snapshot 与 Proposal 的确切版本/哈希，并记录查询范围、权限与敏感性。Status event 保存 before/after 与原因；completion event 只有在 status=`COMPLETED` 时才保存三类 处置结论；recommendation event 明确 `ADVISORY_ONLY`、action scope、剩余 Unknown、风险/可逆/可测/回滚和所需 Owner/授权。Round completion 还引用新增证据、假设支持/削弱/推翻、frame/Agent recommendation/MVU 变化、action-relative sufficiency 及继续/停止理由；它证明何时发生采集、映射、等待、恢复和选择，不用事件名代替 Map 内的 claim-evidence 关系，也不记录隐藏 chain-of-thought。

Learning 执行尝试 还应在现有 Audit Event/State 中记录 `LEARNING_SOURCE_RESOLUTION_SELECTED / PM_INTERRUPT_REQUESTED / HUMAN_RESPONSE_RECORDED / LEARNING_CHALLENGE_RECORDED / SOURCE_RESOLUTION_SUPERSEDED`（按实际发生使用），并引用当前 MVU、七类 `source_resolution_type`、选择依据、证据、是否打断、interrupt reason、一个核心问题与必要澄清、PM claim type、`LIGHT / STANDARD / STRONG` 挑战及选择依据、Agent 首选建议与最强反方、agreement/disagreement、authority、假设/frame 变化和 next action/stop reason。事件不能把 PM context/claim、judgment 或 authorization 改写为 user fact，也不能用 Sponsor 授权覆盖冲突证据；`audit.view` 只按权限呈现这些结构化依据及必要原句，不能重建或补写隐藏思维链。默认不永久保存全量逐字对话；这些事件只是现有 Learning Loop 的审计语义，不创建新持久节点或独立 Artifact。

Run interaction policy 变化必须追加 `INTERACTION_POLICY_CHANGED`，记录 actor、Run、`scope=CURRENT_RUN`、时间、变更前后值、入口来源（`new/resume` modifier、`interview skip|resume` 或自然语言等价）、恢复点和当时 当前 MVU（如已有）。`NO_PM_INTERVIEW` 实际跳过 prompt 时追加 `PM_INTERVIEW_SKIPPED_BY_POLICY`（或同等稳定事件），引用同一 policy change、当前 MVU、已回答内容 引用、当前未回答及后续 skipped questions、替代来源路由、保留 Unknown、skipped-interview impact、Agent 建议，以及对 allowed/not-allowed actions 的影响。Resume 只更新 policy 和恢复点，不伪造或重放旧回答。两类事件合起来必须可回放 actor/run/scope/time/MVU/skipped questions/alternative source/unknown/allowed-action effect。正式 approval/authorization 仍单独记录，不能因 policy 名称而被误记为“访谈已跳过”。

问题假设审视至少追加 `ASSUMPTION_AUDIT_CHECKPOINT_CREATED / ASSUMPTION_AUDIT_RERUN / ASSUMPTION_AUDIT_RECOMMENDATION_SELECTED`，引用 Raw Signal、Route Record、Knowledge Snapshot、Problem Evidence Map、checkpoint 与 取代 的确切版本/哈希。事件引用 checkpoint 中的原话/事实角色、现象/影响/问题假设/期望结果/提出方案、动态关键假设、反证/替代/历史/no-action/症状原因检查、exactly one MVU、推荐信息来源、下一信息请求、三类下一建议及结构化理由，不保存 hidden Chain-of-Thought；route re-evaluation recommendation 不能冒充实际 `ROUTE_REROUTED`。

问题综合按实际发生追加 `PROBLEM_SYNTHESIS_STARTED / PROBLEM_DEFINITION_CANDIDATE_CREATED / PROBLEM_SYNTHESIS_RETURNED_TO_LEARNING / PROBLEM_DEFINITION_CANDIDATE_STALE / PROBLEM_DEFINITION_CANDIDATE_SUPERSEDED`，绑定全部确切输入引用与哈希、Node、Skill、模型和政策版本、Candidate 版本与哈希、Result 和结构化原因。`RETURN_TO_LEARNING` 记录会实质影响结果的缺口与唯一一个新 MVU，但只有后续 Learning State 或 Round Event 才能证明学习已真正恢复；事件不得补写 Evidence 或隐藏思维链。Candidate 创建或 `COMPLETED` 事件只证明稳定可审候选已形成，不能冒充 Quality Review、PM Confirmation 或 Problem Ready 已通过。

Problem Ready 至少形成 `PROBLEM_QUALITY_REVIEW_STARTED / PROBLEM_QUALITY_REVIEW_COMPLETED / PROBLEM_REVIEW_NO_PROGRESS / PROBLEM_READY_CALCULATED / PROBLEM_READY_TRANSITION_REJECTED / PROBLEM_READY_TRANSITION_COMMITTED`（按实际发生使用）。Review 事件绑定独立审查尝试（attempt）、隔离上下文声明、专门 Skill/model version、确切的 frozen Candidate 及 Evidence/Learning/Knowledge/Product Memory 引用、full/delta mode、全局不变量回归、仅提供建议的 Finding/repair path 与处置结论。Ready calculation 只绑定当前确切 Candidate、同版本 Quality Review 处置结论、Evidence/Learning/Synthesis 引用和 rules version；`NOT_READY` 追加确切的机械未满足条件与 deterministic repair target，`READY` 自动提交进入 Product Decision 的迁移。新 Run 不产生 `PM_ACKNOWLEDGED / OWNER_CONFIRMED / OWNER_CONFIRMATION_INVALIDATED` Problem 事件；Owner 责任由紧邻的 Product Decision choice 承担。Agent 模拟只能追加 `ADVISORY_ONLY`，不得生成正式 transition；一期不产生评分、第二次确认、action-scoped matrix、低风险 `NOT_REQUIRED` 或 Junior PM escalation event。

`product.decision` 按实际发生记录 `PRODUCT_DECISION_STARTED / DECISION_DRAFT_CHECKPOINT_SAVED / DECISION_REVIEW_DISPATCHED / DECISION_OWNER_CHOICE_RECORDED / DECISION_DISAGREEMENT_CHALLENGED / DECISION_RECORD_CONFIRMED / DECISION_TRANSITION_REJECTED / DECISION_ROUTE_COMMITTED` 等价事件。Draft checkpoint 事件绑定确切输入、AI Brief、Review 引用、分歧、Owner discussion state、版本/哈希与取代关系；它不能产生正式 outcome/route。存在实质性分歧时，challenge 事件只记录一次有界的实质挑战，包括 evidence gap、risk、Agent preference reason、Owner response 与仍有效 constraint；它不保存 hidden CoT，也不能用重复事件无限争论。Record-confirmed 事件还绑定 Owner 五项确认、条件化 disagreement/双方理由/authorization/accepted uncertainty-risk/recheck-stop、chosen-outcome-only details、适用 action-risk classification、系统自动引用/identity/version 与 final 哈希。Controller transition validation 失败时，reject 事件保存确切的 unmet condition/repair target 并保持当前 Decision state；通过后才追加 route committed。两者是既有 Controller/Audit 语义，不产生 Decision Ready Artifact/Gate。后续改判必须建立新 Record 并记录取代关系，不能修改旧事件/Record。内部风险、比较、挑战和确认动作保留在同一 Node 执行尝试/事件链中，不借事件名重新注册五个 Graph Nodes。

Product Planning 可按实际发生记录 `PRODUCT_PLAN_V0_CREATED / PLANNING_PROFILE_SELECTED / PLANNING_PROFILE_CHANGED / PLANNING_ROUND_STARTED / PLANNING_ROUND_COMPLETED / PLANNING_WAVE_SELECTED / PLANNING_WAVE_ADVANCED / PLAN_BLOCK_DEEPENING_STARTED / PLAN_FINDING_OR_PROPOSAL_RECORDED / PLAN_SLICE_STARTED / PRD_SLICE_CANDIDATES_CREATED / PLAN_COVERAGE_VALIDATED / PLAN_RECONCILIATION_STARTED / PLAN_RECONCILIATION_COMPLETED / PLAN_REPAIR_ROUTED / PRODUCT_PLAN_CHECKPOINT_CREATED / PRODUCT_PLAN_SUPERSEDED / PLAN_REFINEMENT_STOPPED` 等价事件。这些都是已确认 `product.planning` 内部 Loop/动作的审计语义，不把 Profile、Round、Wave、Slice、Coverage 或 Reconcile 重新注册成顶层 Graph Node。每个事件绑定确切的 Plan 快照与哈希、Planning Profile、适用 Wave/Round、目标 block、候选 slice/coverage 处置结论、Findings/Proposals、处置/实质性判断/Owner、保留分歧、全局影响、change summary、`supersedes` 和 downstream impact 引用；LIGHT 内联时可合并执行尝试，但不能丢失精确绑定。sub-agent 事件仍复用下段通用合同。事件名不能代替 Product Plan 内容，也不能让多个 writer 并发更新正式 Plan。

Plan Ready 按实际发生追加 `PLAN_REVIEW_FINALIZED / PLAN_READY_CALCULATED / PLAN_READY_TRANSITION_REJECTED / PLAN_READY_TRANSITION_COMMITTED / PRD_RUN_CREATED` 等价事件。Gate Calculation 绑定当前确切 Plan、Coverage 与处置结论、仅提供建议的 Review Finalize、依赖与冲突处置、所有必要的实质性 Decision 引用和规则版本；按需一页 Plan 摘要是同源人类视图，不产生确认事件。若 Planning 出现超出确切 Decision 的实质性产品取舍，`PLAN_REPAIR_ROUTED` 返回 `product.decision`，形成新 Decision 后再继续。`PRD_RUN_CREATED` 必须逐项引用已激活且符合条件的 Slice 与 delivery intent；未来项或等待项的处置结论不得借同一 Ready 事件创建 Run，`EXPERIMENT` 也不得创建第二种 Run 类型。这是既有 Controller 与 Audit 行为，不新增人工审批 Node 或第二 Gate。

PRD Run 可在进入 `prd.generate` 前追加一次等价 `PRD_WORKSPACE_INITIALIZED` 生命周期事件；正式内部调用名是 `prd.workspace.initialize`。事件只证明 workspace、精确的 parent 引用、state/version 和目录/恢复位置已准备，不证明 PRD 已生成、被理解、审查或 Ready；其余初始化字段仍待 PRD Run Review。`prd.generate` 按实际发生追加 `PRD_GENERATION_STARTED / TEMPLATE_PROFILE_RESOLVED / PRD_GENERATION_CHECKPOINT_SAVED / PRD_CANDIDATE_ARCHIVED` 等价事件，绑定确切输入、Profile/template/mapping、内部动作版本、可选 checkpoint、candidate path/version/哈希与恢复点；内部三个动作不能借事件名注册为三个节点。第一版尚无已发布运行消费者，因此不实现旧名 alias、迁移 parser 或兼容事件。

PRD Review/Release 按实际发生追加 `PRD_FIDELITY_CHECKED / PRD_REVIEW_DISPATCHED / PRD_REVIEW_CROSS_CHECKED / PRD_REVIEW_OF_REVIEW_COMPLETED / PRD_OPTIMIZE_STARTED / PRD_OPTIMIZED_CANDIDATE_ARCHIVED / PRD_OPTIMIZE_NO_PROGRESS / PRD_REPAIR_VERIFIED / PRD_REVIEW_COMPLETED / PRD_REVIEW_FINALIZED / INTERNAL_REVIEW_VIEW_RENDERED / PRD_FINAL_SUMMARY_RENDERED / PRD_READY_CALCULATED / PRD_RELEASED / HANDOFF_READY_TO_DISPATCH / HANDOFF_DISPATCH_AUTHORIZED / HANDOFF_DISPATCH_STARTED / HANDOFF_DISPATCH_UNKNOWN / HANDOFF_DISPATCH_RECONCILED / HANDOFF_DISPATCH_FAILED / HANDOFF_DISPATCHED` 等价事件。Fidelity/dispatch 事件绑定当前 Candidate、自动提取的 Goal Fidelity Review Packet、Decision/Plan/Slice/Knowledge/Evidence/constraint 精确引用、deterministic results、Reviewer role/profile/审查尝试、首轮隔离、Findings/cross-check/disagreement、限定 self-review rounds、repair status/delta/target 与 global-invariant regression；Optimize 事件只在一轮批量修订真正开始、形成新送审 Candidate 或确认 no-progress 时追加，绑定 loop/round/budget、source+new Candidate、Aggregate/Findings、claimed repair/delta 和 re-review scope；无内容变化的 Review Attempt 不伪造 optimized Candidate 事件。`PRD_REVIEW_FINALIZED` 只证明适用的审查尝试、Finding 处置结论、确切 Candidate 和同源内审意见视图完整一致；它不是 Gate、批准或 Ready。`PRD_READY_CALCULATED` 绑定六类确切检查、rules/policy version 和 `READY | NOT_READY`；`NOT_READY` 必须含 unmet/affected 引用/repair/resume，`READY` 与 self-contained release promotion 连续原子记录。未触发 review-of-review 时不制造占位事件。最终摘要事件只引用同一 source/Review/Audit，不证明用户阅读或批准。Ready/Released 事件是 Controller 生命周期事实，不能被写成组织外置汇总审批通过；dispatch 发送尝试必须绑定 policy version、idempotency identity、确切的 Connector/target/action/artifact 引用与哈希、actor/preauthorization、result/remote receipt 和必要 reconcile 引用，并与 semantic approval 分离。第一版不实现从未发布过的旧 Package/approval 事件迁移。

当前尚无已发布运行版本，因此第一版不建设旧 Owner approval event 的兼容回放。若项目 side-effect policy 需要一次人工授权，只记录新的、确切的 Connector authorization，不创建或复用内容审批事件。

Evals 按实际发生追加 `EVAL_APPLICABILITY_DECIDED / EVAL_FULFILLMENT_CHANGED / EVALS_BUILD_STARTED / EVAL_SCOPE_DEFINED / EVAL_GENERATION_DISPATCHED / EVAL_REVIEWED / EVAL_PACK_STALE / EVAL_PACK_JOINED` 等价事件，分别绑定 确切 PRD/Plan/Eval Policy/content 引用、Applicability、人类结论/理由、Fulfillment、missing inputs/owner、scope、Pack/Review 引用、sub-agent 执行尝试、Impact/失效 reason 与 join 哈希。旧 `NOT_REQUIRED / DEFERRED` 事件原样保留并追加 migration record；不得重写成仿佛历史上已经使用双维合同。Applicability decision 属于 `prd.generate` 内部动作，`evals.build` 是 PRD Run 内按需可恢复子节点；scope/generate/review 事件都不能借事件名注册成用户可见顶层节点。生成 sub-agent 只交候选，不能自报 Ground Truth、`REVIEWED` 或 PRD Ready。

任何横向 sub-agent 执行尝试 至少记录 `SUBAGENT_DISPATCHED / SUBAGENT_COMPLETED / SUBAGENT_FAILED / SUBAGENT_TIMED_OUT / SUBAGENT_RESULT_JOINED` 的等价事件，并保存 parent Run/执行尝试、subtask role、requested model profile、实际 provider/model/version、确切 input 引用/哈希、Skill/policy versions、permission profile、开始/结束、result 哈希/status、timeout/retry 和可得 cost。Host 不支持时记录 `NOT_AVAILABLE / DEGRADED_TO_SEQUENTIAL`，不能伪造并发。Join event 必须引用所有 required/optional 结果、未满足 action 和未消解分歧；多个 Agent 同意不产生新的 Evidence confidence，Audit 不保存 hidden Chain-of-Thought。

Bug 路径至少追加 `BUG_BASELINE_ASSESSED / BUG_ROUTE_RECOMMENDED / BUG_ROUTE_AUTO_SELECTED / BUG_ROUTE_CLARIFIED / BUG_ROUTE_OVERRIDDEN / BUG_FIX_BRIEF_CREATED / BUG_HANDOFF_DISPATCHED`（按实际发生使用）；Product Logic Defect 继续使用正式 Decision/Changelog/Impact 事件。事件必须引用 Assessment、Brief、baseline 引用、五项自动分流条件、Controller rules/version、必要 clarification/override 和适用 Eval 的确切版本/哈希，不能只记录一个“已判为 Bug”的标签。新 Run 不产生固定 `BUG_ROUTE_CONFIRMED` 事件。

Document Experience 复用 Audit Event Schema，至少可记录 `HUMAN_VIEW_RENDERED / DOCUMENT_EXPERIENCE_VALIDATED / HUMAN_VIEW_STALE_DETECTED / READABILITY_REVIEWED`，并引用 source 哈希、policy/profile/template versions、audience/language、validation result 和适用 finding。它们是现有 执行尝试 内的呈现执行事件，不是新的业务 Artifact、节点、Gate 或第二份 Audit Ledger。
...
- 状态恢复：读取 当前 state snapshot，并用 meaningful events 校验关键迁移历史；事件足够时可以辅助修复 snapshot，但不承诺完整 event sourcing 或从事件重建所有中间状态。
...
上一条通用“获授权原位例外”不适用于 PRD package 中已落盘、引用或分享的 `archived/` checkpoint，也不适用于任何 `released/` 文件；这些文件没有原位修改例外。PRD 只有尚未成为实质性检查点、未被引用或分享的当前 scratch buffer 可以原位编辑。
...
State Controller 在正式产物写入前执行 `artifact.version.guard`：未被引用的当前 Attempt 草稿可以原位更新；已交付、已审核、已批准、已进入 Gate 或已被下游引用的产物只能创建新版本。新版本必须带上一版本引用、变更摘要、创建原因和内容哈希；Decision 还必须维护显式 `supersedes` 链。旧版本 哈希 在发布新版本前后都要复核。项目级 `CHANGELOG`、Product Changelog 和可变 `current` 指针只提供不同语义下的导航或变化摘要，都不具有覆盖或删除历史的权限。

PRD 文档进一步遵循 §12.4.4：实质性过程稿进入 `artifacts/prds/archived/` 的 self-contained Candidate directory，正式可交接版本进入 `artifacts/prds/released/` 的 self-contained release directory；既有 `DOCUMENT_CHANGELOG.md` / Release / State / Audit records 追加版本与状态。失效或撤销的 release 原目录继续保留；Handoff 只能消费确切的 released directory 及 Markdown/assets 哈希。该文档账本不能替代 Product Changelog、架构 `CHANGELOG.md`、上游 Decision/Plan/Slice/Knowledge 记录或 Audit Ledger；可选 PRD Content Checkpoint 也没有该替代能力。

Product Plan 深化复用相同的不可变实质性检查点原则，以及可审计的 changelog、取代关系和 impact 语义：v0、每次实质性 reconciliation checkpoint、正式 PRD Slice/Coverage/Reconciliation 的实质性检查点、formal Review Candidate 与进入 Plan Ready 的版本，都不可被后续版本覆盖。它不因此复刻一份 PRD package，Slice/Coverage/Reconciliation 也只是同一 Product Plan 的 versioned section/view/result，不另建“深化报告”、Coverage 大文档或第二真源；Planning Refinement 固定为 `product.planning` 内部 recoverable Loop，正式 Review 绑定 frozen Candidate。具体物理目录及 Module/Iteration Map 的内嵌或独立表达继续留待真实实现和 Node Review。
...
- 外部写入 Connector 必须再次检查 mount point、对应 profile 的 Ready、versioned side-effect policy/预授权或确切的写入授权、Project Policy、action constraint、项目作用域和目标 allowlist；`still_allowed` 不得被解释成对其他 action 的授权。
...
## 21. 宿主插件与宿主适配器（Host Plugin / Host Adapter）

本节给 Codex Plugin 和其他未来宿主的适配实现者看。主要结论是：宿主层只负责发现、意图映射、能力适配和受控工具调用，不能复制或改写 Core 的业务状态机与权限语义。
...
| `resume` | `$better-product-graph resume [run_id]` | “从已保存位置继续这个 Run” | `run.resume` | 先校验 当前 state、确切 引用/files、外部/分支变化与 失效/Impact，再展示简短直白 Resume Brief；不是重放整段会话 |
...
`audit.view` 展示授权范围内的原文/claims、Classification Record、evidence/knowledge/policy 引用、Agent 推荐与实际 destination、并行 `existing_links`、澄清、override、reroute、版本和哈希；Discovery 路径展示 Evidence Reference provenance、Problem Evidence Map 版本链，以及问题假设审视 checkpoint 的原话/事实角色、现象/影响/问题假设/期望结果/提出方案、方向性关键假设、反证/可信替代/历史/no-action/症状原因检查、exactly one MVU、推荐信息来源、下一信息请求、三类下一建议和版本差异；再分别展示后续 Evidence Request、Learning runtime status history、completion 处置结论、仅提供建议的下一步推荐、action-relative sufficiency 与 Knowledge Proposal 引用。Bug 路径还展示 Baseline Assessment、cause class/surface tags、Agent Route Recommendation、Controller Route、必要澄清/override、Brief/Eval 与 Handoff 引用。它读取 signal-scoped Signal Ledger 或 Run Audit Ledger，不保存、不生成也不暴露模型隐藏 chain-of-thought；缺少结构化理由时应显示审计缺口，不能让模型事后编造“思考过程”。
...
每次开始或恢复项目时，Host 先解析并校验**确切的 project root**，再检查它是否已经位于 Git repository/worktree 中。若 `git rev-parse --show-toplevel` 能解析到当前目录或其父级仓库，就复用该 repository/worktree，不创建嵌套 repo；若完全不在 Git 中，则默认直接在确切的 project root 执行本地 `git init -b main`，不为这一步打断用户。Host 必须拒绝把 HOME、工作区集合根或其他未明确的广泛目录当作项目根。

初始化前先确定 `.gitignore` 与敏感文件边界；任何后续 add/commit 都必须遵守该边界。自动 init **不等于**自动 add、commit、push 或创建 remote，也不授予外部副作用权限。日常节点、对话、autosave 和机械状态变化不制造 commit；只有实质性检查点、正式冻结或发布版本、明确集成点，才可以在相应授权/项目策略下提交，冻结版本可使用可追溯 tag。
...
并行 sub-agent 涉及文件修改时必须使用彼此隔离的 branch + worktree；sub-agent 不能并发写主 worktree、当前 pointer 或 released artifacts。主 Agent 在确切的 base 上审查每个 diff、处理冲突并整合，仍由既有 State Controller、Owner 与 Handoff 规则决定正式状态。该规则只约束执行隔离，不把 branch/worktree 注册成业务节点。

---

## 22. 第一期范围

本节给一期实现负责人和验收负责人看。主要结论是：第一期必须把完整产品语义和强制边界跑通，但不把外部连接、未来协作或重型基础设施伪装成已实现能力。
...
- 自动生成并校验最小 `build-manifest.json`：绑定 Plugin SemVer、确切 Git commit/dirty、architecture baseline、execution contract versions/fingerprint、稳定 inventory 与 artifact 哈希；在 fresh installed copy 验证 relative resources、allowlist、identity 与源码工作区隔离。当前不要求签名、SBOM、远程 attestations 或规划稿逐文件人工 哈希。
- Host project preflight：解析并校验确切的 project root；复用已有父级 repository/worktree，否则在项目根静默执行本地 `git init -b main`。先落实 `.gitignore`/敏感边界，不自动 add/commit/push/remote；失败如实记录 `DEGRADED/BLOCKED`，不新增业务节点或 Gate。
- Host Adapter 的 bounded sub-agent capability/model profile 合同与 Core Sub-agent Execution Policy：一期仅采用当前 Host 内部 sub-agent；对抗 Review、可并发 Reviewer、独立研究/Eval/Analysis 候选优先使用只读最小权限 worker 执行尝试，支持确切快照、统一结果、join 保留分歧和 `BEST_AVAILABLE / BALANCED / FAST` 意图。宿主不支持持久化、并发或模型选择时必须如实降级，不把能力写成已实现事实。
#### 状态、恢复与正式真源

- Plugin 内本地 Deterministic State Controller module/library：基于 versioned rules/tests、确切 records/哈希 和 CAS，原子维护每个 Run 的 当前 state snapshot 与 append-only meaningful event stream，并引用独立版本化 artifacts；`state.yaml/state.json/events.jsonl` 只是未冻结的文件名示例。不使用 LLM/subagent 作为正式 Gate，不建设 Registry、数据库、Service、MCP、CLI 或 daemon。
...
- 第一期只保证 `product_plan / prd / decision / incident / bug_fix / handoff / audit / internal_review` 八类真实消费者；`product_plan` 首期范围只覆盖按需一页摘要，`internal_review` 只渲染确切 PRD 的遗留或分歧 concern。所有视图绑定 source/policy/profile/template 的确切版本，PRD 默认继续使用 §13.1 的 `better-product-plan` 模板和现有 Supplement 机制。
- 本地轻量恢复：读取当前 state，验证精确引用、files 和 versions，检查 external result 与 branch/worktree 变化，渲染简短 Resume Brief 后从当前 step 继续；实质性变化先进入既有失效/Impact/conflict。Git 管内容版本，Run State 管当前位置，Audit Events 管关键变化，不互相复制。
- PRD 文档生命周期：`artifacts/prds/archived|released/<PRD-ID>_<短标题>_v<版本>_<日期>/` 使用 per-version self-contained directory；主 Markdown 与目录同 stem、相对引用本目录 assets、无附件不建空目录。沿用 append-only `DOCUMENT_CHANGELOG.md`、可重建当前导航与 version guard；Ready 后，确切 release 直接成为本地交付单元，不再 build 独立 Handoff Package。Markdown 与 assets 是唯一正式来源，exports 按需派生。
- Product Decision & Roadmap Memory 的本地版本化文件、结构化索引、精确引用和 `current` 导航指针；同一 Decision 投影为职责分明的 Ledger/Roadmap/Product Changelog/Audit，不建设四套流程或数据库服务。KMG 未接入时，本地 records 足以完整运行；未来接入后才增加团队共享/正式 发布语义。
- 独立可恢复 `product.decision` 节点：内部编排 AI Decision Brief、按风险/未知触发的 bounded adversarial/domain sub-agent Review、Owner 讨论/挑战与明确选择、Controller deterministic route；允许 versioned run-local Draft/checkpoint 跨会话恢复，但不把五项内部动作注册为 Graph Node，默认不全量调用 Reviewer。只有最终 Owner-chosen Decision Record/route 是正式边界。五种 machine outcome、基本边界、Decision Record 通用五项、chosen-outcome 最小补充、R0—R3 轻量 action-risk classification 与 实质性 disagreement 的一次实质挑战均已确认；Agent 不迎合也不无限阻挠，Owner 在权限内最终选择。清晰自然语言 outcome/scope 经结构化和必要一次 challenge 可直接构成 choice，不再追加确认 UI。Decision Brief/交互/摘要/Handoff 从同一 Draft/Record 渲染，Record 确认后 immutable。
- `product.decision` 终止时复用现有 State Controller transition validation：检查确切的 Owner confirmation/Record、合法 outcome、通用或结果专属字段、必要 risk/constraints、当前引用和 disagreement/accepted risk；通过后按结果确定性路由，失败留在当前节点并输出直白 unmet condition + repair target。不建设 `decision.ready`、Decision Ready Gate/Artifact/Reviewer、Owner 二次确认，也不把外置 PRD 汇总审批搬入此处；Research 内部形态、Roadmap 正式合同和下游 Ready 仍待后续 Review。
#### 业务入口与路由
...
- 本地只读 Knowledge/Product Memory Snapshot，以及正式本地 Decision Ledger、条件化 Roadmap/Research Request 投影、实质性 Product Changelog 和逐产物 Impact List。新 Signal 通过 `existing_links` 关联 historical Decision/PRD，不新增 Router destination，并按 §10.7 四级影响规则选择摘要、提醒或对确切动作逐项处置。向 KMG 提交 raw source、derived knowledge、接收影响通知与同步确认的合同，等待 Knowledge Requirements 反推后再定义；Experiment Portfolio 仅是未来原型风险，不是一期真源。
#### 发现、问题与产品决策

- Discovery Evidence 基础合同：`evidence.collect → evidence.map`、不可变 Evidence References、append-only/run-local `problem.evidence.map.v1`、独立可恢复 `problem.learning.loop`、Learning Round Delta、版本化 Evidence Request、MVU 驱动循环，以及从 Map 提交 Knowledge Change Proposal 的边界。Learning State 明确分开五种 runtime status、三种 completion 处置结论与仅提供建议的 next-action recommendation；`WAITING_FOR_EVIDENCE` 可恢复且不限 human，停止采用 action-relative sufficiency。Learning Loop 内部支持七类 `source_resolution_type` 和已确认的 PM bounded joint judgment 六步：一个 MVU/核心问题、Junior PM 非诱导脚手架、一次最高价值挑战、明确 Agent 首选与最强反方、风险/冲突/可逆性驱动的挑战强度，以及分歧/authority/停止记录；这些内部动作复用 Learning State/Audit，不增加顶层节点、Gate、业务 Artifact 或业务路线，人工 override 仍待讨论。
...
- 独立轻量 `problem.assumption.audit` 节点：Evidence Map 后、Learning Loop 前按五步逻辑生成 versioned run-local checkpoint，默认零 PM 访谈；动态选择方向性假设，只选 exactly one MVU、最佳来源与一项下一信息请求，完成语义仅为可信认知起点；输出三类下一建议，并在输入发生实质性变化或出现 fundamental reframe 时重跑，不创建正式业务 Artifact、Reviewer Loop 或 Ready Gate，也不替代 Learning Loop 的三维完成合同或人工 override。
- 独立轻量 `problem.synthesize` 节点：只在 Learning=`COMPLETED + READY_FOR_SYNTHESIS` 后，读取 确切 Discovery inputs 并一次性生成 versioned Problem Definition Candidate；支持失败恢复、实质性 gap 携新 MVU 返回 Learning、source 失效 重算和 取代。Candidate 完成只表示稳定可审，不表示 Problem Ready 或 Owner choice。
- Problem Ready 只保留两类执行者：内部 Product Quality Reviewer Agent 以隔离上下文和独立审查尝试（attempt）只读确切 Candidate，并输出仅提供建议的 Finding；程序化 State Controller 重算 Candidate、同版本 Review 处置结论与上游精确引用这三类机械条件，输出 `READY | NOT_READY` 并唯一写状态。固定 `problem.owner.confirm` 已折叠到紧邻的 Product Decision，不建设 action-scoped Problem Ready matrix、Junior PM escalation 或低风险 `NOT_REQUIRED`。
- `problem.quality.review` 完整节点合同：确切 frozen Candidate + Evidence/Learning/Knowledge/Product Memory 输入；用户/场景/目标/阻碍/影响、方案偷渡、追溯/外推/因果/反证/Unknown/action relevance 检查；action-scoped Finding/Verdict 与四类 repair path；首次 full review、后续 delta-targeted + global invariant regression；no-progress 返回 Learning/Owner；Junior PM 优先关键 Finding/原因/建议。Reviewer 始终只读，不编辑 Candidate 或写状态。
- Run Interaction Policy：默认 `ALLOW_PM_INTERVIEW`，与 Guided/Standard/Compact style 分离；用户可在 `new/resume` 设置，也可在进行中使用 `interview skip|resume` 原子变更当前 Run。Controller 每个 PM prompt 前强制执行；skip 保留已答、记录未答/替代来源/影响，不代表信息充分或授权继续。完整 Discovery 原则上至少一次实质访谈/等价对话，除非无 实质性 PM-only unknown 且继续提问信息增益低，或用户显式 skip；未来 `NON_INTERACTIVE` 不实现。
#### 规划与 PRD

- Discovery、Problem Ready、Decision、Outcome-first Planning、四个逻辑规划视图和 PRD Run；Evidence 路径不增加独立重型 Ready Gate。`product.planning` 内部 recoverable Refinement Loop 支持 v0、逐块深化、global reconciliation、实质性检查点、取代关系、impact 与 stable Candidate；正式且仅提供建议的 Review 完成后按 repair path 返回。`plan.slice → plan.coverage.validate → plan.reconcile` 形成候选、诊断和协调。固定 `plan.owner.confirm` 已取消；普通忠实 Planning 直接进入机械 `plan.ready.gate`，只有实质性的新产品取舍回到同一 Product Decision。PASS 只为当前 activated/eligible slices 创建 PRD Runs。
...
- `product.decision` 内部条件化 R0—R3 action-risk classification；不新增节点/Gate/PM 表格，不把问题永久标风险。一期只强化 disclosure、仅提供建议的 Review 与外置团队关注，正式专业 blocking/waiver 延后到真正无人值守阶段。
#### Golden Cases
...
- `prd.generate` 内部 Evals Applicability 判断、独立 Fulfillment State，以及 PRD Run 内按需可恢复 `evals.build` 子节点 seam：stable content Candidate 后与 `prd.render` 并行，bounded sub-agent 可按 scope/generate/review 合同形成候选，join 绑定同一确切版本；`REQUIRED` 未满足时只阻止 Ready、不阻止 Candidate 继续成熟。不增加顶层业务路线、Run Profile、PM 问卷或 HITL，也不要求自动化 generator 已完成。
- PRD Fidelity/Alignment 作为现有 Product Review 的首要 rubric：确定性 Validator 检查 确切 引用/version/slice/处置结论，独立只读 Product Reviewer 检查 scope creep、future 偷带、新规则/虚构事实/Unknown 升级、Guardrail/依赖/回滚/Eval 遗漏和上游冲突。实质性 新内容返回最早正确上游；不新增节点、固定 Owner confirm、独立报告或第二真源。Review 收敛后可按需渲染同源一页最终摘要。
#### 审查与就绪

- `review.parallel` 的 Product Goal Fidelity profile：自动构造同源 frozen goal packet；目标忠实逻辑必需，Engineering/Testability 默认且 LIGHT 可合并，UX/Domain/AI Behavior/Security/Privacy/Compliance 按风险增加；首轮互不可见、只读、主 Agent 后聚合；Finding 使用 确切 upstream commitment/evidence/scope、`concern_level`、confidence + confidence_basis、cross-check、repair target 与 处置结论；只对高影响项最多两轮 review-of-review；delta repair status 与全局目标/范围/不变量回归可审计。不要实现完整七阶段、分数/等级、覆盖率阈值、`.product-audit` 或 Shell 脚本。
- 轻量可恢复 `review.aggregate / 审查意见汇总`：LIGHT 可同一用户动作自动运行但仍留 执行尝试。主 Agent 聚类、保留分歧/unsupported并建议最早 repair target；Controller 只校验 执行尝试、同一 确切 Candidate/Goal Packet、Finding 字段、late/失效 与无损追溯。
- `review.finalize / 审查收尾`：Controller 内部确定性 action，只检查 执行尝试、Finding 处置结论、确切 Candidate 与同源内审意见。它不裁决 Reviewer、不要求 PASS、不因 concern 阻塞、不注册 Node/Gate/HITL；主 Agent采纳的修订进入 optimize，其余遗留透明交外置团队。
- 可恢复 `prd.optimize / 根据审查意见修订 PRD`：只处理主 Agent 采纳且可在当前 PRD 修的聚合建议，批量做最小必要修订，生成一个新的 archived Candidate 和 delta/finding mapping。Candidate version 与 Review Attempt 分开；轮次 2/3/4，连续两轮没有实质性进展时提前停止。
- `prd.ready.gate / PRD 最终就绪检查`：Controller 在 finalize 后重算 确切 Candidate、同版本 Review/finalize/内审意见、当前 引用、REQUIRED Evals、Template/Document Experience/version/changelog 与既定机械合同，只输出 `READY | NOT_READY`。Advisory concern 本身不阻塞；READY 无人工二次确认自动生成 self-contained release。
#### 线上问题与 Bug 快线
...
- Product、Engineering、Testability 等仅提供建议的 PRD Reviewer；按风险加入 UX/Security/Privacy/Compliance/Domain 等逻辑角色。它们不拥有当前期 approval/veto/waiver，也不自动套用到 Incident Packet 或可靠 Implementation Deviation Brief。
#### 交付与外部回传

- Product self-contained Released artifact set（包括实验 intent）、Incident Verification Packet 与 Bug Fix Brief Handoff；PRD Ready 后不打包，`handoff.dispatch` 作为可选可恢复 Connector 执行尝试，按每目标 `disabled/manual/auto_when_ready` policy、幂等 identity、UNKNOWN reconcile 和 receipt 规则执行。无 Connector/权限不影响本地完成，也不新增公开命令、内容审批或第二真源。
- 本地统一回传模拟：纯生命周期状态更新绑定 status，typed Incident/Bug/Experiment/Handoff 结果追加到已有记录；只有新的产品事实/冲突/挑战才派生关联 Product Signal，并以“最早被新证据实质影响的有效环节”生成 Impact/返工建议。普通意见不自动 reopen，实质性 Decision/承诺/Roadmap 变化仍需有权 Owner 新决定。
...
- Feishu Project Connector 与原生 Doc direct render；保留 `handoff.dispatch` mount、manual/auto policy、执行尝试/receipt 和 DOCX fallback 合同，但真实租户的创建/编辑、素材上传、权限、项目关联、格式降级与远端 reconcile 均需 `PROTOTYPE_REQUIRED / PENDING_PERMISSION_VALIDATION`，一期不设默认路径。
...
- 跨独立 Agent runtime/Host/provider 的 Multi-Agent Collaboration：只保留角色、确切快照、capability/model metadata、最小权限、统一结果与 disagreement-preserving join 的协议 seam；未来经 Collaboration/External Audit Connector 或稳定协议接入，不作为一期依赖。
...
本节给实现负责人和验收负责人看。主要结论是：按 Slice 0—8 逐步跑通真实端到端行为，每个切片都同时关注目标、必须跑通的路径、对抗验证和本切片不冻结的边界；不能用孤立组件完成代替端到端成立。

本节的 `Slice` 指 Better Product Graph 自身的端到端实现与验证批次，不等于 §11.2 中产品规划的“纵向拆解”。

### Slice 0：最小运行骨架

- **目标**：建立可以被后续切片复用的最小本地运行、分发、状态、审计与恢复骨架。
- **必须跑通**：下列 11 项共同成立，包括唯一公开 Skill、三个 `v0alpha` 逻辑合同、状态控制器、文档体验、子智能代理能力探测、Git 预检和最小恢复。
- **对抗验证**：重点拒绝第二公开入口、内部模块绕过 Controller、危险 Git 写入、共享工作树并发写、旧引用盲续和把降级能力伪装成已支持。
- **不在本切片冻结**：完整文件名与字段 Schema、历史路径迁移、第二 Host、外部 Connector 和独立多智能代理运行时。

1. 唯一源码树和 Codex 构建产物。
2. Node Result / Run State / Audit Event 三个 `v0alpha` 逻辑合同；每个 Run 只持久化 当前 state snapshot + append-only meaningful event stream，Node Result 不另成第三条账本，文件名/完整字段暂不冻结。
3. State Controller、CAS、Attempt 和轻量 Audit Ledger；事件只覆盖实质性检查点、正式状态、Owner、Finding、副作用、暂停失败和 sub-agent 结果，不保存 hidden CoT、逐 tool call 或逐草稿哈希。
...
8. `templates/human-views/`、Human View Metadata 与 validation result 呈现合同；验证 source 变化后 失效/re-render，但不新建业务 Artifact、Gate、Runtime、Service、MCP 或 CLI。
9. Host Adapter sub-agent capability/model profile 探测与最小 dispatcher/join：确切 inputs、只读权限、bounded timeout/retry、统一结果和审计；不支持并发/模型选择时如实 `NOT_AVAILABLE / DEGRADED_TO_SEQUENTIAL`，不建设独立多 Agent Runtime。
10. 项目 Git preflight：校验 确切 project root，复用已有 repository/worktree 或在非 Git 项目根静默 `git init -b main`；应用 `.gitignore`/敏感边界，覆盖失败降级与并行 branch + worktree/diff review，不自动 add/commit/push/remote。
11. 最小 Resume：读取 state、验证 确切 引用/files/当前 versions、检查 external result 与 branch/worktree 变化、渲染直白 Resume Brief 并从 当前 step 继续；实质性 变化先走 失效/Impact，不通过全量会话重放恢复。

### Slice 1：一个 Idea → 一份 PRD → 本地 Handoff

- **目标**：证明一个真实 Idea 能从保真信号经过发现、决策、规划和 PRD 审查，形成可恢复、可审计的本地交付物。
- **必须跑通**：下文的完整主链、五种产品决定、Problem/Plan/PRD Ready 边界、十一个入口意图、文档体验、产品记忆投影和本地 Handoff。
- **对抗验证**：用错误版本、失效 Review、无权状态写入、隐藏分歧、重复人类确认、越权外部写入和 Reviewer 自报 Ready 等输入证明边界真的生效。
- **不在本切片冻结**：Research 内部节点、Roadmap 正式共享合同、结果选择阈值、未来无人值守领域治理和下游 Ready。
...
该案例进入 Discovery 后，先以不可变 Signal 和确切 Knowledge Snapshot 形成 `problem.evidence.map.v1`，由 `problem.assumption.audit` 生成首个 versioned checkpoint，并在 `READY_FOR_LEARNING` 后至少跑通一次补证据、Map 新版本、`COMPLETED + READY_FOR_SYNTHESIS` 和独立 next-action recommendation。随后调用 `problem.synthesize`，从确切的 Raw Signal、Knowledge/Product Memory、Map、Assumption checkpoint、Learning State/Round Deltas 和分歧中冻结一份可保留 Unknown 的 versioned Problem Definition Candidate；分别验证一次正常 `COMPLETED`，以及一次发现会根本改变方向的实质性缺口后，携新 MVU `RETURN_TO_LEARNING`。进入 Planning 后先形成 Target Operating Outcome、Observable Evidence、Non-sacrificable Guardrails 与 Product Plan v0；Selector 对简单 Case 选择 `LIGHT`，在 `product.planning` 内部 Refinement Loop 一次内联完成必要深化和 global reconciliation，但仍保留目标、证据、关键假设、验收、风险与追溯。`plan.slice` 先提出 end-to-end PRD 候选，`plan.coverage.validate` 证明每项重要内容有明确处置结论，`plan.reconcile` 处置 Finding 并生成当前 Plan Candidate。另用扩大依赖的 delta 验证动态升级到 `STANDARD`、Round checkpoint 和旧版不可覆盖；sub-agent 只交 Proposal、主 Agent 统一整合。达到 §11.1.1 停止条件后，独立只读 Reviewer sub-agents 对确切的 frozen Candidate 做仅提供建议的 Review；局部、结构、Decision 或 Problem Finding 各回正确 repair path，Reviewer 不写 Plan/state。Review finalize 后，Controller 对确切的 Plan、Coverage、处置结论、Review、conflict 和实质性 Decision 引用执行 `plan.ready.gate`，不要求 Owner 阅读或确认整份 Plan；只有新的实质性产品取舍才返回 `product.decision`。PASS 只为当前 activated/eligible slice 创建 PRD Run，未来、实验或等待项分别保留在其去向。已有稳定 Plan 的小改动另测 affected-area review + global impact check。

同一 Slice 用确切 Candidate 验证 Problem Ready 的两类执行边界：独立 Reviewer 在隔离上下文中执行一次审查尝试（attempt），只读审查并产生仅提供建议的 Finding/Verdict；本地程序化 State Controller 只重算当前且 materially valid 的 Candidate、同版本 Review 及处置结论、上游精确引用，输出 `READY` 后无感进入 Product Decision，或输出带 unmet condition + repair target 的 `NOT_READY`。普通 Unknown、关注等级与未采纳建议可透明携带；Review 或引用失效、Finding 没有处置结论时，才按机械合同返回修复点。固定 `problem.owner.confirm`、`PM_ACKNOWLEDGED` 与 `OWNER_CONFIRMED` 不存在；Decision Brief 同屏展示确切 Problem，Owner outcome choice 承担唯一固定责任。Reviewer/Agent 自报 Ready、修改 Candidate 后复用旧 Review、`NO_PM_INTERVIEW` 跳过 Decision 或模拟结果直接 Handoff 都被拒绝。

随后用同一个 Decision Run 验证 `product.decision` 可在 AI Brief 后暂停并从 确切 Draft/checkpoint 恢复；低风险且未知较少时不 fan-out 全量 Reviewer，高风险/专业未知命中时只调度必要 bounded sub-agent。Agent 必须给首选建议，Owner 明确选择，Controller 才能基于 确切 Decision Record 写 route。五个内部动作不得出现在 Graph Manifest 的节点清单中；Draft、Reviewer 或 Agent recommendation 不能冒充最终 Decision/route。分别渲染五种 outcome 和三种 `planning_activation`，确认推荐层遵守 §10.2.1 信息预算，Owner choice 只需通用五项且由系统填充 identity/确切 引用/version/audit。再让 Owner 选择 materially different outcome，验证 Agent 只做一次聚焦 evidence gap/risk/preference reason 的实质挑战，而后由有权 Owner 决定；Record 条件保存双方理由、authorization、accepted uncertainty/risk、recheck/stop 与 execution constraints。逐一验证 chosen outcome 只生成对应 `outcome_details`，未选项无空字段；正式 Record 不可覆盖，改判生成 取代旧版的 version。再让同一问题分别拟议内部原型、小流量实验与全量执行，验证风险等级随 action/exposure 改变；STOP/WAIT 不被迫展示风险，EXPERIMENT/COMMIT 必有判断，`RISK_PENDING` 不默认为低风险，R3 仍可写 Plan/PRD，但专业 concern 与执行约束必须披露给外置审核。最后逐项破坏 Owner choice、outcome field、risk/constraint、upstream 引用 和 disagreement record，确认 Controller 留在当前 Decision 并返回具体 repair target；修复后分别路由 STOP/WAIT/RESEARCH/EXPERIMENT/COMMIT 三种 activation，且 Manifest 中没有 `decision.ready`。另用重大风险 Case 验证一屏预算不会隐藏安全、合规或不可逆风险。本 Slice 不冻结 Research 内部节点、Roadmap 正式 合同、结果选择阈值、未来无人值守 Domain governance 或下游 Ready。

Problem Quality Review 首轮覆盖完整 Candidate 与全部审查维度；分别构造方案偷渡、单例外推、因果误判、隐藏 Unknown 和 action 不匹配 Finding，验证四类 repair path 不直接写状态。修复后第二轮只重点读取 实质性 delta/未解决 Finding，但重新检查全局不变量；用连续纯措辞修订验证 `NO_PROGRESS` 返回 Learning/Owner，而不是无限循环。默认 Junior PM 视图只先展示关键 Finding、原因、影响和首选修复建议。若宿主可用，Quality Review 作为 `BEST_AVAILABLE` sub-agent 执行尝试；不可用则如实记录降级，不能伪造并发或把外部 Claude Connector 当内部 sub-agent。

同一 Slice 用 `decision / prd / handoff / audit` 四个首批 Profile 跑通端到端视图链：结构化 Artifact → renderer → Validator → 按需 Readability Reviewer → 原有 Ready/Handoff。至少验证字段齐全但术语不可读、文本流畅却隐藏 unknown、Owner 授权被写成用户事实、旧 source 视图 失效，以及“包已生成”不能呈现为“下游已接收”。
...
- **目标**：证明一份完整规划可以拆成多个状态、节奏和交付彼此独立的 PRD Runs。
- **必须跑通**：`STANDARD / PROJECT_SCALE` 两档、四个规划视图、Slice/Coverage/Reconcile、激活与 eligibility 计算，以及三份不同状态的 PRD Run。
- **对抗验证**：检查矩阵机械切格、技术分层式 PRD、遗漏或重复、父 Wave 锁死无冲突并发、未来切片提前建 Run 和等待项被伪装成已激活。
- **不在本切片冻结**：Planning Profile 的精确阈值、Module/Iteration Map 的最终物理表达和 Batch 的完整实现细节，仍由真实原型校准。
...
- **目标**：证明带解决方案的单条反馈不会被直接复制成 PRD，而会先形成有证据边界的问题理解。
- **必须跑通**：G04、问题假设审视、至少两轮 collect/map、七类来源路由、PM 六步共同判断、跳过/恢复访谈和三种 Learning completion 处置结论。
- **对抗验证**：检查同源重复冒充代表性、PM 转述或授权冒充用户事实、批量问卷、诱导脚手架、无限挑战、无来源 Unknown 被静默消除和 skip 绕过正式边界。
- **不在本切片冻结**：G04 的轮次/问题数只是 Case 参数，不成为全局 Gate；正式 KMG 提交合同和各类来源的最终阈值仍待原型。
...
同一 Case 验证普通补证不会重复运行问题假设审视；只有发生实质性证据变化或 fundamental reframe，才生成带 `supersedes` 的新 checkpoint。节点全程默认不问 PM；若 PM 是最佳信息来源，只把一项明确请求交给后续 Learning Loop。完成只代表“可信认知起点 + 下一信息动作”，不输出最终本质问题、Problem Definition/Decision，不写正式知识，也不调用 Reviewer/Evaluator Loop 或独立 Gate；Sponsor 授权与用户价值证据分开，可建议后续受限实验但不能硬阻塞。

该 Slice 至少跑通两轮 `collect → map → update assumptions/MVU`，并覆盖七类 `source_resolution_type` 的代表样本：AI 能自查/研究的信息不反问 PM，专业事实找 Owner，用户事实找数据/用户；只有 PM 独有背景、价值判断或正式授权才打断。打断前给出当前判断、证据边界、为什么问、首选建议和答案影响；PM 不知道时保留 Unknown 并生成包含决策影响、正确来源、有效证据、答案分支和停止条件的 Evidence Request。每轮只围绕一个 MVU，可批量询问少量紧密相关问题，但不设全局硬数量。分别验证低风险可逆实验可以在弱但透明的证据下被建议，而高风险/不可逆动作需要更强证据；Sponsor 授权与用户价值证据分离，可用缩小范围、指标、kill criteria 和回滚塑造建议。继续问答信息价值较低时分别验证 `READY_FOR_SYNTHESIS`、`ROUTE_REEVALUATION_RECOMMENDED` 与 `INSUFFICIENT_TO_PROCEED`，并独立记录研究/实验/恢复等 next-action recommendation；建议不能自动创建 Research/Experiment。稳定新事实只在本地保留为带 确切 provenance 的 future Knowledge source candidate，不能由该 Run 直接改 正式 Snapshot；如何提交等待 Knowledge Requirements 反推。
...
- **目标**：证明实验是同一产品流程中的受控交付意图，而不是绕过证据、风险和 Ready 的快捷通道。
- **必须跑通**：G01 的条件式可接受结果、同一 Planning→PRD→Eval→Review→Ready→Release→可选发送链，以及结果回到 Product Decision。
- **对抗验证**：检查缺测量/回滚仍 Ready、高风险自动降为 LIGHT、Reviewer concern 冒充正式阻塞、实验结果自动改 Decision，以及另建 Experiment Plan/Ready/Handoff。
- **不在本切片冻结**：并行 Experiment Portfolio 不作为一期 Gate，只保留未来由真实规模触发的原型问题。

使用独立 G01 Case（老板拍板做 AI 自动回复、现有信号仅含三条销售询问）验证：在不同风险、可逆性、测量和授权条件下，可以得到多个符合条件的结果；被测 Agent 看不到 expected envelope / rubric。`EXPERIMENT` 必须与 `COMMIT` 跑同一 Product Planning→PRD→Eval→Review→`prd.ready.gate`→self-contained release→optional dispatch pipeline，只通过 delivery intent 与条件化实验 section 表达非长期承诺、关键未知、exposure、测量、continue/adjust/stop、Guardrail 和 rollback。低风险简单 Case 可 LIGHT，但高风险/不可逆 Case 必须升档；缺测量或回滚不能 Ready。下游 typed result 统一进入 `signal.ingest`、绑定确切的 Decision/PRD/Run、形成 Evidence 并返回 Product Decision；expand/iterate/stop/inconclusive 不能自动改 Decision。验证 AI concern 或旧 `BLOCK_RECOMMENDED` 只能形成仅提供建议的 Finding 与外置审核关注，不能自行写 formal Block/approval/waiver；机械合同缺口仍由 Validator/Ready 独立发现。并行 Portfolio 不作为本 Slice 一期 Gate，只保留未来真实规模触发的原型问题。

### Slice 5：Incident 轻量交接与 Bug Baseline/Fix Quick Path

- **目标**：让持续伤害信号快速进入研发核查，同时保证疑似 Bug 先依据当前产品基线分清本质。
- **必须跑通**：G03、三种 Bug cause class、五项自动分流条件、Bug Fix Brief、非确定性 Bug Eval Pack，以及 Incident Packet v1→v2/v3 的轻量交接和回传。
- **对抗验证**：检查 Junior PM“直接修”覆盖基线、surface tag 决定本质、无可靠基线走快线、非关键材料阻塞严重 Incident、提单冒充回滚授权和 Bug 文档膨胀成 PRD。
- **不在本切片冻结**：跨团队真实 TTL、字段细节和专业外部动作权限仍需 Connector/组织原型；默认 Incident 不因此进入重型 Review/Ready。
...
Implementation Deviation 只生成 `bug.fix.brief.v1`、五项轻量检查和 Engineering Handoff，不创建 Product Plan、PRD 或默认 Reviewer Loop；Product Logic Defect 生成取代旧版的 Decision 和带版本的新变更 PRD；Spec Ambiguity 返回 Discovery 或 Product Decision。另需验证：Junior PM 要求“直接修”不能覆盖“当前基线可靠、预期与实际明确、不创建新规则、AC 可判定、没有会改变分流的实质性冲突”五项条件。条件齐全时零确认自动分流，不齐全时只作一次能改变路线的最小澄清。确定性 Bug 可标记 `NOT_NEEDED`，非确定性 AI、推荐、搜索或排序 Bug 生成 Bug Eval Pack。Case Runner 必须隔离预期结果范围与检查准则。
...
- **目标**：证明新知识和下游结果只让真正受影响的对象回到最早正确修复点，不覆盖历史或全链重跑。
- **必须跑通**：Impact List、Ready 失效、统一 `signal.ingest` 回传、状态/result/新事实分流、一次 取代旧版的 Decision 和一次 取代旧版的 PRD release。
- **对抗验证**：检查 Connector/提交者直接 reopen 上游、普通意见触发全链重跑、Audit 代替产品语义、旧版本被覆盖，以及本地记录冒充 KMG 已发布。
- **不在本切片冻结**：更细的领域实质性判断政策、KMG 正式共享政策、提交、确认与 Impact 同步。

验证新知识或 正式 Decision/Roadmap snapshot 影响、PARTIAL/INSUFFICIENT、逐产物 Impact List、`IMPACT_PENDING / REVIEW_REQUIRED`、Ready 失效和定向重跑。再让研发/测试/飞书分别经同一 `signal.ingest` 返回：纯 dispatch status、绑定 Incident/Bug/带实验 intent 的 PRD typed result、无新证据的个人意见、PRD 遗漏、Plan/Slice 冲突、Decision 关键假设反证和独立新机会。断言前两类先更新绑定记录，只有产品新事实才派生关联 Signal；Agent 区分 evidence/result/opinion/duplicate，给影响层、依据/反方与翻转条件，Controller 不让 Connector/提交者直接 reopen 或选择返工点。

返工分别落到 Engineering repair、当前 PRD 的新 Candidate、Planning、Product Decision、Problem Learning、Roadmap 或新 Signal/Run；未改变产品事实的反馈不全链重跑。至少用一次取代旧版的 Decision 和一次取代旧版的 PRD release 验证旧版本不被覆盖，并保留触发原因、Impact 和下游引用。Decision、承诺或 Roadmap 发生实质性变化时，由当前有权 Owner 作出新决定；普通意见只记录并关联，但 Human View 必须说明 Agent 首选建议和升级所需证据。当前 Slice 只要求本地记录与 `local-only` 边界真实可审计，不把 KMG 发布状态设为 Ready 或 Handoff 前置；未来的正式共享政策、提交、确认与 Impact 同步必须在 KMG 合同原型中另行验证。Audit Log 只记录动作而不替代产品语义；本 Slice 不冻结更细的领域实质性判断政策。

### Slice 7：Product Evals 合同

- **目标**：证明产品评测的“是否需要”与“当前做到哪一步”可以独立表达，并与同一 PRD 内容版本收口。
- **必须跑通**：三档 Applicability、五态 Fulfillment、缺输入受阻、Ground Truth/provenance、联合 Review、同源并行 build/join 和按影响范围失效。
- **对抗验证**：检查复杂性自动触发 REQUIRED、旧 `DEFERRED` 隐藏缺口、synthetic data 冒充 Ground Truth、只读 Reviewer 改 Pack、案例数量替代风险覆盖，以及纯排版导致全量重做。
- **不在本切片冻结**：自动化 `evals-generator`、完整 generate Schema、TDD-ready umbrella 和 Test Graph 实现。
...
同一 Slice 还要验证 `prd.content.build` 冻结 确切 stable content 后，`prd.render` 与按需 `evals.build` 可并行并在 join 时绑定同一 source 哈希；语义变化只使受影响 cases/rubric/traceability 失效，纯排版不全量重做。Eval Pack 覆盖 core outcome、normal/boundary/failure/adversarial、input/context/judgment、Rubric、结果处置、Ground Truth、coverage、traceability 和 unresolved gaps；案例数量不能替代风险覆盖。Agent synthetic data 保持 `CANDIDATE/SYNTHETIC`，专业 Ground Truth 未经 Owner/Reviewer 不得升级；只读 Reviewer不能改 Pack，生成 Agent/Optimizer 才能出新版本。此 Slice 不要求自动化 `evals-generator` 已实现，只验证 bounded sub-agent seam，不提前冻结完整 generate 合同；也不要求 TDD-ready umbrella 或 Test Graph 已实现。

### Slice 8：可选 Connector

- **目标**：在出现真实消费者后，用一个真实接入验证稳定挂载合同。
- **必须跑通**：只选择 Input Connector、Claude、Feishu 或 Development Graph 中的一种，验证确切输入/输出、权限、幂等与真实回执。
- **对抗验证**：检查 Connector 绕过 `signal.ingest`、自作产品路由、无授权外部写入、未知结果盲重试和 receipt 被外推为已接收/批准。
- **不在本切片冻结**：不同时实现全部 Connector，也不因一个接入成功声称其他外部能力可用。
...
本节给实现、测试和架构验收人员看。主要结论是：文档合同只能定义验收方法，只有对安装候选和真实 Case 运行后产生的证据才能证明通过。

### 24.0 两类系统验收套件（Suite）的职责与证据门槛
...
以下每条都是原验收条件；主题标题只用于快速定位，不改变强制性。
...
- 构建只复制 source→dist allowlist；没有内部目录镜像/链接、越界 symlink、非 allowlisted inventory 或源码工作区依赖。fresh installed copy 的 `build-manifest.json` 能核对 Plugin SemVer、确切 Git commit/dirty、architecture baseline、execution contract versions/fingerprint、文件 inventory 与 artifact 哈希；源码目录的 identity 不能替代 installed-copy identity。
- 开始/恢复时能从 确切 project root 检测 Git：已有父级 repository/worktree 时复用且没有嵌套 `.git`；非 Git 项目在通过广泛目录与敏感边界检查后静默初始化 `main`。init 后没有自动 staged files、commit、remote 或 push；Git 不可用/只读/失败时按当前交付影响显示 `DEGRADED/BLOCKED`。
#### 状态、恢复与审计

- 每个 Run 的 当前 state 能用直白 Resume Brief 回答事项/目标、当前/last/next、确切 artifact 引用、未解决项/等待/暂停原因、已发生外部副作用/receipt，以及必要的 branch/worktree identity；缺任一会导致重复动作或错误续跑的最小信息时不得声称可安全恢复。
- Resume 必须先解析 确切 引用/files/当前 versions 并检查新 external result 与 branch/worktree 变化；Decision/Plan/PRD 等 实质性 改变后沿旧 next 盲续失败，纯格式变化却全链重跑也失败。Machine enum 可以保留，但只显示裸 code、不说明当前状态和下一步不通过人类可读性验收。
- meaningful event stream 记录 node enter/complete、formal/实质性 checkpoint、Owner 决定/分歧、重要 Finding、side effect/receipt、pause/failure/stop 和 sub-agent dispatch/result/failure。保存 hidden CoT、每次 tool call、无状态内部尝试、逐措辞/autosave/每份草稿 哈希，或为了恢复重放整段对话，均违反轻量审计边界。
...
- `$better-product-graph audit [run_id|signal_id]` 能只读展示原文/claims、分类、证据/policy、推荐与实际路线、澄清、override、reroute、版本/哈希，不返回隐藏 chain-of-thought。
#### 证据与问题

- Discovery 的活动节点名和调用链为 `evidence.collect → evidence.map`；Collect 只保存不可变 Evidence Reference 与 source/time/version/query scope/哈希/permission/sensitivity/freshness/independence provenance，Map 产生 append-only、run-local `problem.evidence.map.v1`，新版本用 `supersedes` 保留完整历史。
...
- `problem.assumption.audit` 在当前 Map 后、Learning Loop 前独立完成并可恢复；默认零 PM 访谈，checkpoint 绑定 确切 inputs/versions/哈希，保真区分用户事实、PM 转述/判断、Sponsor 授权与 Agent inference，拆分现象/影响/问题假设/期望结果/提出方案，记录动态关键假设、反证/历史/no-action/症状原因与可信替代或主动检查未发现，选出 exactly one MVU、推荐信息来源和一项下一信息请求，并保存结构化理由和三类下一建议，不包含 hidden Chain-of-Thought。
- Assumption Audit 的完成只表示形成可信认知起点和下一信息动作，不表示找到最终本质问题；若最佳来源是 PM，本节点只把请求交给后续 Learning Loop。Learning Loop 节点/恢复边界、Evidence Request 等待语义以及 status/处置结论/recommendation 三维合同已确认；人工 override 和全部 Schema 字段不因此被冻结。
- `RETURN_TO_EVIDENCE` 回到 collect 或 map，`READY_FOR_LEARNING` 只授权进入当前 Learning Loop，`ROUTE_REEVALUATION_RECOMMENDED` 只触发受审计的路线重评；Incident 和已确认的 Implementation Deviation 不运行该节点，普通补证不重跑，只有输入发生实质性变化或问题框架发生根本变化时，才创建取代旧版的检查点。
- 每版 Map 有一个明确 MVU、决策影响、可能答案及各自 next action、最佳来源和采集成本；每轮从七类 `source_resolution_type` 选择一类或记录 取代，知识/历史/数据/可授权外部资料由 AI 自查，专业事实找 Owner，用户事实找数据/用户。只有 `PM_CONTEXT_REQUIRED / PM_JUDGMENT_REQUIRED / PM_AUTHORIZATION_REQUIRED` 才默认打断 PM。
...
- `problem.synthesize` 只接受确切 Learning=`COMPLETED + READY_FOR_SYNTHESIS`，并从 确切 Raw Signal、Knowledge/Product Memory snapshots、Problem Evidence Map、Assumption checkpoint、Learning State/Round Deltas 与分歧记录生成 versioned Problem Definition Candidate。Candidate 能保留 Unknown，并清楚表达用户/场景/目标/阻碍/影响/期望改变、Evidence boundary、Assumption/Unknown、scope/non-problems 和用户方案与问题的关系；完成只表示稳定可审候选，不等于 Problem Ready。
- Synthesis 默认不继续搜索、完整访谈、补造 Evidence，或选择是否做、何时做和具体方案。小的未知项保留；发现会根本改变方向的实质性缺口时，必须带新 MVU `RETURN_TO_LEARNING`。来源版本或哈希变化后，旧 Candidate 标为失效，新候选明确记录取代关系；后续 Quality Review、Problem Ready 与 Product Decision 只能引用确切 Candidate，不得读取 `latest/current`。
- Problem Quality Reviewer 以独立 执行尝试 和隔离上下文只读 确切 Candidate/Evidence，使用专门 review Skill 形成 versioned Finding/Verdict；同模型可以担任 Reviewer，但不得继承生成会话、自行编辑 Candidate、代签 Owner 或写 Run State。外部 Reviewer Connector 未配置时内部 Reviewer 仍能运行；需要专业事实时转 Domain Owner，而不是冒充研发/测试 Graph 或专业 Owner。
- Problem Quality Review 首次完整检查用户、场景、目标、阻碍、影响、方案偷渡、Evidence 追溯、范围外推、因果、反证与替代、未知项和行动相关性；每条 Finding 绑定 Evidence、受影响和仍允许的行动、修复条件与四类修复路径。后续 Review 聚焦实质性变化和未解决 Finding，同时回归全局不变量；连续只改措辞而没有实质性进展时，返回 Learning 或 Owner。Junior PM 默认先看到关键 Finding、原因、影响与首选修复建议。
- 对抗审查、同 snapshot 可并发 Reviewer、独立研究/Eval/Analysis 候选优先以 bounded sub-agent 执行尝试 执行；主 Agent 只冻结 确切 inputs、编排、join、保留分歧并提交 transition request。每个 执行尝试 使用最小权限与统一结果；关键审计可请求 `BEST_AVAILABLE`，但实际模型/provider/version 由 Host 映射并审计。Host 不支持时明确 `NOT_AVAILABLE / DEGRADED_TO_SEQUENTIAL`，不伪造并发能力。
- sub-agent required 分支失败只让相关 action 未满足，optional 分支可 `NOT_AVAILABLE` 而不阻塞无关路径；join 不以多数票消除冲突，也不因多个 Agent 同意提高 Evidence confidence。任何 sub-agent 都不能写 state、当前指针、正式知识或 released artifact，不能执行外部副作用，也不能代替 Human Owner、Deterministic Gate、Connector approval；外部 Claude 是 Connector 而非内部 sub-agent。
#### 产品决策与产品记忆

- 一期项目配置/Host identity 仍识别当前操作者为有权 Product/Decision Owner，但固定 `problem.owner.confirm` 已移除。Decision Brief 同屏展示 确切 Problem Definition；Owner 选择 STOP/WAIT/RESEARCH/EXPERIMENT/COMMIT 就表示以该版本作为决策依据，不再弹“是否确认问题”。若 Owner 指出用户、场景、目标、阻碍、结果或范围 实质性 错误，必须返回 Synthesis/Learning 形成新 Candidate；不能在 Decision 中静默改写。
- `problem.ready.gate` 由 Plugin 内程序化 Deterministic State Controller 只重算三类条件：当前且 materially valid 的确切 Candidate；同版本、仅提供建议的 Quality Review 已完成，且 Finding 处置结论完整；上游 Evidence/Learning/Synthesis 引用、state、version 和哈希可解析且一致。Reviewer 的关注等级或未采纳 concern 不阻塞；只有既定机械合同缺口才 `NOT_READY`。
- Gate 只输出 `READY`（自动进入 Product Decision）或 `NOT_READY`（确切 unmet condition + deterministic repair target），不打分、不增加确认/UI。普通 Unknown/证据不足可透明携带，由 Product Decision 选择 Research/Experiment/Wait/Stop/Commit；Problem Ready 不绑定尚未选择的 action，也不检查 PRD、研发、上线或发布 Ready。Product Decision 是正常 Discovery/Product 路径唯一固定的人类语义责任点，且明确自然语言 choice 不再追加确认 UI。
- `product.decision` 作为一个独立可恢复节点运行；AI Brief、按需 adversarial/domain Review、Owner 讨论/挑战、Owner choice 和 deterministic route 都是内部能力而非五个 Graph Nodes。Draft/checkpoint 可跨会话恢复但不产生正式决定；Agent 给首选建议，实质性 时才补最强反方；Owner 明确选择后才生成 final Decision Record，Controller 才写 route。普通场景不因架构存在 Review 能力而全量 fan-out。五种 outcome 和三种 activation 的 human view 均有中文结论句、why、判断边界、next consequence/action 与 change condition，machine code 只作可追溯对照。
- Agent 能从确切 Draft 预填 Decision Record；Owner 默认只确认已选决定、适用范围、最多三个关键理由、最大未知项与翻转、停止或重启条件，以及下一步与检查点或触发条件。身份、版本、时间戳、确切的 Problem、Evidence、Knowledge 和 Review 引用、建议与结果、取代关系、下游和理由与 Audit 引用由系统自动维护；只有建议与选择存在实质性分歧时，才额外显示分歧与 Owner 理由。
...
- STOP、WAIT 与未来承诺严格分开：STOP 结束当前主动方向，WAIT 必须有 review window/trigger 且不承诺，已经决定未来做必须是 `COMMIT + SCHEDULED / CONDITION_TRIGGERED`。STOP/WAIT 进入 immutable Ledger；只有新信息命中 restart/recheck、关键假设或 实质性 risk/opportunity 才主动重审。维持只追加 review result，改判新建 `supersedes` Record；不得删除历史、无期限 WAIT、自动推翻或全量周期提醒。
...
- `new/resume interaction=no-pm-interview` 可在启动/恢复时设置；访谈中 `$better-product-graph interview skip [run_id]` 立即停止当前未回答和后续 PM 访谈，`interview resume` 或 `interaction=guided` 从最高价值未解决 PM-only unknown 恢复。唯一公开 Skill 现在有十一个 intent words；skip 不表示 Evidence 足够、不能跳过 Decision/Ready/外部授权，并必须记录 skipped-interview impact。完整 Discovery 默认至少一次实质访谈/等价当前对话，除非无 实质性 PM-only unknown 且继续提问信息增益低，或用户显式 skip；`NON_INTERACTIVE` 仍 unsupported。
- 低风险可逆实验可在证据较弱但边界透明时推进，高风险/不可逆动作需要更强证据。Run-local Map 中的稳定新事实只保留为带 确切 provenance 的 future Knowledge source candidate；在 Knowledge Maintenance Graph 发布前不成为 正式 Knowledge Snapshot，具体提交合同仍延后。
...
- `IMPLEMENTATION_DEVIATION` 只生成 `bug.fix.brief.v1` 和轻量 Engineering Handoff，不生成 Plan/PRD/默认 Reviewer Loop；`PRODUCT_LOGIC_DEFECT` 生成 取代旧版的 Decision 与新 versioned change PRD；`SPEC_AMBIGUITY` 返回 Discovery/Product Decision。严重/持续伤害先 Incident。
- Bug Fix Brief 有确切 baseline 引用、expected/actual、恢复边界、non-goals、AC、回归面、测试/Eval、依赖/风险/Owner/目标；来自 Incident Packet 的重复事实只引用/追加视图。确定性 Bug 可标记 `NOT_NEEDED`，非确定性 AI/推荐/搜索/排序 Bug 有 Bug Eval Pack。
...
- 每个产品决定至少形成符合 §10.3.1 与 §10.3.3 的紧凑 Decision Record；存在实质性分歧时另按 §10.3.2 处理。Owner 确认后，Record 不可变且带版本；任何实质性改判都以新版本取代旧版。重大或跨产物决定再按是否会改变决定及风险，扩展备选、Review、Roadmap、Product Changelog Proposal 和 Impact List；微小决定不会被迫走同等重量流程。
- 实质性 Agent–Owner disagreement 中，Agent 先给首选并进行一次 evidence/risk-based challenge；有权 Owner 仍可选择不同 outcome。Record 保留双方理由、authorization、accepted uncertainty/risk、recheck/stop 与 execution constraints，但授权不改变 epistemic confidence。可逆可测且证据弱时 Agent 优先建议 EXPERIMENT；低置信 COMMIT 不得被渲染成已验证，R3 约束持续传递到专业执行 Gate。
- Decision 节点结束不经过第二个 Ready 流程：State Controller 对 确切 confirmation/Record、五种 outcome、通用/条件字段、必要 risk/constraints、当前 upstream/history 引用 和 disagreement/accepted risk 重算。满足时按 STOP/WAIT/RESEARCH/EXPERIMENT/COMMIT 确定性路由；不满足时保持当前 Decision，并返回大白话 unmet condition + repair target，不打分、不重复 Owner 确认。
- Decision Ledger、Roadmap Registry、Product Changelog 与 Audit Log 能分别回答决定依据、承诺阶段、产品意义变化和执行事实；任一项缺失时不能用另一项推断替代。
- 每个正式 Decision（含 `STOP`）都进入 append-only/versioned Ledger；Roadmap 只收未来行动。新 STOP 通常不建 Item、WAIT 不承诺、RESEARCH 默认进入 Research Request、EXPERIMENT 激活同一 Product Pipeline但不是 committed product、COMMIT+NOW 仅在真实 Plan Run 后 `in_progress`，非 NOW 只 `committed`。一期没有 Experiment Portfolio 真源。Product Changelog 只覆盖 实质性 产品意义变化并可追溯 source 引用，Audit 自动保留 actor/version/action/state/receipt。
- KMG 未接入时，本地 Decision/Roadmap/Changelog/Audit records 足以完成 BPG 本地 Run，且不会伪称团队已共享/正式 发布；未来接入后，KMG 只负责共享、治理和发布，不重新替 Owner 选择 outcome。具体提交、确认和影响同步合同仍待 Knowledge Requirements 反推。
- 新 Signal 继续从 Router 的四个去向中选择，同时以 `existing_links` 绑定确切的历史 Decision、Roadmap 和 PRD。支持性 Evidence 或不影响既有决定的变化只追加链接或摘要；挑战关键假设时用一屏提醒并给首选建议；命中停止、复查或实质性风险条件时，只约束确切的受影响行动。旧 Decision 不可覆盖，改判时创建取代旧版的新 Decision；Impact List 逐项区分 `UNAFFECTED / REVIEW_REQUIRED / INVALIDATED / PAUSED / WAITING_DECISION`。
#### Golden Cases
...
- Product Planning 不从 v0 直接切 PRD：高价值、高风险或高依赖部分先逐块深化，每轮回到 Target Operating Outcome 做 global reconciliation；实质性变化形成 immutable version/checkpoint、changelog、取代关系和 impact。并行 sub-agents 只读同一份确切快照并返回 Findings/Proposals，由主 Agent 保留分歧后统一整合；已有稳定 Plan 的小改动只 deep-review affected area 并做 global impact check。
- Planning Refinement 在可恢复 `product.planning` 内部运行并产生 versioned checkpoints/stable Plan Candidate；formal Review–Optimize 只审 确切 frozen Candidate，独立 Reviewer sub-agents 只写 Finding/Verdict，Optimizer/主 Agent 才能生成新 Candidate。局部表达问题定向修复/复审，结构问题回最近 Refinement checkpoint/global reconcile，Decision/Problem 失效回对应上游；Review 已完成/不可用已披露、Finding 均有 处置结论 并完成确定性收尾后才进入既有 Plan Ready。探索期 partial review 永远 建议性；`product-plan / prd` 复用同一 engine 合同但使用各自 profile/rubric，实验 intent 只条件化 PRD rubric。
...
- `plan.slice` 从 确切 Module/Iteration/Dependency 视角先提出 PRD Candidate Slice Map/List；候选含目标/问题、用户结果、阶段、模块、依赖、验证和拆分理由。端到端结果可以跨模块，Matrix 不按格生成 PRD，前端/后端/API 不被包装成独立产品 PRD；Agent 先建议，只在 实质性 切分取舍时询问 PM。
- `plan.coverage.validate` 能区分遗漏、重复/冲突、端到端体验断层、依赖和上游一致性，并让本轮、后续、Experiment、等待、不做和 unresolved 都有显式 处置结论。LIGHT 内联、STANDARD 简表、PROJECT_SCALE 可用只读 sub-agent；Finding 只诊断，不静默改 Plan或自动 BLOCK。
...
- 固定 `plan.owner.confirm` 已移除。一页 Plan 摘要按需同源渲染，不是审批；忠实模块、迭代、依赖和切片展开由 Agent 自动完成。只有超出确切 Decision 的实质性产品取舍才从 Planning 返回同一 `product.decision`。程序化 `plan.ready.gate` 只重算当前的确切 Plan、Coverage/处置结论、仅提供建议的 Review finalize、依赖或冲突和所有实质性 Decision 引用，并对 unmet condition 给出 deterministic repair target；PASS 只为当前 activated+eligible slices 创建 PRD Runs。
#### PRD、评测、审查与就绪

- PRD Run 启动时，后台 `prd.workspace.initialize` 只创建 workspace、绑定 确切 引用、登记状态/版本并准备 `archived/released`；Graph/human flow 不把它渲染为“程序创建/编写 PRD”的智能节点。一个可恢复 `prd.generate` Agent 节点通常在同一 执行尝试 连续执行 `prd.content.build → template.resolve → prd.render` 并把首个 versioned candidate 写入 `archived/`。Review subagents 只读审查，Optimizer/Agent 修订，Controller 只做版本、证据与发布状态检查；Owner 只处理 实质性 上游判断、专业授权或 Connector 副作用权限，不固定确认每份 PRD。
- `template.resolve` 能按“项目显式配置 > 受信项目知识 > BPG 配置的当前 fallback/default”确定 确切 Template Profile；可确定时不问 PM，只有冲突且有效性不可判定时一次询问。当前 fallback 精确解析 `references/upstream-skills/better-product-plan/references/product-prd-template.md`；general v0.1 仍是 Draft/Bootstrap 候选。无论选择哪份模板，都验证父 Plan/Slice/Decision 引用、当前增量贡献、Confirmed/Assumption/Unknown/Verify、Shared Contract/跨 PRD 依赖、Evals 引用 和版本/Review/release 状态的 mapping 或约定扩展。
- 模板缺字段时能进入 Profile 扩展/附录或明确 `TEMPLATE_INCOMPATIBLE`，不静默丢失；Agent 不为填模板编造 OKR、埋点 ID、性能指标或 Owner，会改变方案的缺口返回 Planning/Decision。领域无关的 general 模板何时成为默认，由未来带版本的配置与 Roadmap 标准决定；当前不冻结固定人工 Review Gate。项目专属 Futu、moomoo、象象银行、券商、投教或监管检查项不得被硬编码为通用必填；frontend、backend 或 service Profiles 不属于一期验收。
...
- PRD Candidate 的首轮 Product Review 能逐项证明其忠实于 确切 Decision/Plan/Slice/Knowledge/Evidence/constraints：发现 scope creep、future 偷带、新产品规则、虚构事实/指标/OKR/Ground Truth、Assumption/Unknown 升级、Guardrail/依赖/共享合同/回滚/Eval 遗漏、上游冲突或无依据扩展用户/市场/平台时形成 Finding 和最早 repair path。非 实质性 清晰化及已确认规则的场景/分支/异常/AC 展开可在当前 PRD 修复；实质性 新内容必须删除/标 Proposal 并返回 Plan/Slice/Decision/Learning。
...
- `review.aggregate` 能从部分到达处恢复，但只有 required 首轮齐全/明确失败、所有有效结果绑定同一 frozen Candidate/Goal Packet、每个原 Finding 都有无损处置时才可完成。late/失效 不混入新版，disagreement/unsupported lead 不被静默删除；输出复用 Review Record/review_summary/Finding/Verdict/Disposition，不生成新 Candidate 或 Ready。
- `review.finalize` 对相同的确切 Aggregate、Candidate 和规则必须产生同一审查收尾结果：适用的 Review 审查尝试已完成，或其不可用状态已披露；每条 Finding 都有处置结论；所有引用绑定当前 Candidate；同源内审意见版本一致。它不裁决专业观点、不要求 Reviewer PASS、不因关注等级阻塞，也不把外置审核写成已通过；主 Agent 已采纳但尚未完成的当前 PRD 修改先进入 Optimize，上游问题返回最早修复点，其余透明关注随同 release。
- `prd.optimize` 只接受绑定确切 Candidate 且允许在当前 PRD 修复的 Aggregate Findings；一轮只产生一个批量修订 Candidate，可回放 Finding 到变更的映射、未修改项与理由、失效与复审范围，且只有后续 Reviewer 能正式确认修复状态。无内容变化的新 Reviewer 结果或复核只新增 Review Attempt，不增加 PRD 文件；轮次按“批量 Findings → 一个 Candidate → 一次复审”计数，到达上限或连续两轮没有实质性进展时不自动 PASS，而是保存恢复点并返回正确上游或 WAIT。
- Optimizer 新版用 `FIXED / IMPROVED_BUT_OPEN / PERSISTENT / REGRESSED / INSUFFICIENT_EVIDENCE` 回写旧 Finding；针对变化的复审同时回归目标、范围、Guardrail、Shared Contract、AC 和 Eval。不连续机械重写；连续两轮没有实质性进展时停止，保留关注项与处置结论，并路由到正确上游或交外置团队判断。
...
- `prd.ready.gate` 只有六类一期必要检查：当前的确切 Candidate；适用 Review/`review.finalize` 与同源内审意见绑定同版本且记录完整；Decision/Plan/Slice/Knowledge/Evidence 等关键引用当前有效且未失效；`REQUIRED` Eval Pack 已按 policy 履行；Template/Document Experience/version record/`DOCUMENT_CHANGELOG.md` 机械通过；既定必填合同/Validator 无机械缺口。合法 `N/A+理由` 不因填满率失败；相同的确切输入和 rules 必须得到相同结果。未解决、仅提供建议的 concern 必须披露，但本身不是 NOT_READY 条件。
- `prd.ready.gate` 不读取 `prd.owner.confirm_understanding` 或 `handoff.owner.approve` 作为前置，不重做语义 Review，也不要求外置组织审批、Connector/外部写入授权、派生 export、研发/测试完成、所有建议均已关闭或百分比分数。`NOT_READY` 列出确切的 unmet condition、affected 引用/version/finding、repair target 和 resume point；`READY` 自动生成 immutable self-contained release，该目录即本地 Handoff 单元。按需一页最终摘要绑定同一 source，用户未打开它不阻塞 Ready；BPG Released 不被呈现为外置组织审批通过。
#### 交接与连接器

- 每个 Connector+Target 默认 `manual`，只有范围确切且带版本的 preauthorization 才可 `auto_when_ready`；`disabled` 不发送。Dispatch 幂等 identity 绑定 PRD ID+Release+Connector+Target+Action；UNKNOWN 必须 query/reconcile，不能盲重试。多目标复用同一正式 release，并保留各自独立的发送尝试与回执；没有可验证 receipt 只能说本地完成/可发送/结果未知，不能声称 `SENT/IMPORTED/RECEIVED/ACCEPTED/APPROVED`。
- `artifacts/prds/archived/` 保留 immutable self-contained 实质性 Candidate directories，`artifacts/prds/released/` 保留全部 self-contained 正式版本；每轮 Review/Optimize 的 Candidate、review 引用、delta 和取代关系可回放。Ready 后只能从确切 Candidate 生成 release，Handoff 绑定确切的 directory 及 Markdown/assets 哈希；旧 release 失效或被取代后仍可审计。同 stem DOCX/PDF/ZIP 是按需派生，不是唯一正式真源或 Ready 前置。
#### 文档体验与正式产物

- `product_plan / prd / decision / incident / bug_fix / handoff / audit / internal_review` 八个一期 Profile 能由已有节点调用同一 Renderer/Validator；每个 Human View 头部都有 确切 `source_artifact_ref/hash`、`policy_version`、`profile_id`、`profile_version`、`template_version` 和 `rendered_at`，source 变化后自动 失效 并重渲染。`product_plan` 必须至少稳定渲染 Plan Ready 一页摘要，`internal_review` 必须与 确切 PRD version 同源绑定。
...
以下是对抗性硬失败条件；一旦命中，就不能用其他条目通过来抵消。
...
- 把后台 `prd.workspace.initialize` 与 `prd.generate` 同等呈现为会创作 PRD 的 Agent Node，或 Controller 在初始化/保存/校验时生成、判断、改写 PRD 语义，失败。反过来由 Agent 绕过 Controller 自建目录、覆盖已引用版本、移动 当前/released 指针、自报 Review/Ready 完成或并发写正式产物，也失败；同一 Run 若发生两次初始化或两条不一致状态同样失败。
- 把 `prd.content.build / template.resolve / prd.render` 注册为三个用户可见节点、增加三次 HITL，或让简单 Run 同时生成完整 Product Spec 与内容重复的 PRD，失败。Template Profile 选择跳过更高优先级 确切 配置、可确定时仍询问 PM、未声明必要语义 mapping、无法承载却静默丢字段、为填模板编造 OKR/埋点/性能/Owner、可选 content checkpoint 被单独发布/Handoff，或把项目专属券商/投教 checklist 提升为所有项目通用 Gate，也失败。
...
- `review.finalize` 丢失 Finding/处置结论、把 失效/mismatched Review 绑定当前 Candidate、未生成同源内审意见却声称完成、以 Reviewer concern level/数量阻塞，或把外置审核写成“已批准”，均失败。Finalize 裁决专业观点、修改 Candidate、自报 Ready，或把自己重新实现成 `review.gate`/人工步骤/业务 Node，也失败。
- `prd.optimize` 绕过 Aggregate 处置结论 修上游问题，顺手新增无依据功能/用户/平台/商业模式/范围，用文案润色掩盖结构缺陷，自报 Finding `FIXED`/PASS/Ready，或让 subagents 并发写正式 Candidate，均失败。每条 Finding/微编辑/无内容变化复审都生成 PRD 文件，一轮生成多个 Candidate，把 Review Attempt 复制成 PRD，新建 `working/`，或通过改名/重启绕过 2/3/4 轮和两轮 no-progress，也失败。
- Finding 缺 确切 evidence/upstream commitment/affected scope/repair target，Severity 只有数字或未解释行动影响，`unsupported` 建议被自动写入当前 PRD，CRITICAL Goal/Scope 偏离被多个 LOW PASS 抵消，或冲突在 aggregate 中消失，均失败。
...
- 把 general Template Draft 冒充已经完成质量验证的冻结模板，或一期提前建设 frontend/backend/service 模板库、Template Adapter/Router/新节点，失败。当前 default/fallback 必须由确切且带版本的配置决定，并允许未来升级、pin 与回滚；具体 promotion 流程留 Roadmap。PRD 目录/主文件必须遵守已确认的同 stem 命名模式，但不得把示例中的具体 PRD ID、短标题、日期或 v0.x/v1.x 数值硬编码成唯一合法值。
#### 证据与问题
...
- `problem.assumption.audit` 直接写 Route/State/正式知识、生成 Problem Definition/Product Decision、调用独立 Reviewer/Evaluator Loop/Ready Gate，或把内部 checkpoint 当作正式业务 Artifact/Handoff 时失败。
- Assumption checkpoint 被原位覆盖、缺 确切 input 引用/哈希/structured rationale，普通补证触发无意义全量重跑，实质性 reframe 却复用旧 checkpoint，或 `ROUTE_REEVALUATION_RECOMMENDED` 被当成实际 reroute 而没有新 Route Record 时失败。
...
- PM/Sponsor claim 与证据冲突时静默迎合，把授权升级为 user-value evidence；或反过来由仅提供建议的 Reviewer/Agent 用自己的意见硬阻塞，均失败。受限实验若被建议，必须明确缩小范围、指标、kill criteria 和回滚，不能用“老板批准”省略安全边界。
...
- 把 runtime status、completion 处置结论 和 next-action recommendation 合成一个 Exit 字段，在 `ACTIVE / WAITING / PAUSED` 时写 completion 处置结论，或在 `COMPLETED` 时缺 处置结论，均失败。旧五项候选 Exit 被重新作为活动枚举也失败。
...
- `problem.synthesize` 接受 `ACTIVE / WAITING_FOR_EVIDENCE / PAUSED` 或非 `READY_FOR_SYNTHESIS` 输入，从 `latest/current` 推断 source，继续搜索/完整访谈，补写不存在的 Evidence，删除 Unknown，或自行选择产品行动/方案时失败。Candidate 被冒充 正式 Knowledge、Decision、Plan、PRD、solution、Quality Review 或 Problem Ready 也失败。
- 已发现会改变方向的实质性缺口却仍返回 `COMPLETED`，或 `RETURN_TO_LEARNING` 缺少实质性缺口、exactly one 新 MVU 和结构化原因时失败；返回事件不得直接篡改旧 Learning State 或声称新 Learning Round 已发生。source 变化后继续复用失效 Candidate、原位覆盖旧 Candidate，或下游引用可变的 `current/latest` 而非确切 Candidate 版本与哈希，同样失败。
- Product Quality Reviewer 与生成者共享未隔离会话、读取 `latest/current`、编辑 Candidate、代签 Owner、直接写 State/Ready，或把外部模型名称当作可信保证时失败；同一模型不是失败，但必须有独立 执行尝试、专门 Skill、确切 inputs 和隔离上下文。
...
- 主 Agent 串行重做所有可独立对抗审查却声称使用 sub-agent、并发执行尝试读取不同或可变的 snapshot，或任何 sub-agent 并发写 state、当前指针、正式知识、released artifact 或外部系统时，均失败。sub-agent 也不能替代 Owner、Gate 或 side-effect approval。
...
- BPG 的 Product Decision、Ready/Released 或内部 Review 被呈现为最终组织审批通过，或内部重复实现外置会签/升级流程，均失败。Handoff 必须携带 确切 Decision/Plan/PRD/Review/Ready/实质性 引用 供外置汇总审批使用，但不复制新的确认记录。
- 以 LLM/subagent 的“看起来通过”代替程序化 `problem.ready.gate`，Controller 读取可变指针、同输入/规则版本得到不同状态迁移、未原子写 state/audit，或把主观判断硬编码为无来源布尔值时失败。Gate 重新做语义审查/评分/审批、增加第二次确认，把普通 Unknown 自动阻塞，或提前检查 action/PRD/研发/上线/发布 Ready，也失败。`NOT_READY` 缺 确切 unmet condition/repair target，Agent 模拟原型未标 `ADVISORY_ONLY`，或未经过程序化重算就正式 Handoff，同样失败。
- 把 AI Brief、风险审查、选项比较、挑战、Owner 确认或 route 分别注册为串行 Graph Nodes，或反过来把 `product.decision` 做成无法暂停恢复的一次巨型 Prompt，均失败。默认全量调用 Reviewer、Agent 只列选项不给首选、实质性 反方被隐藏、非 实质性 场景为模板完整强造反方、Draft/Reviewer 替 Owner 选择、Owner choice 未冻结 确切 Decision Record 就路由，或 Agent/subagent 直接写正式 route，也失败。
...
- Owner 被要求手填 identity、版本、确切 引用、supersession、downstream 或 audit 元数据，确认页在无必要时超出通用五项，实质性 Agent/Owner 分歧被隐藏，或无分歧时仍强制生成 disagreement/Owner reason，均不符合 Decision Record 最小合同。默认视图倾倒完整 cognitive report、没有渐进展开，或反过来借“一屏”隐藏重大风险，也失败。
- 实质性 disagreement 时 Agent 没有首选、只问“确定吗”、静默迎合，或在一次实质挑战后仍无限争论/阻塞有权 Owner，均失败。Record 缺双方理由/authorization/accepted uncertainty-risk/recheck-stop，把 sponsor-directed 授权写成高 epistemic confidence/用户价值证据，或为了表达分歧新增 `OVERRIDE / SPONSOR_COMMIT` outcome/node，也失败。R3 constraint 被普通 Product Owner 的坚持清除，或未随 Plan/PRD/Handoff 传递时同样失败。
- 新增 `decision.ready`、Decision Ready Gate/Artifact/Reviewer/重复 Owner confirm，或 Agent 自报完成后绕过 Controller 路由，均失败。transition validation 只返回裸 `NOT_READY`/分数、不列 确切 unmet condition + repair target，失败后错误退出当前 Decision/创建下游 Run，或把 Research path、Roadmap 写入/下游 Ready 当作已在此验证，同样失败。
...
- KMG 未配置就阻止全部本地 Decision/Planning/PRD Run，或把本地 future source candidate 伪装成已共享/正式 receipt，失败。KMG 接入后重新替 Owner 决策、改写原 outcome，或四类视图各存一份可独立修改的事实，同样失败。
- 新 Signal 因命中历史 Decision 而跳过既有 Incident/Bug/Discovery/Inbox destination，或静默改写旧 Decision，失败。普通支持证据频繁打断 Owner、单条 non-实质性 delta 触发全量重跑、实质性 challenge 没有 Agent 首选建议/确切 affected artifacts，或不可豁免专业风险被普通 Owner 放行危险动作，也失败。Owner 在 实质性 challenge 下维持原决定却没有理由、accepted uncertainty/risk 和 recheck/stop condition时，影响处置不完整。
...
- 以“实验可逆”为由忽略测量、范围、kill criteria、伤害护栏或回滚，把 Experiment 当作证据不足逃生口时失败；Experiment recommendation 必须只提供建议，并返回有权 Product Decision。
#### 交互政策与文档体验
...
- 完整 Discovery 因“简单/赶时间/Agent 自信”自动跳过实质访谈，且无法证明无 实质性 PM-only unknown、继续提问信息增益低，也失败；用户显式 skip、当前对话已等价完成、Incident、可靠 Implementation Deviation 与 status/receipt 路径不被强制制造一次访谈。
- Human View 字段齐全但术语堆叠、结论埋藏或不面向声明 audience 时，确定性 Schema 通过不能替代按 Profile 要求的 Readability finding/处置结论；unknown/evidence boundary 被直接省略时 `document.experience.validate` 失败，被流畅文案弱化或误导时 Readability Reviewer 必须提出 finding/condition，不能以“可读”掩盖事实缺口。
- Renderer 把 PM/Owner 授权写成用户事实，或删掉 authorization 与 epistemic confidence 的边界时失败；Readability Reviewer 只能给 finding/建议性/condition，不能自行写 HARD_BLOCK。
- Human View 绑定旧 source 哈希、旧 policy/profile/template，缺 metadata，或 source/依赖变化后仍标有效时，State Controller 必须在原 Ready/Handoff 迁移中判为 `STALE/INVALID` 并要求重渲染；Agent 不能自行宣布通过。
...
- Collect 把摘要、翻译、推断或 Agent 生成内容伪装成原始证据，缺 provenance/哈希/查询范围/权限，或 Map 原位覆盖旧版本时失败。Problem Evidence Map 直接写正式知识、把当前指针当历史真源，或 future Knowledge source candidate 缺确切 Map/Evidence 引用、范围与冲突时失败。
...
- `signal.ingest` 未在同一事务保存原文、来源、权限/敏感性、时间、external ID（如有）和 哈希 时不得产生正式 Signal；接收后的原始 artifact 被 `prepare` 改写时完整性检查失败。
...
- `new` 被解释成产品承诺/外部动作授权、Incident Assess 自动执行止损，或 Bug Baseline 在 当前 baseline、差异、不新增规则、AC、无冲突五项不完整时自动生成 Bug Fix Brief，均失败。
...
- Product Logic Defect 原位覆盖旧 Decision/PRD、未创建 取代旧版的 Decision/change PRD，或 Spec Ambiguity 被强行归为实现 Bug 时失败。
...
- 研发回传脱离原 Packet 另起无来源结论、原位覆盖 v1、缺少 `supersedes`/哈希，或没有把核查结果路由到 Bug Fix/Product Decision/Discovery/Incident Product Response/Close 时失败。
...
- 纯 dispatch/lifecycle 状态被错误派生为 Product Signal，typed result 未先更新其绑定 Incident/Bug/Experiment/Handoff 记录，或 typed result 中的新产品事实没有带 确切 result/upstream 引用 进入关联 Signal 时失败。协议 event kind 可以触发中央确定性快分流，但不得被 Connector 外推为产品业务结论；Router 仍必须只有四个 destination。
...
- 下游回传默认退 PRD或启动新 Product Run、Implementation Deviation 通过改 PRD 处理、上游 Planning/Decision/Problem 错误被 PRD 文案圆回、未改变产品事实的状态/重复信息触发全链重跑，均失败。实质性 PRD 变化必须生成 取代旧版的 Candidate/release；实质性 Decision/承诺/Roadmap 变化必须由有权 Owner 形成新决定，但不得为此新增固定审批 Gate。
#### 就绪、版本与正式产物

- Schema 合法但缺 Reviewer 的 Ready 请求被 State Controller 拒绝。
- 旧版本 verdict 不能用于新 PRD Ready。
- `prd.ready.gate` 对旧的或非当前 Candidate、不同版本 Review/route、失效 upstream 引用、`REQUIRED + BLOCKED_MISSING_INPUT` Eval、缺失 Template/Document Experience/version/changelog 任一情况返回 `READY`，或 `NOT_READY` 只说“请完善”而没有精确引用、repair 和 resume 信息，均失败。Gate 重做产品语义审查、按字段填满率或分数放行，或要求 Owner 二次确认、外置审批、Connector 授权、研发/测试完成或所有建议均已关闭，也失败。
...
- 已保存/引用/分享的 archived Candidate 或附件被原位覆盖，Review 结果未绑定 确切 Candidate+asset 哈希，Optimizer 未生成新 archived version/delta，或把 token/keystroke/autosave 全量塞入 `archived/`，均不符合 PRD 文档生命周期。
- `released/` 只保留最新目录、把 superseded/invalidated/revoked release 移到 archived/或删除、覆盖已引用 Markdown/assets、让 release 依赖 package-root/兄弟目录共享可变 assets、状态变化改写 Changelog 历史行，或 当前 pointer 被当成历史真源，均失败。无附件的简单 PRD 被强制创建空 assets 目录不是硬失败，但不应成为生成器默认。
- 未经 Fidelity/required Review、Ready 和 Document Experience validation 就写入 `released/`，Handoff 通过 `latest/current` 猜版本，未绑定确切的 directory、Markdown/assets 哈希和 source draft，主文件仍叫 `prd.md/final.md/最新版.md`，或把 DOCX/PDF/ZIP 当作唯一正式真源或 Ready 必填，均失败。反过来，继续强制 `prd.owner.confirm_understanding` / `handoff.owner.approve` 才允许 Ready/release，同样违反当前 HITL 简化。`final/published/approved` 目录不能替代规定的 `released/` 语义。
...
- `DOCUMENT_CHANGELOG.md` 被用来替代 Product Changelog、架构 CHANGELOG、上游 Decision/Plan/Slice/Knowledge 或 Audit Ledger，可选 content checkpoint 被用来取代正式 PRD，或 manifest/当前 pointer 创造了文件和追加记录中不存在的事实，均失败。
- 当前一期不以 KMG 提交、确认、正式共享发布或跨团队 Impact 同步作为 Ready 或 Handoff 前置；未接入 KMG 的项目必须以本地记录正常运行并披露边界，也不能伪称已共享发布。未来若项目政策确实要求正式共享，必须先在 KMG 合同中定义确切的行动范围、回执、失败恢复与无关行动隔离，再决定是否形成对应约束。
- PRD（包括实验 intent）的 Feishu/Development Connector 在 Ready 前不能调用；Ready 后也必须按每目标 `disabled/manual/auto_when_ready` policy 执行。默认自动、预授权未覆盖 确切 target/scope 仍发送、外部写权被复用为 PRD 语义审批、已预授权却重复要求固定 Owner 内容确认，或把 BPG Released/Connector Sent 写成组织审批通过，均失败。UNKNOWN 结果盲重试、多个目标共用同一 receipt、Connector 回写 正式 PRD、手工 DOCX 导入无验证却标 `SENT/IMPORTED`，也失败。Incident Verification Packet/Bug Fix Brief 分别通过对应轻量检查、权限与批准后可以交接，但不得伪装成 Product Ready。
- AI/sub-agent Reviewer concern、旧 `BLOCK_RECOMMENDED` 值或 concern level 不能直接写 formal Block、阻止本地 Ready/Released 或获得 waiver authority；它们必须以 建议性/处置结论/内审意见交外置团队。未来无人值守阶段的专业 blocking/policy/waiver 仍只是 Roadmap seam。
...
- 未解决、仅提供建议的 concern 未被同源内审意见透明披露，或外置团队审核尚未完成却写成“无风险/已批准”，失败。

### 24.3 规划完整性

- Product Plan v0 被直接作为 PRD Matrix/子 Run 输入，局部深化没有回到系统最佳运行结果协调，多个局部 PASS 被拼成全局通过，或尚未满足 §11.1.1 停止条件就 Plan Ready，均失败。
- 把 Planning Refinement 注册成独立持久 Node、与 formal Review 合并成主 Agent 自审、让仅提供建议的 partial review 成为 Ready 证据、Reviewer 直接修改 Plan/state，或 formal verdict 没有绑定确切的 frozen Candidate 与哈希，均失败。local omission 被无谓送回 Discovery、系统结构 Finding 只改措辞、失效 Decision/Problem 没有回正确上游、no-progress 仍无限 Optimizer，或为 Product Plan/PRD/实验 intent 复制不兼容 Review–Optimize runtime，也失败。
- 深化发现只能给 v0 找支持、不能拆/并/删/重排或返回 Research/Experiment/Product Decision，实质性 Plan 变化原位覆盖旧版、缺 changelog/取代/impact，或另建可漂移“深化报告”成为第二真源，均失败。
- 并行 sub-agents 读取不同/mutable Plan snapshot、并发写正式 Plan/当前/state、aggregate 抹掉分歧或绕过 branch+worktree/diff review 时失败；已有稳定 Plan 的局部小改动被无理由强制全量重跑也不符合本期原则。
...
- Plan Owner 被迫逐字段打勾、被要求固定确认整份 Plan、相邻确认被复制成独立审批 Node、Agent 冒充 Owner，或按需一页摘要被宣称为外置汇总审批通过，失败。忠实 Planning 的 实质性 新取舍未回 `product.decision`，或 `plan.ready.gate` 做语义产品判断/评分、读取旧 Review、忽略致命冲突/实质性 Decision 引用，只返回裸 NOT_READY 无 repair target，也失败。
...
- 扩流、延期、改指标/成功标准、取消 Guardrail 或转长期承诺会使旧 PRD/release 进入 实质性 Impact/复审；不能靠 Connector 参数绕过新 Decision/版本。
- 下游 typed result 必须经 `signal.ingest` 绑定 确切 Decision/PRD/Run 并成为新 Evidence。expand 不自动 `COMMIT`，iterate 产生新版本，inconclusive 不能解释成软成功，所有处置返回 Product Decision。
- 一期不要求或维护 Experiment Portfolio；若无真实并行干扰证据却预建 Portfolio Schema/Service/Registry/业务真源，失败。

### 24.5 Reviewer 仅提供建议 与外置审核

- 当前期所有 Product/UX/Engineering/Testability/Security/Privacy/Compliance/Domain AI/sub-agent Reviewer 都只能形成仅提供建议的 Finding，不拥有 formal Block、veto、approval 或 waiver authority，也不能冒充外置专业责任人。
- Finding 的人类视图至少给出直白关注事项、关注等级、确切 evidence/引用、可能影响、专业建议和最早返工点；等级只用于排序与外置团队聚焦，不能单凭等级阻止 BPG Ready/Released。
- 每条 Finding 必须有 处置结论：采纳并修复、不采纳并说明理由，或保留供外置团队判断。Reviewer 数量/一致不能提升 Evidence confidence，单个 concern 也不能无限占住 Run。
- `review.finalize` 只验证 Review 执行尝试、Finding/处置结论、确切 Candidate 与同源内审意见的机械完整性；它不是批准或 Gate。未解决 concern 透明披露后可以进入唯一 `prd.ready.gate`。
- `prd.ready.gate` 只因既定产品合同、版本、引用、REQUIRED Evals、Template/Document Experience/changelog 等机械缺口返回 NOT_READY；若 Reviewer 恰好指出同一机械缺口，权威来源仍是 Validator 合同，不是 Reviewer。
...
- `evals.build` 在没有 确切 stable content source/哈希 时不得启动；`prd.render` 与 Eval 分支绑定不同 content versions、join 静默选最新版，或语义变化后继续复用受影响 Pack，均失败。纯排版变化触发全量 Eval 重做也属于错误的 impact 粒度。
- Joint Fidelity Review 必须同时检查 Eval Pack 没有引入 PRD/Decision/Plan 未决定的产品规则、成功标准或 Ground Truth，也没有遗漏上游明确要求的 Eval/Guardrail。发现 实质性 mismatch 时返回 Eval、PRD 或更早上游，不能通过固定 PRD Owner 确认把两者强行对齐。
...
- 取代旧版的 Decision 或 Roadmap snapshot 变化生成逐产物 Impact List，只使受影响的 Plan/PRD（含 delivery intent）/Eval/Ready/Handoff 进入相应状态。
...
- Learning Round 恢复成功率、重复查询/重复 Map 更新率、`WAITING_FOR_EVIDENCE` 按来源的时延与超时处置、Round Delta 可复述率，以及停止后新信息实际改变动作的比例；另记录 status/处置结论/recommendation 混淆率、三类 处置结论 人工修正率、`INSUFFICIENT_TO_PROCEED` restart 成功率和建议被误当授权的次数。
- `problem.synthesize` 的 `COMPLETED / RETURN_TO_LEARNING / FAILED` 分布、实质性 gap 捕获与误判率、Candidate 字段/Unknown/非问题人工修正率、方案泄漏率、确切来源绑定与 失效 检出率、恢复成功率，以及 Candidate 被误当 Problem Ready 或被下游以 `current/latest` 引用的次数。
- Product Quality Reviewer 独立 执行尝试/上下文隔离遵守率、同模型与外部模型 Finding 一致性和人工纠正率、Reviewer 编辑/状态写入越权拦截率；Problem Ready 与 Product Decision 重复人类确认次数应为零；Controller 同输入/规则版本确定性、CAS/原子恢复成功率，以及 `ADVISORY_ONLY` 被误当正式 Gate 的次数。
...
- PRD `review.parallel` 的 Goal Fidelity Review Packet 是否全部绑定同一份确切快照与哈希，目标/范围忠实度 Finding 的漏报、误报和人工纠正率；LIGHT 合并角色是否丢失该逻辑检查，条件 Reviewer 触发与首轮隔离是否符合合同。
...
- `review.finalize` 同 确切 input/rules 的重放一致率、失效/missing/丢 Finding/缺 处置结论 拦截率、同源内审意见版本错配率，以及 finalize 被误实现成 Reviewer approval/Gate 或与 `prd.ready.gate` 重复检查的次数。
- `prd.optimize` 的平均/分 profile 修订轮次、两轮 no-progress 早停率和人工纠正率、finding→change 可追溯率、claimed repair 被 Reviewer 判为 `FIXED/REGRESSED/PERSISTENT` 的分布、当前 PRD repair 误改上游语义的次数，以及每 Run archived Candidate 数/Review Attempt 数和无内容变化误生 PRD 文件次数。
- `prd.ready.gate` 同 确切 input/rules 的重放一致率、六类检查分类与 repair target 人工纠正率、旧 Candidate/Review/route 错配拦截率、REQUIRED Eval/失效 upstream/template-version 缺口漏放率，以及外置审批/Connector/研发测试状态被误当本地 Ready 前提的次数。
- 可并行任务 sub-agent 使用率、关键路径时延改善、主 Agent 重做率、required/optional 超时与降级分布、`BEST_AVAILABLE / BALANCED / FAST` 实际映射和成本、权限越界拦截率、snapshot/哈希 不一致率、join 分歧保留率，以及多 Agent 同意被错误提升为 Evidence confidence 的次数。
- `NO_PM_INTERVIEW` 的 prompt 拦截与绕过检测率、进行中 `interview skip` 生效时延、skipped-interview impact 可理解率、合法继续/等待分布、`interview resume`/`interaction=guided` 恢复成功率，以及是否误跳过 Product Decision/外部写入授权或错误降低风险门槛。另记录完整 Discovery 未访谈的原因分布和“无 实质性 PM-only unknown + 低信息增益”人工纠正率。
...
- 各 Profile 的确定性 validation 失败率、Readability finding/处置结论、首读者正确复述结论/未知/下一步的比例，以及 source 变化后 失效 检出与重渲染时延。
...
- Finding 关注等级与 处置结论 分布、遗留 concern 在同源内审意见中的披露完整率、外置团队采纳/拒绝/补充率，以及 AI concern 被错误当作 BPG formal Block/approval 的次数。
...
> **历史、风险和审计参考层导航：** §§25—30 与附录 A 用于追溯能力来源、历史变化、开放风险、节点审查状态和架构理由。第一次理解产品流程时可以跳过；当前版本状态以文首和 Changelog 为准，运行是否通过以真实实现与验收证据为准。

本节说明哪些既有能力被吸收、哪些边界明确不照搬；它不表示这些 Skill 已经集成或运行通过。
...
| C3 | 角色、冲突和失效证据保留；一期 Waiver Policy 已被 Reviewer 仅提供建议 决策取代，正式 blocking/waiver 仅为未来无人值守治理 seam |
| C4 | Knowledge 读取确切快照并保留本地影响追踪；KMG submission/ack/Impact sync 详细合同按最新决定延后到 Knowledge Requirements 反推后设计 |
| C5 | Incident、Bug、Discovery 三条真实路径 |
| H1 / M6 | 后续决策已把 Product 多文件 Handoff Package 折叠进 self-contained Released artifact set；必要 Supplement 成为同一 release 的扩展/附录并由 Release/State/Audit 引用 追溯。Experiment/Incident/Bug 仍用各自合同 |
...
| M10 | 后续 ADR-077—079 以唯一 `signal.ingest` 承接下游回传：status/result 先更新绑定记录，新产品事实才派生 Signal，并按 确切 Impact 返回最早受影响点；不保留下游专用入口 |
...
本节是历史修改与决策演进索引，用于回答“现行合同怎样形成”；它不与 Changelog 共同扮演当前版本变化记录。V1.5 相对上一版本改了什么，以文首定位和 [架构 Changelog](../../architecture/CHANGELOG.md)为准。
...
2. 冻结最小 source→dist allowlist、relative resource resolution、唯一公开 Skill 与 installed-copy identity 自动检查；安装候选绑定 SemVer、Git commit/dirty、architecture baseline、execution contract versions/fingerprint、inventory 和 artifact 哈希。
...
5. 消除模板 promotion 矛盾：只冻结可配置 profile、确切 version、fallback、pin/rollback；general v0.1 仍为 Draft/Bootstrap 候选，何时 promotion 留 Roadmap，不设当前固定人工 Review Gate。
...
11. **把一期 Reviewer 收敛为仅提供建议**：所有内部 Reviewer 只给 finding、建议和返工点；concern 透明进入同源内审意见，由仍然存在的外置团队作最终审核。正式专业 blocking/policy/waiver 只留未来 seam。
12. **分开 Finding、处置结论 与外置关注**：不按多数票，不删除 Finding；采纳修复、不采纳理由和遗留外置判断各自可追溯，不再建设一期 Waiver 合同。
...
16. **增加 Product Decision & Roadmap Memory**：用不可混用但同源的 Decision Ledger、Roadmap Registry、Product Changelog 和 Audit Log 保存产品上下文；所有 PRD 绑定确切版本引用。BPG 本地 records 支持无 KMG 完整运行，Knowledge Maintenance Graph 负责接入后的团队 shared/正式 发布；V1.3 进一步把 Proposal/Outbox/Impact sync 详细合同延后到 Knowledge Requirements 反推后定义。
...
22. **把统一 Bug Quick PRD 拆成基线判断与差异化分流**：`bug.baseline.assessment.v1` 使用 cause class + surface tags 两维模型；纯实现偏差用无 PRD 的 `bug.fix.brief.v1`，产品逻辑缺陷生成 取代旧版的 Decision/change PRD，规格歧义进入 Discovery/Decision，风险 Reviewer 和 Bug Eval 按需触发。
...
24. **把 Document Experience 从原则升级为 Core 横向执行规则**：不新增 Graph Node/Loop/Artifact/Gate；已有构建/渲染动作按 artifact Profile 解析 versioned Policy 和项目 template，调用共享 Renderer、确定性 Validator 与按需 Readability Reviewer。Human View 绑定确切的 source 与哈希，且可重建，不成为第二真源；一期保证七个真实消费者 Profile，其中 `product_plan` 仅先覆盖 Plan Ready 一页摘要。
25. **确认问题假设审视为独立轻量节点，并收敛其五步内部逻辑**：`problem.assumption.audit` 位于 Evidence Map 后、Learning Loop 前，默认由 AI 在 PM 深访前自助去锚定：还原原话与事实角色，拆分现象/影响/问题假设/期望结果/提出方案，动态识别方向性假设，检查反证/历史/no-action/症状原因与可信替代，最后只选一个 MVU、最佳信息来源和下一信息请求，并保存 versioned run-local checkpoint。它只形成可信认知起点，建议返回证据、进入学习或重新评估路线；不强制制造反方，不做最终问题/产品决定，不发布正式知识，也不增加 Reviewer/Evaluator Loop、Ready Gate 或正式业务 Artifact。
...
27. **增加当前 Run 范围的强制跳过/恢复 PM 访谈能力**：启动/恢复可用 `interaction=no-pm-interview`；访谈中可用唯一公开 Skill 的 `interview skip|resume` 即时变更当前 Run。Controller 在每个 PM prompt 前强制检查，保留已答、记录未答/替代来源/影响。Skip 不表示信息充分，也不跳过 Evidence、Decision、Ready 或外部写权；完整 Discovery 默认至少一次实质访谈/等价对话，除非低信息增益无 实质性 PM-only unknown 或用户显式 skip。
...
29. **把 Learning Exit 收敛为三维合同**：废弃把等待、完成原因与研究/实验建议混为五项候选枚举的设计；Learning State 分开记录五种 runtime status、三种 completion 处置结论与仅提供建议的 next-action recommendation。停止按拟议行动的证据充分性判断；`WAITING_FOR_EVIDENCE` 可恢复且不限 human，`READY_FOR_SYNTHESIS` 不等于统一 Problem Ready，`INSUFFICIENT_TO_PROCEED` 不伪造 Problem Definition/PRD，研究/实验仍由 Product Decision 决定。
...
31. **补齐 PRD Review–Optimize 文档生命周期**：每个 PRD package 固定 `archived/` 实质性 candidates 与保存全部正式版本的 `released/`，根目录使用 append-only `DOCUMENT_CHANGELOG.md` 和可重建导航指针；每轮冻结 candidate/review/delta，Ready 后从 确切 candidate 生成 release，Handoff 只引用 确切 release/哈希。文档呈现修订和产品语义变更分流，released PRD 是当前增量的人类交付合同并绑定 确切 上游真源，内部内容底稿不与其竞争。
32. **确认 Problem Synthesis 为独立轻量收敛节点**：`problem.synthesize` 只在 Learning=`COMPLETED + READY_FOR_SYNTHESIS` 后读取 确切 Discovery inputs，冻结 versioned Problem Definition Candidate；它保留 Evidence/Assumption/Unknown、范围/non-problems 和用户方案关系，不继续搜索、决策或生成方案。实质性 gap 携新 MVU 返回 Learning，source 变化后 失效 并以新版本 取代；`COMPLETED` 只表示稳定可审候选，不表示 Review、PM Confirmation 或 Problem Ready。
33. **确认 Problem Ready 的语义/确定性分工**：内部 Product Quality Reviewer Agent 以独立审查尝试（attempt）和隔离上下文只读确切 Candidate，并输出仅提供建议的 Finding；Plugin 内程序化 State Controller 基于确切 records 和 versioned rules 重算，并作为唯一组件写入 state/audit。固定 Problem Owner confirm 后续被移除，人类责任合并到 Product Decision，见第 36、38、77 项。
34. **确认 Problem Quality Review 节点与横向 Sub-agent Execution Policy**：Quality Reviewer 对确切的 frozen Candidate 运行首次 full、后续 delta-targeted + global-invariant Review，输出 action-scoped Finding/Verdict、四类 repair path，并在没有实质性进展时返回 Learning/Owner。对抗审查、可并发 Reviewer、独立研究/Eval/Analysis 候选优先由 bounded、最小权限 sub-agent 执行；主 Agent 只编排、join 并保留分歧，Host 以 capability/model profile 映射实际能力。sub-agent 不新增业务节点、不写正式状态或外部副作用，也不替代 Owner/Gate/Connector approval。


35. **分开一期 Sub-agent 与未来 Multi-Agent Collaboration**：一期只依赖当前 Host 内部 bounded worker 的执行尝试；未来允许不同 Agent/runtime/Host/provider 协议化分工主策划与审计，但独立性必须来自独立实例、上下文、目标和冻结输入输出审计，不接受同会话换提示词冒充。Multi-Agent 复用确切快照及 role/result/permission/join 合同，经可插拔 Connector/稳定协议接入，不新增业务节点、不硬编码 Claude，也不改变 Owner、Controller 和副作用批准边界。
...
37. **把 Git 收敛为一期 Host Preflight**：开始或恢复时校验确切的 project root，复用已有 repository/worktree；非 Git 项目默认本地 `git init -b main`，但不自动 add/commit/push/remote。`.gitignore`/敏感边界先行，失败如实 `DEGRADED/BLOCKED`；并行 sub-agent 使用独立 branch + worktree，由主 Agent 审核 diff 后整合，只在实质性检查点或冻结版本提交或 tag。Git 不进入业务 Graph、Gate 或审批。
38. **确认轻量 Problem Ready Gate**：State Controller 自动检查当前且 materially valid 的 Candidate、同版本且仅提供建议的 Quality Review 及处置结论，以及一致可解析的上游精确引用。只输出 `READY` 并自动进入 Product Decision，或输出 `NOT_READY` 加确切的机械未满足条件与 repair target；不评分、不做 Owner confirmation、不因仅提供建议的 concern 或普通 Unknown 阻塞，并明确取消 action-scoped Problem Ready matrix。
39. **把 Product Decision 收敛为一个可恢复节点**：`product.decision` 内部编排 AI Decision Brief、按需 bounded adversarial/domain Review、Owner 讨论/挑战与明确选择、Controller deterministic route；原五个 `decision.*` 动作不注册 Graph Node。Draft/checkpoint 只用于跨会话恢复，Owner-confirmed Decision Record/route 才是正式边界；默认不全量 Review，Agent 给首选，实质性 时才外显最强反方。后续第 74—75 项确认 MVU guide 与 STOP/WAIT/future COMMIT，完整领域治理和剩余 实质性判断 仍待 Review。
...
41. **把默认 Decision Human View 收敛为一屏决策**：默认只展示一个直白首选、最多三个关键依据、最多一个 实质性 认知提醒、一个定性判断边界和一个具体下一步；最强反方仅 实质性 时一句，完整 Evidence/选项/认知/风险/历史/Audit 渐进展开。所有层级共用 确切 Draft/Record，Validator 检查信息预算但不以死字数隐藏重大安全、合规或不可逆风险。
42. **确认 Decision Record 通用最小合同**：Owner 只确认 chosen decision、applicability scope、最多三个关键理由、最大 Unknown/改判边界和下一动作；实质性 分歧才显示 Owner reason。系统自动填身份、版本、确切 引用、关系与 audit，确认后 immutable/versioned，改判新版本 取代；Human/Audit View 共用同一 Record。
...
45. **确认 Product Planning 双向深化原则**：先形成可推翻的 Plan v0，再优先逐块深化并每轮回到整体结果协调，稳定后才进行横向模块 × 纵向迭代拆分。实质性 变化用 version/checkpoint/changelog/取代/impact；sub-agent 只交 Proposal、主 Agent 单点整合；小改动 affected-area review。其 `product.planning` 内部 Refinement Loop 形态由第 50 项确认，切片/覆盖/协调由第 52—54 项确认。
46. **确认 Agent–Owner disagreement 合同**：Agent 必须先给首选；Owner 选不同 outcome 时只做一次基于证据缺口、风险和首选理由的 bounded 实质挑战，再由有权 Owner 决定。实质性 分歧条件保存双方理由、authorization、accepted uncertainty/risk、recheck/stop 和 execution constraints；授权不改变置信度，高关注事项必须随产物进入外置审核，不新增 outcome/node。
47. **确认 Product Decision 结束规则**：节点终止复用现有 State Controller transition validation，不新增 `decision.ready`、Gate/Artifact/Reviewer/Owner 二次确认。Controller 重算 确切 Record/confirmation、outcome、字段、risk/constraints、引用 与 disagreement/accepted risk；通过后按五种结果确定性路由，失败留在 Decision 并返回直白 unmet condition + repair target。Research/Roadmap/下游 Ready 细节仍待各自 Review。
48. **确认 Product Memory 四类职责投影**：每个正式 Decision（含 STOP）进入 Ledger；只有未来行动进入 Roadmap，RESEARCH 使用 Research Request，EXPERIMENT 通过同一 Plan/PRD pipeline 表达受控行动而不偷换承诺或创建 Portfolio 真源；Product Changelog 只记录 实质性 产品意义变化；Audit 自动记录执行事实。四者同源引用而非四套流程/真源；KMG 未接入时本地完整运行，未来接入只增加共享/发布且不重新替 Owner 决策，具体 submission/Impact 合同待 Knowledge Requirements 反推。
49. **确认历史 Decision 受新 Evidence 影响规则**：新 Signal 保留既有 Router destination 并通过 `existing_links` 关联 确切 historical artifacts；按 supports、non-实质性 delta、challenge key assumption、hit kill/recheck/实质性 risk 分级，只有 实质性 项主动提醒/逐 action 处置。旧 Decision immutable，改判以新 Decision 取代；Impact List 只标真正受影响对象，不默认全量重跑。冲突合并与跨团队 Impact 实现细节仍待后续 Review。
50. **确认 Planning Refinement 与 formal Review–Optimize 分工**：Refinement 是 `product.planning` 内部可恢复生成、探索、拆解和全局协调 Loop，主 Agent 单点整合并产出 checkpoints/Candidate；formal Review 由独立只读 Reviewer sub-agents 审查确切的 frozen Candidate，Optimizer/主 Agent 按 repair path 生成新版本。局部修复定向复审，结构问题回 Refinement，失效 Decision/Problem 回对应上游；审查尝试完成或不可用情况已披露、Finding 均有处置结论并完成确定性收尾后，才进入 Plan Ready。Plan/PRD 复用同一 engine/profile 合同，实验 intent 只条件化 rubric，不把仅提供建议的 partial review 当作 Ready 证据。
...
52. **确认 `plan.slice` PRD 切片**：同时使用横向模块化与纵向迭代化，从完整 Plan 产生可分别交付/验证/上线/回滚的 PRD Candidate Slice Map/List；不按模块、迭代、矩阵格或前后端机械拆，也不写 PRD 正文。Agent 先提出含目标/结果/阶段/模块/依赖/验证/理由的建议，只在 实质性 合并/拆分取舍时打断 PM。
...
54. **确认 `plan.reconcile` 规划协调**：将 Coverage、局部深化和新 Evidence Finding 放回整体结果，恢复 Plan/Module/Iteration/Slice/Dependency/Roadmap 一致。非决策本质调整可自动形成 checkpoint；实质性 Decision/承诺/风险变化须 Owner确认并写既有账本，可回 Slice/Iteration/Decision/Learning；允许带责任人和复查条件的显式灰度，不建审批 Gate或第二真源。
55. **确认轻量 Plan Ready，随后移除固定 Plan 确认**：程序化 `plan.ready.gate` 只检查当前 Plan、Coverage/处置结论、仅提供建议的 Review finalize、依赖或冲突与实质性 Decision 引用。按需一页摘要不是审批；只有新的实质性产品取舍回到 Product Decision。PASS 仅为当前 activated+eligible slices 创建 PRD Runs。
...
57. **根据 Better-Product-Plan 实模板取消默认双文档**：实际模板和 planning-artifact mapping 已能承载完整 PRD 语义，因此保留 `prd.content.build → prd.render` 的能力分离，不默认持久化完整 Product Spec。简单 Run 同一次 Agent 执行尝试 直接形成 PRD candidate，复杂跨会话/多模板/多 Agent/模板迁移场景才可保存内部 checkpoint。过渡 fallback 使用仓库 确切 path，并补映射父 Plan/Slice/Decision、增量贡献、证据状态、Shared Contract/Evals 和文档生命周期；不兼容模板必须扩展或失败，不能静默丢字段，也不把项目 checklist 通用化；第一版不实现曾讨论过的 `spec.build` 兼容 alias。


58. **把 PRD 生成收敛为单一可恢复节点与 Template Profile**：用户只看到 `prd.generate / 生成 PRD`，内部连续运行 content build、确定性 template resolve 和 Agent render，不增加节点/HITL。模板按项目配置、受信知识、BPG 当前配置的 default/fallback 优先级解析；缺项进入扩展/附录或判不兼容，不编造事实。当前 upstream 模板作为带版本的确切回退模板，领域无关 general Draft 只是 Bootstrap 候选；未来由配置升级、pin、回滚，具体内容与 promotion 另行优化。一期不建 frontend/backend/service 模板库或 Template Router。
...
63. **确认 `review.aggregate / 审查意见汇总` 的轻量可恢复 join 边界**：它保存 reviewer 到达/失败/失效 与高影响复核上下文，主 Agent 聚类、保留分歧/不支持建议并提出最早 repair target，Controller 只检查 required 执行尝试、确切 bindings、Finding 合同与无损追溯后写状态。LIGHT 可同一用户动作自动完成，但仍留恢复记录。Aggregate 不修改/生成 Candidate、不自报 Ready，复用现有 Review Record/Finding/Verdict/Disposition/review_summary，不新增 Artifact/Gate/HITL/第二真源。
64. **退役 `review.gate`，采用轻量 `review.finalize`**：只提供建议的 Reviewer 没有批准或阻塞权，Gate 名会误导。Controller 只检查审查尝试、Finding 处置结论、确切 Candidate 与同源内审意见；不裁决专业观点、不要求 PASS、不因 concern level 阻塞。未完成但获主 Agent 采纳的修订进入 optimize，上游根因返回上游，其余 concern 披露后进入唯一 Ready。


65. **确认 `prd.optimize` 与有界 Review–Optimize 子图**：只对主 Agent采纳且根因位于 当前 PRD 的聚合建议做一轮批量最小必要修订，产生一个新 archived Candidate、delta、finding→change 和 re-review scope；根因在上游时必须返回，不在 PRD 圆回来。Optimizer 只提 claimed repair，独立 Reviewer 才确认 status。Candidate version 与 Review Attempt 分离；LIGHT/STANDARD/PROJECT_SCALE 上限 2/3/4 轮，连续两轮 no-progress 提前停止且不自动 PASS。
66. **确认 `prd.ready.gate / PRD 最终就绪检查` 的一期基线**：它是 Controller 在 Review Finalize 后执行的最终确定性 release boundary，只查当前的确切 Candidate、同版本 Review/finalize/内审意见、当前关键引用、REQUIRED Evals、Template/Document Experience/version/changelog 与既定机械合同。仅提供建议的 concern 需要披露但不阻塞。仅输出 `READY | NOT_READY`；通过后无需 Owner 二次确认，自动生成 self-contained release。
67. **以自包含 Released PRD 取代独立物理 Handoff Package**：`archived/` 与 `released/` 中每个确切版本使用 `<PRD-ID>_<短标题>_v<版本>_<日期>/` 目录，主 Markdown 同 stem 并只相对引用本目录 assets；无附件不建空目录。Ready 后，确切的 Markdown 与 assets 直接成为正式本地交付单元，DOCX/PDF/ZIP 按需派生。第一版不实现曾讨论过的独立 `handoff.package.build`、legacy event 或迁移 parser，不再复制十几个上游文件或生成独立 manifest/packager；Feishu 原生 Doc 路径保持权限/素材/格式/项目关联原型验证。
68. **确认可选可恢复 `handoff.dispatch / 产品交付发送`**：它只读确切的 Released artifact set，按目标 `disabled/manual/auto_when_ready` policy 调用 Adapter；默认 manual，自动发送需要范围确切且带版本的 preauthorization。幂等 identity 绑定 PRD ID+Release+Connector+Target+Action，UNKNOWN 先 query/reconcile；多个目标各自保留独立的发送尝试与回执，并共享同一来源。无 Connector/权限不影响本地 Ready/Released；receipt 只证明对应远端动作，不等于 Received/Accepted/Approved。无新 Package、内容审批、公开命令、固定 HITL 或第二真源；统一回传与影响处理见第 69—71 项。
...
70. **确认下游回传按最早受影响点返工**：状态/执行结果先更新绑定记录，新产品事实才派生关联 Signal；Implementation Deviation、PRD、Planning、Decision、Problem Learning、Roadmap 与独立新机会分别回到最早被证据实质影响的有效层。普通执行反馈不全链重跑，Released PRD 不覆盖，实质性 修改以新 Candidate/release 或 Decision 取代 并保留 Impact/downstream 引用。
71. **分离反馈提交、语义 实质性判断 与状态权限**：任何来源可提交，但不得直接 reopen/invalid 上游或选返工点；Agent 以 确切 binding、新可核查信息、是否推翻 assumption/scope/rule/AC 做无分数的轻量判断，说明影响层、依据/反方、建议和翻转条件；Controller 只执行 policy/权限明确的动作。普通意见 record+link，机械 result/local repair 可自动，实质性 Decision/承诺/Roadmap 变化才需有权 Owner 新决定，不新增审批 Gate。


72. **把 Run Resume/Audit 收敛为两个最小机器记录**：每个 Run 只持久化当前 state snapshot 与 append-only meaningful event stream；前者保存 current/last/next、精确引用、等待/暂停与 side-effect receipt，后者只记录实质性检查点、正式状态、Owner、Finding、副作用、暂停失败和 sub-agent 结果。恢复按 state→引用/files→外部/分支变化→直白 Resume Brief→当前 step 进行；发生实质性变化时先走失效/Impact。Git 管内容，State 管位置，Audit 管关键变化；不冻结示例文件名/完整 Schema，不保存 hidden CoT、逐 tool call、措辞或草稿哈希，不新增 Registry/数据库/Service/MCP/CLI/Node/Gate/HITL。
...
75. **固定 STOP/WAIT/future COMMIT 与历史重审边界**：STOP 结束当前主动方向，WAIT 保留未承诺可能性且必须有 review window/trigger，已决定未来做使用 `COMMIT + SCHEDULED / CONDITION_TRIGGERED`。所有 Record、确切快照与条件不可覆盖；新信息仅在命中条件、关键假设或实质性风险机会时提醒，维持原判断则追加结果，改判则以新 Record 取代；不建 watcher 或全量周期重审。
76. **确认 Knowledge source coverage、继续推迟 Node 17 接口**：所有正式 Decision 及其维持、复查、推翻或取代都是未来 KMG Raw Source Corpus 必要候选，不能只存 Released PRD 或压缩最终结论。具体复制、引用或索引方式，以及提交时机、权限、自动化、采纳、保留和同步，仍由未来 raw+derived Knowledge requirements 反推；当前不新增合同、真源或节点。
77. **合并 Problem Owner 责任到 Product Decision**：固定 `problem.owner.confirm` 退役；Problem Ready 只做仅提供建议的 Review 完整性与精确引用机械检查。Decision Brief 同屏展示确切 Problem，Owner outcome choice 承担责任；实质性的问题纠正返回 Synthesis/Learning。


78. **移除固定 Plan Owner Confirmation**：忠实 Planning 自动完成，按需一页摘要不是审批；只有超出 确切 Decision 的 实质性 新取舍回到同一 Product Decision，Plan Ready 不再检查确认记录。
79. **完成 HITL Density Audit 当前 处置结论**：正常 Discovery/Product 路径只保留 Product Decision 一处固定人类产品责任点；其余仅为高价值信息获取、实质性 choice 或外部副作用授权。可靠 Implementation Deviation 可零内容确认，外置团队审批在 Graph 外。
...
81. **把一期 Reviewer 全部收敛为仅提供建议，并生成同源内审意见**：未解决或存在分歧的 concern 与确切 PRD 在同一目录独立展示，主 PRD 只放短状态和相对链接；Review 不再拥有 Block/approval/waiver，外置团队保留最终审核。
82. **增加进行中访谈的运行时控制并收紧默认跳过**：单一公开 Skill 增加 `interview skip|resume`，总计十一个 intent words；完整 Discovery 默认至少一次实质访谈/等价对话，除非无 实质性 PM-only unknown 且低信息增益，或用户显式 skip。
...
- `build-manifest.json` 的最终 Schema、artifact 哈希 归一化算法、dirty tree 候选政策和 `execution_contract_fingerprint` 输入集合要在 Wave 1 实现并固定测试向量。
...
以下 156 项按主题分组，原编号保持不变。分组只帮助定位，不表示同组风险同等优先，也不把风险自动升级为实现承诺；具体优先级仍由 §30 的 Node Review 和 Slice 1—8 的真实证据决定。

### 28.1 Core、Host、Reviewer 与 Evals（1—15）
...
15. 自动化 `evals-generator / evals.build` 的生成质量、成本、可维护性、并行 join/失效 精度，以及何时需要外部 Agent Connector；TDD-ready Test Design Contract 的稳定边界、umbrella 命名和 Test Graph 消费方式仍待独立 Roadmap 设计，V1.4 不声称这些能力已经实现。

### 28.2 规划、实验与产品记忆（16—34）
...
24. 一期 建议性 concern 是否会被实现或人类误读为 BPG formal Block/approval；内审意见能否让外置团队快速聚焦而不把建议误当正式需求。
...
27. 带实验 intent 的 typed result 经统一 Intake 绑定 确切 Decision/PRD/Run 后，其数据完整性、选择性解释检测和 inconclusive 校准能否可靠支持回到 Product Decision，而不形成第二条 result-return Graph。
28. 外置团队对遗留 concern 的采纳/拒绝/补证回传能否与 确切 Review Record 对账，而不生成第二真源。
...
48. 普通 reroute 与 Product Changelog Proposal 的 实质性判断 边界是否会造成变更噪声或遗漏正式产品行为变化。
...
56. Bug Fix Brief 的五项轻量检查、建议性 风险 Reviewer 触发阈值和研发消费者接受度，能否在不过度加重流程的同时阻止规则漂移并把高风险 concern 透明交给外置团队。
...
63. 按拟议行动、风险、可逆性、可测量性和回滚判断证据充分性能否既允许低风险学习，又防止高风险行动被“可逆实验”话术降级；人工 override 如何在不破坏 status/处置结论/recommendation 分离的前提下设计，仍需后续讨论和原型验证。
64. 仅限当前 Run 的 Evidence Map 到 Knowledge Change Proposal 的去重、冲突、适用范围、新鲜度和发布延迟，是否会产生重复 Proposal、长期待处理事项，或把局部事实错误推广成跨项目正式知识。
...
68. Validator 对 source/policy/profile/template 失效、最低理解项和 Handoff 状态的误报/漏报率，以及依赖链变化后的重渲染成本是否可接受。
...
75. `MATERIAL_INPUT_CHANGE / FUNDAMENTAL_REFRAME` 重跑触发能否避免普通补证反复重算，同时在根本框架变化时及时创建 取代旧版的 checkpoint。
...
80. 七类 `source_resolution_type` 的区分准确率，尤其是 AI 可查、PM 私有背景、专业 Owner 和用户研究之间的误路率，以及重新选择来源的 取代 体验。
...
### 28.10 学习完成三维合同（88—90）

88. State Controller 能否稳定阻止 status/处置结论/recommendation 混写，尤其是 WAITING/PAUSED 误标完成、COMPLETED 缺 处置结论，以及建议被 Host/Connector 误当授权。
89. `INSUFFICIENT_TO_PROCEED` 的已尝试、缺口、原因、建议和 restart condition 是否足以支持以后恢复，又不会让系统为追求结束率滥用该 处置结论 或伪造 Synthesis。
90. Problem Quality Review 的 建议性 Finding 与机械 Candidate/引用 不一致能否稳定分开：Controller 不因 concern level 阻止 Product Decision，但必须拦截 失效/missing/mismatched Candidate、Review 处置结论 或上游 引用；repair target 映射仍需实现测试。
...
### 28.12 PRD 文档生命周期（95—98）

95. `archived/` 实质性 checkpoint 阈值能否既保留 Review/人工编辑/模板迁移证据，又避免 autosave 噪声和过高存储/阅读成本。
96. Ready 后从 确切 candidate 生成 release 的原子性、崩溃恢复和并发 CAS 能否避免“Ready 已通过但 release/Changelog/当前 只写了一半”。
97. 文档呈现变化与产品语义变化能否被可靠区分，防止把规则/AC 变化伪装成排版修订，或让纯文案改动触发不必要的完整 Product Decision。
98. invalidated/revoked release、当前 pointer、已有 Handoff 与 replacement/restart reason 的对账能否在真实知识影响和多次 release 后保持一致，且不删除历史文件。

### 28.13 问题综合（99—102）

99. Synthesis 对 实质性 direction-changing gap 的识别能否避免假完成和无意义返回：既不过早冻结错误 frame，也不把可保留的小 Unknown 反复送回 Learning。
100. 确切 Discovery input binding、失效 detection、失败恢复与 取代 能否在 Map/Learning/Knowledge 更新后稳定重算 Candidate，避免读取 `latest/current`、原位覆盖或重复搜索。
...
102. Synthesis checkpoint 的恢复成本和下游 确切 Candidate 引用能否保持轻量；Quality Review、Problem Ready 与 Product Decision 是否会误把 `COMPLETED`、Candidate 或 Synthesis Audit Event 当作已经完成 Owner choice。

### 28.14 Problem Ready（103—106）

103. 同模型不同隔离上下文能否让 Product Quality Reviewer 显著降低自审偏差和提示注入影响；专门 review Skill、独立 执行尝试 与外部 Reviewer Connector 的一致性、成本和故障降级需要真实 Case 验证。
104. 一期项目配置/Host identity 能否稳定识别当前 Product Decision Owner，避免身份缺失或跨项目误认；自然语言 outcome/scope 能否可靠结构化为一次 choice 而不重复确认；Handoff 引用 是否足以支持外置汇总审批且不被误呈现为组织最终批准。未来无权 Junior PM、代理授权和角色升级出现后再单独验证 escalation policy。
...
### 28.15 Problem Quality Review（107—110）
...
### 28.17 未来多智能代理协作（115—119）

115. 跨 Agent runtime/Host/provider 的 participant identity、认证和不可冒充性如何建立；同一会话换提示词、共享隐藏上下文或复用身份被误标为独立审计的检测方式仍需验证。
116. Collaboration/External Audit 协议如何对确切快照、role、Skill/policy、capability/model metadata、权限和 `Finding/Proposal/NodeResult` 做版本化，以及不同 Host Schema/生命周期如何协商兼容。
...
### 28.18 Git 预检与并行工作树（120）

120. 确切 project root 解析、父级 repository/worktree 复用和 HOME/广泛目录拒绝能否在不同 Host/多工作区环境稳定工作；`.gitignore`/敏感边界、Git 不可用或只读时的 `DEGRADED/BLOCKED`、并行 branch + worktree 清理/冲突整合，以及 实质性 checkpoint/tag 粒度仍需实现测试。

### 28.19 产品决策、风险、记忆与新证据（121—131）
...
122. `decision` Renderer/Validator 能否在中文及未来多语言下稳定生成自然、非术语化又不改变 machine outcome 的结论；why/反方/下一步/改判条件最低项能否避免裸 code 而不膨胀成模板文；Decision Brief、摘要和 Handoff 是否始终绑定同一 source/哈希、杜绝手工第二真源，需要首批 Profile 验证。
123. 一屏决策的“三项依据/一项认知提醒/一项边界/一步行动”预算能否提高首读决策效率而不丢失 实质性 信息；认知提醒是否只外显真正改判发现、渐进展开是否可发现，以及高风险披露能否稳定覆盖默认预算而不被简洁模板隐藏，需要真实 Owner 复述和对抗测试。
124. Decision Record 的 Owner 五项确认层与系统自动审计层能否既减少 PM 填表负担，又稳定保留 确切 引用/authorization/epistemic/supersession；实质性 disagreement 判定和反向 `superseded_by` 索引不修改旧 Record 的实现需原型验证。
...
128. 实质性 disagreement 的识别和“一次实质挑战”能否稳定避免静默迎合与无限争论；低置信 COMMIT 的显示、sponsor-directed authorization 与 epistemic 分离、accepted risk/constraint 向下游及外置审核传递，需要 Golden Case 与权限测试。
129. Decision transition validation 的 versioned rules、五种 route 原子写入、reject/repair 后恢复，以及 STOP/WAIT/RESEARCH/EXPERIMENT/COMMIT activation 的幂等性需要原型验证；尤其要证明不建 Decision Ready 仍能阻止旧 引用/缺字段流转，且不会把 Research/Roadmap/下游 Ready 职责偷渡进本节点。
130. 四类语义投影能否在本地文件实现中稳定保持 确切 引用 而不复制真源；STOP/WAIT/RESEARCH/EXPERIMENT/COMMIT 的 Roadmap/Research Request/Plan-PRD intent 投影、自动 Changelog/Audit、KMG 未接入的 local-only 标识及首次同步幂等性需要真实 Case 验证。
131. 新 Evidence 四级 实质性判断 分类能否避免 alert fatigue、漏报 kill/recheck 风险和无差别全量重跑；一屏提醒、Owner 维持决定记录、逐 artifact Impact 准确率，以及 KMG 冲突合并、跨团队 delivery/ack 和批量摘要阈值仍需原型验证。

### 28.20 规划、PRD 切片、覆盖与就绪（132—137）

132. Planning Refinement 与 formal Review 的 确切 Candidate 隔离、结构/表达/Decision/Discovery repair routing、partial 建议性 与 Ready-eligible verdict 区分、跨 product-plan/PRD profile 复用、实验 intent 条件化 rubric 和 no-progress 退出是否可靠，需要端到端原型验证。
...
134. `plan.slice` 能否在不依赖僵硬矩阵规则的情况下稳定形成大小合适、端到端、可验证且相对独立的候选；Agent-first 建议是否减少 PM 填表且只在 实质性 取舍时打断，消息治理等跨模块结果和 Evals 需要能否被正确表达，需要 Slice 1/2 对抗样本验证。
135. `plan.coverage.validate` 的五类检查和多种 处置结论 能否既发现静默遗漏/重复/体验断层/循环依赖，又避免“本轮全实现”与无差别 BLOCK；LIGHT 内联、STANDARD 简表、PROJECT_SCALE 只读挑战的质量/成本，以及 unresolved 不被滥用为垃圾箱，需要真实 Plan 比较。
136. `plan.reconcile` 对非语义自动调整与 实质性 Owner Decision 的区分能否稳定；返回 Slice/Iteration/Decision/Learning 的根因路由、灰度冲突的责任/复查、多个 sub-agent 方案的主 Agent join，以及新 Plan checkpoint/Impact 是否保持同源，需要端到端恢复测试。
137. 按需 Plan 一页摘要是否足以让 Owner 或外置读者正确复述当前结果、激活切片、后置项和风险；取消固定确认后，`plan.ready.gate` 对确切 Coverage、仅提供建议的 Review Finalize、冲突和实质性 Decision 引用的重算、可控未知项边界，以及已激活且符合条件的 PRD Run 创建幂等性，需要用 LIGHT 与 PROJECT_SCALE 案例验证。

### 28.21 PRD 生成、评测、审查、交接与恢复（138—153）

138. `prd.workspace.initialize` 能否在状态恢复和 Audit 中保持唯一语义；Controller 是否能稳定只准备 workspace/引用/version/directories 而绝不触碰 PRD 语义，以及 Agent/Reviewer/Optimizer/Owner 与程序化保存/校验之间的写权限隔离，需要 PRD Run 实现测试。完整初始化字段、失败/重试仍待节点 Review。
139. `prd.content.build → prd.render` 在简单 Run 中能否同一次完成而不丢语义，在复杂 Run 中是否能按条件保存最小可恢复 checkpoint 而不形成第二正式需求；Better-Product-Plan 确切 template mapping、Supplement/不兼容判定、模板迁移和项目 checklist 隔离，需要真实 PRD、模板替换与恢复测试。
140. `prd.generate` 的内部三个动作能否在失败后从确切检查点恢复且不暴露为伪节点；Template Profile 优先级、冲突时一次询问、必要语义扩展或不兼容、事实不编造和 archived→released 生命周期能否稳定执行；general Draft 的领域清理是否误删通用的多语言、兼容、数据、安全、隐私、灰度或跨团队能力。该模板何时成为默认由未来带版本的配置与 Roadmap 标准决定，当前不冻结固定人工 Review Gate。
...
142. `evals.build` 子节点的恢复、与 `prd.render` 同源并行 join、语义 delta→局部 失效 和纯排版不重跑能否稳定；最小风险覆盖是否避免 case 数量膨胀，synthetic/专业 Ground Truth 边界与并行 Reviewer 权限是否可靠；TDD-ready 输出能否被 Test Graph 消费而不越权生成工程测试或 verdict，需独立 Roadmap、Golden Cases 和下游原型验证。
143. PRD Fidelity 的确定性引用、Slice 与处置结论检查，和语义 Reviewer 能否稳定区分“忠实展开”与实质性新规则，避免既漏掉范围蔓延、偷带未来范围或把未知项升级为事实，又把合理场景、分支、异常和 AC 扩写误退上游；退役两项固定确认后，Controller 自动形成 release 的原子性、按需最终摘要、Connector 政策与发送，以及 BPG 与外置审批状态隔离，需用真实 PRD，并在 Feishu Connector 与本地无 Connector 两种情况下对照验证。
144. `review.parallel` 的 Goal Fidelity Review Packet 是否能为所有 Reviewer 稳定绑定同一个确切 Candidate、目标承诺和 Evidence 快照；LIGHT 合并角色后是否仍保留目标忠实度，条件式 Reviewer 组合是否漏触发专业风险；首轮隔离、Finding 与交叉检查字段、最多两轮二次审查、五类修复验证和两轮无进展停止，能否抑制多数票抵消致命偏离、缺少上游依据的功能混入及无限改写，需用真实 PRD、Reviewer 冲突和弱证据 PASS 样本对照验证。
145. `review.aggregate` 的可恢复汇总尝试能否稳定处理必需或可选 Reviewer 的晚到、失败、失效和外部结果，并证明每条原始 Finding 都未丢失；还需验证语义聚类、根因关联和最早修复位置的人工纠正率，Controller 确定性完整性检查的拦截率，以及 LIGHT 自动汇合能否在减少开销的同时，不把 Aggregate 偷换成 Gate、Ready 或新 HITL。验证样本需覆盖错误合并、丢失 Finding、旧 Candidate 结果晚到和必需 Reviewer 失败。
146. `review.finalize` 能否针对相同的确切 Aggregate、Candidate 和规则稳定重放，可靠拦截缺失的审查尝试、丢失 Finding、缺少处置结论、失效绑定与内审意见错版，同时不因 Reviewer 不可用或关注等级而阻塞，不裁决专业观点，也不与 `prd.ready.gate` 重复；需要用外置关注项、旧 Candidate 和无进展样本验证。
...
148. `prd.ready.gate` 的六类检查是否足以阻止 Candidate、Review、Finalize 与内审意见错版、REQUIRED Eval 缺失、上游失效和 Template、版本或 Changelog 缺口，同时不把语义 Review、Owner 二次确认、外置审批、Connector 可用性或写权、研发或测试完成、建议性关注项关闭偷渡为前提；`N/A+理由`、`NOT_READY` 中确切的修复与恢复信息、相同输入重放和 READY→release/Handoff 的原子性需验证。
149. 自包含的 Candidate 或 release 目录能否在标题改名、多版本 assets、相对媒体引用、大文件与 Git 边界和独立复制时稳定保持确切哈希；Markdown 到 DOCX、PDF 或 ZIP 的语义与媒体保真、无附件轻量路径，以及真实飞书租户的文档创建与编辑、素材上传、权限、项目关联和格式降级均需原型验证。Feishu 原生文档状态保持 `PROTOTYPE_REQUIRED / PENDING_PERMISSION_VALIDATION`，DOCX 手工导入只是回退方案，不是已发送事实。
150. `handoff.dispatch` 的目标政策解析、默认 `manual`、防越权自动发送、幂等标识、UNKNOWN 结果的查询与协调、并发多目标隔离和回执状态映射，需用真实 Connector 原型验证；尤其要证明外部失败不撤销本地 Ready，同一 release 不被复制成多份正式内容，手工导入不会在无证据时升级为 SENT 或 IMPORTED，且来源失效能正确触发后续 Impact，而不改写历史执行记录。
151. 统一 `signal.ingest` 能否在 Issue Collector、飞书、研发或测试 Graph 的自然语言、协议事件和带类型结果中稳定保全原始内容及其来源与形成过程，确定性识别协议类型，并在状态、结果和 Product Signal 三类处理间无损分流；缺失确切引用时 `signal.relate` 的关联准确率、最小澄清率，以及中央策略避免 Connector 内部 Router 漂移的效果，需用真实回传验证。
152. Agent 的无分数 实质性判断 判断能否稳定区分 evidence/result/opinion/duplicate，并定位 PRD/Planning/Decision/Learning/Roadmap/新事项的最早受影响点；具体领域中谁可触发何种状态动作、哪些证据足以推翻 assumption/scope/rule/AC，以及普通意见噪声、误 reopen、漏报 实质性 challenge 和版本/Impact 扩散率，仍需真实样本校准。当前只确认轻量边界，不冻结复杂领域 policy。
153. 当前 state snapshot 与 meaningful event stream 能否在暂停、跨会话、sub-agent 失败、UNKNOWN Connector result 和 branch/worktree 变化后稳定生成足够但不过载的 Resume Brief；需验证 确切 引用/失效 检出、重复 side effect 防护、事件噪声比、纯格式变化误重跑率，以及 Git/State/Audit 不互相复制。不得以此扩张全量 event sourcing、Registry 或完整会话重放。

### 28.22 MVU 决策、停止等待与未来知识来源（154—156）
...
155. STOP/WAIT/future COMMIT 是否能阻止无期限候选和伪 Roadmap，同时让真正的新 Evidence 命中 restart/recheck/key assumption/实质性 opportunity 时及时提醒；需验证条件匹配、提醒噪声、维持决定的追加记录、取代 与未来 scheduler/Connector 不越权。
156. 未来 KMG 能否在不复制 Decision Ledger 或丢失精确来源记录（provenance）的前提下，消费所有正式 Decision/evolution 作为 raw source，并同时建设 derived knowledge；复制、引用或索引方式，以及权限、保留、去重和同步，必须由真实 KMG consumer requirements 原型决定，不能由当前 BPG 预设。
...
本节只解释历史继承关系，不记录 V1.5 的当前变化。V1.5 的状态以文首为准，版本改动以 [架构 Changelog](../../architecture/CHANGELOG.md)为准。
...
V1.3 是对 V1.2 逐节点讨论结果的**一致性收敛版**，不是又一次扩大范围。它保留 V1.2 已确认的 Better Product Graph 系统边界、Outcome-first、横向模块化 × 纵向迭代化、单一 `signal.ingest`、同一 Product Pipeline、建议性 Reviewer、自包含 PRD release、可选 Connector、轻量 State/Audit/Git 与 Knowledge Maintenance 权责边界。

V1.3 主要删除或降级了讨论过程中残留的旧设计：固定 Problem/Plan/PRD/Handoff/Bug 人工确认、Reviewer blocking/waiver、独立 Experiment lane/package、未有消费者却提前建设的 legacy alias、过细的 KMG Proposal/Impact 实现合同，以及把 Audit Event 写成第二状态真源的倾向。模板只冻结可配置、可升级、可 pin/回滚和 确切 fallback 的 seam；general v0.1 仍是 Draft/Bootstrap 候选。Golden Cases 仍是未来实现验收规格，尚未运行。
...
| 4 | Bug Baseline Assessment / Bug Fix Brief / Product Logic & Ambiguity 分流 | `CONFIRMED` | 确切的当前有效基线、原因分类与表现标签、五项自动分流条件、Agent 优先的 Junior PM 辅助、按条件进行一次最小澄清或人工改判、Bug Brief、Decision、Discovery、Incident 分流及 Eval 和 Handoff 边界已确认；固定 PM Route Confirmation 退役 |
|  | **阶段三：证据、问题发现与就绪** |  |  |
| 5 | `evidence.collect` / `evidence.map` / Problem Evidence Map | `PARTIALLY_CONFIRMED` | 不可变的来源与形成过程、MVU 驱动收集、语义映射、仅限当前 Run 的版本与正式知识隔离已确认；标签校准、阈值和真实来源失败处理待原型 |
| 6 | `problem.assumption.audit`（问题假设审视） | `CONFIRMED` | Evidence Map 后、Learning Loop 前的独立轻量节点；默认 AI 自助/零 PM 深访；原话与事实角色分离；现象/影响/问题假设/期望结果/提出方案；动态方向性假设与反证/历史/no-action/症状原因检查；exactly one MVU + 最佳来源 + 下一信息请求；只完成可信认知起点；run-local versioned checkpoint；三类下一建议；实质性 reframe 重跑；无独立 Reviewer/Gate/正式业务 Artifact |
| 6a | `problem.learning.loop` / Evidence Request | `CONFIRMED` | 已确认独立可恢复 Loop 边界；每 Round 一个核心 MVU + 少量紧密相关请求；七类来源路由；PM 访谈采用 bounded joint judgment 六步，Junior PM 获得非诱导脚手架但标准不降，挑战强度按风险/证据冲突/可逆性选择 `LIGHT / STANDARD / STRONG`，一次最高价值挑战后记录分歧并及时停止；collect/map loop-back；Learning Round Delta 使用最小 structured rationale 而非默认全量逐字对话；Evidence Request 是 versioned wait/resume 合同而非节点；五种 runtime status、三种 completion 处置结论、建议性 next-action recommendation 三维分离；`WAITING_FOR_EVIDENCE` 不限 human 且可恢复；停止采用 action-relative sufficiency；普通资料研究留在 Loop，正式 Research/Experiment 只能建议并交 Product Decision。人工 override 和完整 Schema 仍待讨论 |
| 7 | `problem.synthesize` | `CONFIRMED` | Learning=`COMPLETED + READY_FOR_SYNTHESIS` 后运行一次独立轻量收敛；绑定确切的 Discovery 输入；生成带版本的 Problem Definition Candidate；保留未知项、非问题与用户方案的关系；发现会实质影响结果的缺口时，携新 MVU 返回 Learning；来源失效后重算并明确取代关系；不搜索、访谈、补证、决策或选方案，`COMPLETED` 不等于 Problem Ready |
| 7a | Problem Ready 语义 Review 与确定性状态边界 | `CONFIRMED` | 独立、隔离的 Product Quality Reviewer Agent 只读确切 Candidate 并输出建议性 Finding；程序化 State Controller 只重算确切 Candidate、Review 处置结论和上游引用，并作为唯一组件原子写状态。固定人类确认已按 7c 折叠到 Product Decision；Agent 模拟只能标为 `ADVISORY_ONLY` |
| 7b | `problem.quality.review` | `CONFIRMED` | 读取确切、已冻结的 Candidate，以及 Evidence、Learning、Knowledge 与 Product Memory；覆盖完整质量维度；使用独立只读审查尝试；Finding 和 Verdict 限定行动范围；支持 `REVISE_SYNTHESIS / RETURN_TO_LEARNING / NEEDS_OWNER / ROUTE_REEVALUATION`；首次完整审查，后续针对变化范围复审并回归全局不变量；无进展时返回 Learning 或 Owner；Junior PM 先看到关键 Finding、原因和首选建议 |
| 7c | `problem.owner.confirm` | `RETIRED / FOLDED_INTO_PRODUCT_DECISION` | 不再固定确认 Problem Candidate。Decision Brief 同屏展示确切 Problem，Owner 选择结果时即承担责任；若存在实质性错误则返回 Synthesis 或 Learning。无别名、遗留事件、审批层或第二摘要真源 |
| 7d | `problem.ready.gate` 轻量确定性政策 | `CONFIRMED` | 自动、无感运行；只检查 Candidate 是否确切、当前有效且未因实质性变化失效，同版本的建议性 Quality Review 与处置结论，以及上游 Evidence、Learning、Synthesis 引用、状态、版本与哈希是否一致可解析；`READY` 自动进入 Product Decision，`NOT_READY` 列出确切的机械未满足条件与修复位置；无 Owner Confirmation、语义审查或审批，不因关注项或普通未知项阻塞，也不检查后续 Ready |
|  | **阶段四：产品决策、产品记忆与实验意图** |  |  |
| 8 | `product.decision` 节点形态 | `CONFIRMED` | 一个独立可恢复节点；AI Decision Brief、按需 bounded adversarial/domain sub-agent Review、Owner 讨论/挑战与明确选择、Controller deterministic route 均为内部能力而非节点；versioned run-local Draft/checkpoint 可恢复但不等于正式决定；Agent 给首选、实质性 时才外显最强反方，Owner 选择、Controller 写 route；默认不全量 Reviewer |
...
| 8b | Product Decision 完整选择与专业治理逻辑 | `PARTIALLY_CONFIRMED` | 已确认：MVU 驱动的 RESEARCH/EXPERIMENT/COMMIT guide、Agent 单一首选/相邻替代/翻转条件、STOP/WAIT/future COMMIT、条件重审和无固定评分。剩余 实质性判断、自动化扩大后的专业治理与其他领域 policy 待真实运行后设计 |
| 8c | Product Decision 默认一屏呈现 | `CONFIRMED` | 一项直白首选、最多三项关键依据、最多一个 实质性 认知提醒、一项定性 confidence + 最大 unknown/翻转条件、一个具体下一步；最强反方仅 实质性 时一句；完整 Evidence/其他选项/认知/风险/历史/Audit 渐进展开；同一 Draft/Record 真源，无 hidden CoT；重大风险披露可突破预算，不用死字数 |
| 8d | Decision Record 通用最小合同 | `CONFIRMED` | Owner 一屏确认 chosen decision、applicability scope、最多三项理由、最大 Unknown + flip/stop/restart condition、next action + checkpoint/trigger；实质性 分歧才显示 disagreement + Owner reason；系统自动填 确切 引用/identity/version/关系/audit；immutable/versioned + 取代；同一 Record 渲染，无第二真源 |
...
| 8g | Agent–Owner 实质性分歧 | `CONFIRMED` | Agent 先给首选；Owner 选择不同结果时进行一次有界的实质性质疑，聚焦证据缺口、风险和首选理由，不只问“确定吗”，也不无限争论；有权 Owner 最终决定；按条件保存双方理由、授权依据、接受的不确定性与风险、复查与停止条件和执行约束；授权不提升置信度；弱证据且可逆、可测时优先建议 EXPERIMENT；R3 关注项随下游与外置审核传递，不新增结果类型或节点 |
| 8h | Product Decision 节点终止与路由 | `CONFIRMED` | 不新增 `decision.ready`、Gate、Artifact、Reviewer 或二次确认；Controller 重算 Owner 的确切选择和 Record、合法结果、通用与结果专属字段、必要风险与约束、当前引用、分歧与已接受风险；通过后确定性路由，失败则留在 Decision，并直白说明未满足条件与修复位置；Research、Roadmap 和下游 Ready 未在这里提前确认 |
...
| 9a | Decision/Roadmap/Changelog/Audit 分工 | `CONFIRMED` | 同一 Decision 的四种语义投影，不是四套流程/节点/真源；所有正式 Decision 含 STOP 进 Ledger；Roadmap 只收未来行动；RESEARCH 进 Request，EXPERIMENT 进同一 Plan/PRD pipeline 且不形成 committed product/Portfolio 真源；Changelog 只收 实质性 产品意义变化；Audit 自动记录执行；KMG 未接入时本地完整运行，接入后共享/发布但不重做 Owner 决策，提交合同仍延后 |
| 9b | 历史 Decision 受新 Evidence 影响 | `CONFIRMED` | 新 Signal 保留四个去向，并以 `existing_links` 关联确切历史记录；支持性证据或不影响既有决定的变化只追加或摘要；挑战关键假设时用一屏提醒并给出首选；命中停止、复查或实质性风险时，逐项约束受影响行动。STOP 或 WAIT 只在重启或复查条件、关键假设或实质性机会命中时主动重审；Owner 可维持原决定、补证、实验、取代或缩小范围；旧 Decision 不可变，Impact List 只标受影响对象；冲突合并和跨团队实现仍待 Review |
...
| 11a | Product Planning 双向深化原则 | `CONFIRMED` | Product Plan v0 是系统假设/全局地图；按价值/风险/依赖逐块深化，每轮 global reconciliation；允许拆并删/重排/转 Research/Experiment/Decision；读取同一份确切快照的 bounded sub-agents 只交 Proposal，主 Agent 单点整合；实质性版本、检查点、变更日志、取代关系和影响记录；停止后才二维拆 PRD；稳定 Plan 小改动只 affected-area review + global impact check；Refinement 属于 `product.planning` 内部 Loop，不建第二真源 |
| 11b | Planning Refinement 与正式 Review–Optimize | `CONFIRMED` | Refinement 在 `product.planning` 内生成、探索、拆解和协调，并产出带版本的检查点与稳定 Candidate；正式 Review 由独立只读 Reviewer 通过审查尝试检查确切、已冻结的 Candidate，Optimizer 或主 Agent 负责修复。局部遗漏定向优化，结构问题返回 Refinement，Decision 或 Problem 失效时返回对应上游；审查尝试、处置结论和 Finalize 完整后进入 Plan Ready，部分审查只提供建议；Plan 与 PRD 复用通用引擎和 Profile，实验 intent 只按条件调整检查准则 |
...
| 11d | `plan.slice`（PRD 切片） | `CONFIRMED` | 同时用横向模块化+纵向迭代化生成 PRD Candidate Slice Map/List；检查独立目标、端到端结果、验证、相对发布/回滚和大小；不按矩阵格/技术层机械拆，不写 PRD；Agent-first，实质性 切分取舍才问 PM |
...
| 11f | `plan.reconcile`（规划协调） | `CONFIRMED` | 把 Coverage/深化/新 Evidence Finding 放回整体权衡；非决策调整可自动 checkpoint，实质性 目标/承诺/风险变化由 Owner确认并写既有账本；可回 Slice/Iteration/Decision/Learning；允许带责任与复查条件的显式冲突，不建审批 Gate/第二状态 |
| 11g | `plan.owner.confirm` + `plan.ready.gate` | `CONFIRMED / CONFIRM RETIRED` | 固定 Plan 确认已折叠为条件式 Product Decision：普通、忠实的 Planning 自动完成，按需摘要不是审批；出现新的实质性取舍时返回 Decision。Controller 只检查确切 Plan、Coverage 与处置结论、只提供建议的 Review Finalize、依赖与冲突，以及必要的实质性 Decision 引用；PASS 只为当前已激活且符合条件的切片创建 PRD Runs |
|  | **阶段六：PRD、评测、审查与就绪** |  |  |
| 12 | `prd.content.build` / `prd.render` | `PARTIALLY_CONFIRMED` | 完整产品语义组织与模板表达的能力边界已确认；`prd.render` 的完整内部写作、优化与恢复逻辑仍待实现设计 |
| 12a | PRD Workspace Initialization 职责边界（非智能 Graph Node） | `CONFIRMED` | 面向人称“准备 PRD 工作空间”；正式内部名为 `prd.workspace.initialize`。Controller 只创建 Run 和工作空间、绑定确切引用、登记状态与版本、准备目录，并负责机械保存、流转和校验；Agent 才在内容构建与渲染时创作语义，Reviewer 与 Optimizer 各守职责，Owner 只在需要实质性产品判断、专业判断或副作用权限时介入；第一版不建旧名兼容层，也不因此冻结完整初始化 Schema、Gate 或 HITL |
| 12b | PRD 内容/模板产物边界 | `CONFIRMED` | 实读 Better-Product-Plan 后取消默认独立、完整的 Product Spec。简单 Run 在同一执行尝试中直接生成一份 PRD Candidate；复杂场景才可配置内部检查点，且不得正式发布。过渡模板的确切版本、必要语义映射、扩展、不兼容处理和项目检查项隔离已确认；不新增 HITL 或旧名别名，完整渲染逻辑未提前确认 |
| 12c | `prd.generate` 节点形态与 Template Profile | `CONFIRMED` | 一个可恢复 Agent 节点内含内容构建、确定性 Profile 解析和 Agent 渲染，不注册三个节点或 HITL；Profile 按“项目配置 → 受信知识 → BPG 当前配置”的回退或默认顺序解析，只有冲突无法判定时才询问一次；首个 Candidate 进入 archived，Ready 后形成确切 release；general v0.1 是可配置、可升级的 Draft/Bootstrap 候选，内容与 promotion 优化留在 Roadmap，一期不建产品类型模板库。具体渲染、写作和优化逻辑仍待实现设计 |
...
| 13b | `evals.build`（PRD Run 内按需可恢复子节点） | `CONFIRMED` | 内容稳定后与渲染分支同源并行；范围界定、生成和审查是内部动作；汇合时绑定确切的 PRD 与 Pack。语义影响只使受影响部分失效，排版变化不触发全量重做；最小 Pack、Ground Truth、只读 Reviewer 与生成者修改边界已确认。TDD-ready 未来接口已登记，统称、完整 Schema、Test Graph 实现与独立 Roadmap 仍为 OPEN |
| 14 | Reviewer 仅提供建议 / future autonomy governance | `CONFIRMED / FUTURE DEFERRED` | 一期全部 AI/sub-agent Reviewer 只给 concern/建议/返工点，不拥有 Block/veto/approval/waiver；遗留项同源披露给外置团队。真实专业身份、policy、blocking、waiver 只在未来无人值守研发/发布时另行设计 |
| 15 | PRD Review–Optimize / Ready | `PARTIALLY_CONFIRMED` | 忠实一致性、目标忠实内核、`review.aggregate`、`review.finalize`、`prd.optimize`、内审意见 companion、HITL 精简与 Ready/dispatch 已确认；其余具体 Reviewer rubric 待实现设计。文档版本管理已确认 archived/released、Candidate/Attempt 分离与 确切 release Handoff |
...
| 15b | PRD Fidelity / HITL / Ready / Dispatch 边界 | `CONFIRMED` | Product Reviewer 首先证明 Candidate 忠实于 确切 Decision/Plan/Slice/Knowledge/Evidence/constraints；实质性 新内容返回最早上游。退役默认 `prd.owner.confirm_understanding` 与固定 `handoff.owner.approve`，第一版不建旧事件兼容层；Controller Ready 后自动生成 self-contained release，本地 BPG Released 不等于外置审批；Connector policy 另决定 dispatch。一页最终摘要按需同源渲染，不是审批/Gate/第二真源 |
...
| 15d | `review.aggregate / 审查意见汇总` | `CONFIRMED` | 轻量、可恢复的内部汇合节点；LIGHT 可在同一用户动作中自动完成，但仍保留汇总尝试。主 Agent 负责聚类、根因、分歧、缺少上游依据的线索和最早修复位置；Controller 只检查必需的审查尝试、同一快照、Finding 字段、晚到或失效标记和无损追溯，然后写状态。复用 Review Record、Finding、Verdict、Disposition 和 `review_summary`；不修改或生成 Candidate，不执行 Gate 或授权，不自报 Ready，不增加 Artifact 或 HITL；正式路由见 15e |
| 15e | `review.gate` → `review.finalize / 审查收尾` | `RETIRED / REPLACED + CONFIRMED` | 当前未实现的 Review Gate 退役；Finalize 是 Controller 内部的确定性动作，只检查审查尝试、Finding 处置结论、确切 Candidate 与同源内审意见。它不是 Node、Gate、Agent 或 HITL，不要求 PASS，也不因建议性关注项阻塞 |
| 15f | `prd.optimize / 根据审查意见修订 PRD` 与有界 Loop | `CONFIRMED` | 可恢复的 Agent 节点和 PRD Profile；只修复主 Agent 已采纳且位于当前 PRD 的建议，批量完成最小必要修订，产生一个新 Candidate、变化说明和 Finding 映射，只声明已修复；Reviewer 验证修复状态。Candidate 版本与 Review Attempt 分离；轮次上限为 2、3 或 4，两轮无进展时提前停止 |
| 15g | `prd.ready.gate / PRD 最终就绪检查` | `CONFIRMED` | Controller 最终执行确定性边界，只检查当前 Candidate、同版本 Review、Finalize 与内审意见、当前引用、REQUIRED Eval、Template、Document Experience、版本、Changelog 和机械合同；建议性关注项本身不阻塞。通过后自动形成 release 和本地 Handoff，不要求 Owner 二次确认或外置审批 |
| 15h | PRD Released 目录与包简化 | `CONFIRMED` | archived 与 released 中的每个确切版本使用自包含、同名主干的目录，Markdown 与相对 assets 是正式来源，exports 按需派生且不阻塞 Ready。Ready 后的 release 即本地交付单元；第一版不建设独立打包动作、遗留事件或迁移解析器，也不影响 Incident 或 Bug 合同 |
|  | **阶段七：交接与下游回传** |  |  |
| 16 | Handoff / downstream feedback | `PARTIALLY_CONFIRMED` | Product self-contained release、`handoff.dispatch`、统一回传接入、轻量 实质性判断 和最早受影响点返工已确认；具体领域 实质性判断 policy、跨团队执行和下游完整 typed result schemas 仍待原型 |
| 16a | `handoff.dispatch / 产品交付发送` | `CONFIRMED` | 可选、可恢复的 Connector 执行尝试，不是本地完成必经步骤。每个目标的政策为 `disabled/manual/auto_when_ready`，默认 `manual`；自动发送需要确切预授权。它只读确切 release，使用幂等标识；结果为 UNKNOWN 时先查询和协调；多个目标各有独立回执。它不打包、改写或 Review，也不重复要求 Owner 审批；没有 Connector 不阻止 Ready，回执不能外推为已接纳或已批准。Feishu 原生文档仍需权限原型，DOCX 导入仅为回退方案 |
| 16b | 统一下游回传与最早受影响点返工 | `CONFIRMED` | 回传统一进入 `signal.ingest`；先用状态或结果更新绑定记录，只有新的产品事实才派生带确切关联的 Signal。Agent 根据绑定、新的可核查信息，以及对假设、范围、规则和 AC 的影响作轻量、无分数判断，给出直白建议、最强反方和翻转条件；Controller 按政策和权限写状态。返工落到 Engineering、PRD、Planning、Decision、Learning、Roadmap 或新事项中最早有效的一层；release 不覆盖，普通意见不自动重开，只有 Decision、承诺或 Roadmap 的实质性变化才需要 Owner 作出新决定；不新增 Node、Gate、Artifact 或 HITL |
|  | **阶段八：知识影响** |  |  |
| 17 | Knowledge impact / proposal | `DEFERRED / PENDING_KNOWLEDGE_REQUIREMENTS` | 仅来源覆盖已确认：所有正式 Decision，包括五种结果及其维持、复查、推翻和取代演进，都是未来 Raw Source Corpus 的必要候选；PRD 与相关原始材料也要保留确切引用。提交合同仍不冻结。未来 KMG 必须先定义原始与派生两层的消费需求，再反推采用复制、引用还是索引，以及时机、自动化、权限、采纳、保留和同步方式；不能只提交压缩认知，也不能合并 Ledger 与正式知识存储 |
|  | **阶段九：状态、Host 与横向运行能力** |  |  |
...
| 18a | Lightweight Run Resume / Audit / Git 分工（横向运行规则，不是 Graph Node） | `CONFIRMED` | 每个 Run 只持久化当前状态快照和只追加的有意义事件流；快照覆盖当前、上一和下一步、确切引用、等待与暂停状态和副作用回执，事件只记录实质性检查点、正式状态、Owner、Finding、副作用、暂停与失败和子智能代理结果。Git 管内容版本，State 管当前位置，Audit 管关键变化；恢复时先校验引用、文件、外部结果和分支变化，再给出直白 Resume Brief。Controller 是状态的唯一写入者；示例文件名和完整 Schema 未冻结，不新增 Node、Gate、Artifact、HITL、Registry、数据库、Service、MCP、CLI 或重型事件溯源 |
...
| 20 | Document Experience 横向规则（不是 Graph Node） | `PROTOTYPE_REQUIRED` | Policy/Profile 最低项、Renderer 保真、Validator 失效/绑定重算、Readability Reviewer 灰度、模板/Supplement、Incident/Bug 轻量性、八个一期 Profile（含 `product_plan` Plan Ready 摘要与 `internal_review` companion view）与第二真源防护 |
...
| 22 | Sub-agent Execution Policy 横向执行形态（不是 Graph Node） | `CONFIRMED` | 对抗性、可并发或需独立旁路的工作优先使用有界子智能代理；主 Agent 只负责编排、汇合和保留分歧；使用确切快照、最小权限和统一结果，禁止正式写入与副作用；Host 的模型配置与实际映射可审计；必需与可选任务有不同失败边界；具体 Codex 持久化、并发和模型选择能力仍为 `PROTOTYPE_REQUIRED` |
| 23 | Multi-Agent Collaboration 未来能力（不是 Graph Node） | `PROTOTYPE_REQUIRED` | 一期只使用当前 Host 内部的子智能代理；未来才支持跨独立 Agent 运行时、Host 或模型提供方协作。届时需审计独立实例、上下文、目标、输入与输出，并复用确切快照、角色、结果、权限和汇合合同；Collaboration 或 External Audit Connector 的身份、认证、协议、持久化、成本和失败治理尚未实现，不作为一期验收依赖 |
| 24 | Project Git Preflight 横向基础设施（不是 Graph Node 或 Gate） | `CONFIRMED` | 开始或恢复时校验确切项目根目录；复用父级 repository 或 worktree，否则在本地静默执行 `git init -b main`；拒绝 HOME、广泛目录和嵌套仓库；先处理 `.gitignore` 与敏感边界；不自动 add、commit、push 或创建 remote；失败时如实标记 `DEGRADED/BLOCKED`；并行子智能代理使用独立 branch 和 worktree，主 Agent 审查差异后整合；只有实质性检查点或冻结版本才提交或打 tag |

第 18a、20—24 行借用同一 Review Card 机制审查横向能力，不把它们登记为 Graph Manifest 节点，也不改变前 19 组业务/运行节点的顺序。

V1.4 继承 V1.3 已完成的产品层逐项处置，并关闭 distribution/eval 文档合同冲突；尚未由实现或真实 Run 证明的部分统一标记为 `PARTIALLY_CONFIRMED / PROTOTYPE_REQUIRED / DEFERRED / IMPLEMENTATION_PENDING`。其中精确 Planning Profile 阈值、具体领域 实质性判断、下游完整 result schemas、State/Event 完整 Schema、CAS/崩溃恢复、`prd.render` 细节、Eval generator、Module/Iteration Map 表达、Codex Host conformance、Plugin Contract Suite、Product Golden v0.2、KMG submission/Impact sync、Multi-Agent 和 Git worktree 生命周期仍需实现或原型证据。Problem Ready 不保留 action-scoped matrix；正式专业身份、blocking policy 与 waiver 只属于未来无人值守治理。V1.4 的冻结不把这些开放项伪装成已经验证，也不新增 Node/Gate/Artifact/HITL。

### 30.4 Human-in-the-Loop Interaction Density Audit 处置结论

> **CURRENT DISPOSITION COMPLETE；访谈自动跳过阈值仍需运行校准。** 当前核心只保留 `product.decision` 一处固定人类产品责任点。它要求明确 Owner intent，但自然语言已经清楚表达 确切 outcome/scope 时不再追加“是否确认”UI。其他人工交互只有三类正当理由：降低一个会改变行动的高价值 Unknown；解决一个 实质性 产品取舍并回到同一 Product Decision；授权一次真实外部副作用。
...
| 自动化 | Signal classify/router、Assumption Audit、Evidence 自助检索/map、Problem Synthesis/Ready、普通 Planning/slice/reconcile、PRD generate、建议性 Review/Optimize/Finalize、PRD Ready/local Release、Git preflight/state/resume | Agent/Controller 按各自边界运行；Evidence Learning 仍可从 PM 获取无法自助且高价值的信息 |
| 按条件触发 | Signal 关联/关键上下文一次最小澄清、Problem Learning PM 访谈、Planning 实质性 choice→Decision、Template 冲突、Eval/Ground Truth/专业输入等待、`handoff.dispatch` 无预授权时的 确切 side-effect authorization | 不得包装成每份产物固定确认；每次先说明当前判断、为何需要人、答案/授权改变什么 |
| Graph 外 | 团队最终汇总审批 | BPG 交付 确切 Released PRD、同源内审意见与审计 引用，但不声称外置审批已通过 |
...
访谈是人类参与中最重要的信息获取环节，不能为了追求“一个固定触点”而自动省略。§7.5 已确认显式 `interview skip|resume` 与更严格默认：完整 Discovery 原则上至少一次实质访谈/等价当前对话，除非 Agent 能证明没有 实质性 PM-only unknown 且继续提问信息增益低，或用户显式 skip。此规则仍需用真实 Run 校准误跳过率、打断密度和 LIGHT/STANDARD/PROJECT_SCALE 交互预算，但不新增 Gate、Artifact 或审批。
...
本附录保留 ADR-001—ADR-097 的完整决定、理由和未选择方案，供实现与审计追溯。它解释为什么这样定，不替代记录版本变化的 Changelog；当前实现状态仍以文首和 Changelog 为准。
