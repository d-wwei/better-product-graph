#!/usr/bin/env python3
"""Expose the v0.7 fixture-review gate without creating semantic evidence."""
...
FIXTURE_COMMIT = "8316141a90cf4c0397ab7334ecb0ae1fd8022bd9"
SUPERSEDED_TREE_HASH = "sha256:3bd097779ef748c4221352fca921ac624d1746dfcd1a7919e914145a59664005"
...
def build_agent_case_payload(case_id: str) -> dict[str, Any]:
    ...
    if case_id not in CASE_IDS:
        raise ValueError(f"unknown v0.7 readability case: {case_id}")
...
def validate_fixture_review_gate(
    ...
    """Validate only the two approved independent current-tree review records."""
    ...
    for label, review in zip(("reviewer-a", "reviewer-b"), typed_reviews, strict=True):
        ...
        if not _valid_attempt_identity(identity):
            reasons.append(f"{label}: reviewer_identity")
        else:
            reviewer_ids.append(identity["id"])
        fixture = review.get("fixture_tree")
        if (
            review.get("schema_version") != "prd-readability-v0.7-fixture-review.v1"
            or review.get("review_status") != "CURRENT"
            or review.get("verdict") != "APPROVED"
            or review.get("concerns") != []
            or review.get("prior_verdict_reused") is not False
            or not isinstance(fixture, dict)
            or fixture.get("commit") != FIXTURE_COMMIT
            or fixture.get("tree_hash") != tree["tree_hash"]
        ):
            reasons.append(f"{label}: stale_or_unapproved")
        ...
        for item in cases if isinstance(cases, list) else []:
            primary = (
                item.get("single_primary_capability")
                if isinstance(item, dict)
                else None
            )
            unintended = (
                item.get("unintended_material_issue")
                if isinstance(item, dict)
                else None
            )
            if (
                not isinstance(primary, dict)
                or primary.get("multiple_equally_reasonable_primary_judgments")
                is not False
            ):
                reasons.append(f"{label}: single_primary_capability")
            if not isinstance(unintended, dict) or bool(
                unintended.get("present", unintended.get("result"))
            ):
                reasons.append(f"{label}: unintended_material_issue")
    ...
    superseded = adjudication.get("superseded_tree")
    concern_ids = (
        {
            item.get("concern_id")
            for item in superseded.get("concerns", [])
            if isinstance(item, dict)
        }
    ...
    if (
        ...
        != "prd-readability-v0.7-fixture-adjudication.v1"
        or adjudication.get("status") != "APPROVED_FOR_PREREGISTRATION"
        or adjudication.get("approved_case_ids") != list(CASE_IDS)
        or adjudication.get("review_refs") != expected_review_refs
        or adjudication.get("fixture_tree", {}).get("tree_hash")
        != tree["tree_hash"]
        or not isinstance(superseded, dict)
        or superseded.get("tree_hash") != SUPERSEDED_TREE_HASH
        or concern_ids
        != {
            "PNG_MARKER_END_ARROWHEADS_MISSING",
            "UNMENTIONED_FOURTH_LOW_RISK_DIRECT_CONFIRM_PATH",
        }
        or any(
            item.get("disposition") != "FIXED_BY_COMMIT_8316141"
            for item in superseded.get("concerns", [])
        )
        or any(
            item.get("replacement_tree_verification")
            != "A2_AND_B2_INDEPENDENTLY_APPROVED"
            for item in superseded.get("concerns", [])
        )
    ):
...
def preregistration_issues() -> list[str]:
    ...
    if (
        suite.get("schema_version") != "prd-readability-suite.v0.7"
    ...
    if (
        ...
        != "prd-readability-expected-envelope.v0.7"
    ...
    if (
        prereg.get("schema_version") != "prd-readability-preregistration.v0.7"
...
def emit_agent_workspace(target: Path) -> None:
    ...
    for semantic_case_id in CASE_IDS:
        ...
        export_manifest = {
            "schema_version": "prd-readability-v0.7-canonical-export.v1",
            "suite_id": SUITE_ID,
            "case_id": agent_case_id,
            "files": export_files,
            "evaluator_files_included": False,
...
def execution_manifest_shape_issues(value: Any) -> list[str]:
    ...
    issues: list[str] = []
    if value.get("schema_version") != "prd-readability-v0.7-execution-manifest.v1":
...
def _freeze_manifest_receipt(project_root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    receipt = {
        "schema_version": "prd-readability-v0.7-manifest-receipt.v1",
    ...
    for entry, path in zip(manifest["entries"], raw_paths, strict=True):
        ...
        try:
            ...
            try:
                ...
                rejection = {
                    "schema_version": "prd-readability-v0.7-public-preflight-rejection.v1",
...
def write_batch_validation_receipt(
    ...
    receipt = {
        "schema_version": "prd-readability-v0.7-batch-validation-receipt.v1",
...
def verify_batch_validation_receipt(
    ...
    if (
        ...
        != "prd-readability-v0.7-batch-validation-receipt.v1"
...
def mechanical_correction_record(
    ...
    if before_hash != after_hash:
        raise ValueError("mechanical correction changed semantic payload")
    return {
        "schema_version": "prd-readability-v0.7-mechanical-correction.v1",
...
def build_reviewer_work_order(
    ...
    *,
    ...
    order = {
        "schema_version": "prd-readability-v0.7-reviewer-work-order.v1",
...
def _canonical_export_files(case_root: Path, case_id: str) -> dict[str, dict[str, Any]]:
    ...
    if (
        ...
        or manifest.get("schema_version") != "prd-readability-v0.7-canonical-export.v1"
...
def emit_reviewer_projection(
    ...
    projection = {
        "schema_version": "prd-readability-v0.7-reviewer-projection.v1",
        "suite_id": SUITE_ID,
        "phase": entry["phase"],
        "case_id": entry["agent_case_id"],
        "run_id": entry["run_id"],
...
def contract_payload(*, export_requested: bool) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema_version": "prd-readability-v0.7-contract-report.v1",
