#!/usr/bin/env python3
"""Expose the v0.5 fixture-review gate without creating semantic evidence."""
...
def build_agent_case_payload(case_id: str) -> dict[str, Any]:
    ...
    if case_id not in CASE_IDS:
        raise ValueError(f"unknown v0.5 readability case: {case_id}")
...
def preregistration_issues() -> list[str]:
    ...
    if suite.get("schema_version") != "prd-readability-suite.v0.5":
        issues.append("suite: schema_version")
    if suite.get("suite_id") != "better-product-graph-prd-readability-v0.5":
    ...
    if expected.get("schema_version") != "prd-readability-expected-envelope.v0.5":
    ...
    if prereg.get("schema_version") != "prd-readability-preregistration.v0.5":
    ...
    required_gate = {
        ...
        "negative_attempt_policy": "EXACTLY_ONE_FINDING_ASSESSMENT_ONE_ISSUE_ONE_REPAIR_REGISTERED_PAIR_NO_EXTRA_FINDING",
...
def contract_payload(*, export_requested: bool) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema_version": "prd-readability-v0.5-contract-report.v1",
