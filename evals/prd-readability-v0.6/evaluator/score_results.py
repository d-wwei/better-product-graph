#!/usr/bin/env python3
"""Score the frozen v0.6 oracle from Controller-verified durable Runs only."""
...
SUITE_ID = "better-product-graph-prd-readability-v0.6"
...
def _expected() -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("schema_version") != "prd-readability-expected-envelope.v0.6"
...
def _preregistration() -> dict[str, Any]:
    ...
    if (
        not isinstance(value, dict)
        or value.get("schema_version") != "prd-readability-preregistration.v0.6"
        or value.get("suite_id") != SUITE_ID
...
def score_controller_runs(
    ...
    if freeze_issues:
        return {
            "schema_version": "prd-readability-v0.6-score-summary.v1",
            "suite_id": SUITE_ID,
            "status": "FAIL",
...
def load_run_registrations(path: Path) -> list[dict[str, Any]]:
    ...
    if (
        ...
        != "prd-readability-v0.6-run-registration.v1"
        or value.get("suite_id") != SUITE_ID
